...
Goal:
Output one mid-level driving command only. Do not output waypoints. The downstream controller will deterministically translate your command into constraints and local waypoints.

Objectives:
- avoid collision first
- maintain safe headway
- prefer smooth and conservative lane changes
- keep lane when lane change gain is small
- when uncertain, abstain and choose conservative follow/brake behavior
...
- If uncertain or scene is risky, set abstain=true and choose follow or brake.
...
class LLMPlanner:
    ...
    def _cache_put(self, scene: dict, parsed: dict, step_idx: Optional[int]) -> None:
        ...
        self.semantic_cache[signature] = payload
    ...
    def _descriptor_distance(self, a: dict, b: dict) -> Tuple[int, bool]:
        hard_match = (
            int(a.get("lane_id", 0)) == int(b.get("lane_id", 0))
            and int(a.get("left_clear", 0)) == int(b.get("left_clear", 0))
            and int(a.get("right_clear", 0)) == int(b.get("right_clear", 0))
            and int(a.get("risk_mode", 0)) == int(b.get("risk_mode", 0))
        )
        dist = (
            abs(int(a.get("front_distance_bucket", 0)) - int(b.get("front_distance_bucket", 0)))
            + abs(int(a.get("ttc_bucket", 0)) - int(b.get("ttc_bucket", 0)))
            + abs(int(a.get("ego_speed_bucket", 0)) - int(b.get("ego_speed_bucket", 0)))
            + abs(int(a.get("left_front_bucket", 0)) - int(b.get("left_front_bucket", 0)))
            + abs(int(a.get("right_front_bucket", 0)) - int(b.get("right_front_bucket", 0)))
        )
    ...
    def _cache_get(self, scene: dict, step_idx: Optional[int]) -> Optional[dict]:
        ...
        if payload is not None and self._cache_reuse_age_ok(payload, step_idx):
            ...
            out["recovery_source"] = "semantic_cache_exact"
            out["cache_hit"] = True
            out["cache_source"] = "exact"
            out["cache_signature"] = signature
            return out
        ...
        for cached in self.semantic_cache.values():
            ...
            if dist < best_dist:
                best_dist = dist
                best_payload = cached
        if best_payload is None or best_dist > 2:
            return None
        state = _ensure_runtime_llm_state(self.cfg)
        ...
        out["recovery_source"] = "semantic_cache_near"
        out["cache_hit"] = True
        out["cache_source"] = "near"
        out["cache_similarity_distance"] = int(best_dist)
        out["cache_signature"] = str(best_payload.get("signature", ""))
        return out
    ...
    def _translate_legacy_response(self, scene: dict, obj: dict, step_idx: Optional[int]) -> dict:
        ...
        elif target_lane > lane_id:
            behavior = "change_right"
        else:
            behavior = "follow" if float(constraints.get("min_headway", self.cfg.planner.normal_headway)) > self.cfg.planner.normal_headway else "keep_lane"
    ...
    def _parse_json_text(self, raw_text: str, scene: dict, step_idx: Optional[int]) -> dict:
        ...
        return self._parse_response_object(obj, scene, step_idx)
    ...
    def _recover_after_parse_failure(self, raw_response: str, scene: dict, prompt_hash: str, step_idx: Optional[int], error: Exception) -> dict:
        ...
        if repair_attempts > 0 and self.backend.name == "real":
            last_repair_error: Optional[Exception] = None
            for repair_idx in range(repair_attempts):
                try:
                    ...
                    parsed["recovery_source"] = "json_repair"
                    parsed["repaired_response"] = repaired
                    return parsed
                    ...
                    append_llm_call_log(self.cfg, {
                        ...
                        "stage": str(self.cfg.workflow.stage),
                        "mode": str(self.cfg.train.mode),
                        "backend": f"{self.backend.name}_repair",
                        "backend_requested": str(getattr(self.cfg.llm, "backend", self.backend.name)),
                        "backend_effective": str(self.backend.name),
        ...
        fallback = translate_midlevel_command(
            self.cfg,
            scene,
            build_safe_fallback_command(self.cfg, scene, reason="safe fallback after parse failure").get("command", {}),
            self.state,
            step_idx,
        )
        fallback["recovery_source"] = "safe_fallback"
        fallback["cache_hit"] = False
        fallback["cache_source"] = "none"
        return fallback
    ...
    def _maybe_replace_low_confidence_plan(self, parsed: dict, scene: dict, step_idx: Optional[int]) -> dict:
        ...
        fallback_reason = "safe fallback because llm abstained" if abstain else "safe fallback because llm confidence is low"
        state = _ensure_runtime_llm_state(self.cfg)
        state["num_safe_rule_degradations"] = int(state.get("num_safe_rule_degradations", 0)) + 1
        ...
        fallback = translate_midlevel_command(
            self.cfg,
            scene,
            build_safe_fallback_command(self.cfg, scene, reason=fallback_reason).get("command", {}),
            self.state,
            step_idx,
        )
        fallback["recovery_source"] = "low_confidence_safe_fallback"
        fallback["cache_hit"] = False
        fallback["cache_source"] = "none"
        return fallback
    ...
    def should_replan(self, scene: dict, step_idx: int) -> bool:
        if step_idx <= 0:
            return False
        reasons = []
        if step_idx % self.cfg.planner.planner_interval == 0:
            reasons.append("periodic")
        if float(scene["front_distance"]) < float(self.cfg.planner.replan_front_distance):
            reasons.append("front_distance")
        if float(scene["ttc"]) < float(self.cfg.planner.replan_ttc_threshold):
            reasons.append("ttc")
        if self.last_plan_step is not None and (step_idx - self.last_plan_step) >= max(1, self.cfg.planner.planner_interval * 2):
            reasons.append("plan_stale")
        if self.last_command_confidence < float(self.cfg.llm.min_command_confidence):
            reasons.append("low_confidence")
        current_signature = self._scene_signature(scene)
        if self.last_scene_signature and current_signature != self.last_scene_signature and _scene_risk_mode(self.cfg, scene) >= 1:
            reasons.append("scene_shift")
        self.last_trigger_reason = "+".join(reasons) if reasons else "none"
        return bool(reasons)
    ...
    def plan(self, scene: dict, step_idx: Optional[int] = None) -> dict:
        ...
        try:
            ...
            append_llm_call_log(self.cfg, {
                ...
                "stage": str(self.cfg.workflow.stage),
                "mode": str(self.cfg.train.mode),
                "backend": f"{self.backend.name}_generate",
                "backend_requested": str(getattr(self.cfg.llm, "backend", self.backend.name)),
                "backend_effective": "safe_rule",
                "status": "safe_rule_fallback",
                "prompt_hash": str(prompt_hash),
        ...
        try:
            ...
            parsed["recovery_source"] = "direct"
            parsed["cache_hit"] = False
            parsed["cache_source"] = "none"
        except Exception as e:
