...
def compute_scene_risk_vector(cfg, scene: dict, novelty: float = 0.0) -> dict:
    ...
    def _lane_change_risk(side: str) -> float:
        ...
        front_ttc = float(scene.get(f"{side}_front_ttc", 99.0))
        rear_ttc = float(scene.get(f"{side}_rear_ttc", 99.0))
        front_closing = float(scene.get(f"{side}_front_closing_speed", max(0.0, -float(scene.get(f"{side}_front_rel_speed", 0.0)))))
        rear_closing = float(scene.get(f"{side}_rear_closing_speed", max(0.0, -float(scene.get(f"{side}_rear_rel_speed", 0.0)))))
        ...
        risk = max(
            _squared_barrier_cost(front_d, 18.0),
            _squared_barrier_cost(rear_d, 12.0),
            _squared_barrier_cost(front_ttc, 4.5),
            _squared_barrier_cost(rear_ttc, 4.0),
            _clip01(front_closing / 8.0),
            _clip01(rear_closing / 8.0),
        )
        if not geom_clear:
            risk = max(risk, 0.45)
        if not dyn_clear:
    ...
    lane_width = max(float(cfg.env.lane_width), 1e-6)
    lane_offset = abs(float(scene.get("ego_y", 0.0)) - float(scene.get("lane_center_y", 0.0))) / lane_width
    ...
    max_longitudinal_risk = _clip01(max(front_collision, headway, overspeed))
    max_lane_change_risk = _clip01(max(left_change, right_change))
    max_context_risk = _clip01(max(0.45 * scene_complexity, 0.35 * _clip01(novelty), lane_stability, boundary_risk))
    current_plan_risk = _clip01(max(max_longitudinal_risk, 0.80 * lane_stability, boundary_risk))
    ...
    def check(self, scene: dict, action, constraints: dict) -> dict:
        ...
        return {
            ...
            "needs_intervention": bool(
                emergency or overspeed or large_lane_offset or lane_instability_high or over_acc or over_steer
            ),
        }
...
def compute_costs(cfg, scene: dict, next_scene: dict, action, constraints: dict) -> dict:
    ...
    collision_cost = _clip01(max(crash_cost, ttc_risk_cost, front_risk_cost))
    ...
    lane_cost = _clip01(max(float(lane_terms["lane_cost"]), offroad_cost))
...
class ConstrainedSACAgent(SACAgent):
    ...
    def _dual_signal(self, name: str, ema_cost: float, batch_cost_tensor: torch.Tensor, limit: float) -> tuple[float, float]:
        limit = max(float(limit), 1e-6)
        batch_cost = float(batch_cost_tensor.item())
        ema_violation = (float(ema_cost) - limit) / limit
        batch_violation = (batch_cost - limit) / limit
        mix = float(np.clip(self.cfg.sac.dual_batch_mix, 0.0, 1.0))
        error = (1.0 - mix) * ema_violation + mix * batch_violation
        error = float(np.clip(error, -float(self.cfg.sac.dual_violation_clip), float(self.cfg.sac.dual_violation_clip)))
        state = self.dual_pid_state[name]
        decay = float(np.clip(getattr(self.cfg.sac, "dual_integral_decay", 1.0), 0.0, 1.0))
        integral = float(state.get("integral", 0.0)) * decay + max(error, 0.0)
        integral = float(np.clip(integral, 0.0, float(getattr(self.cfg.sac, "dual_integral_clip", 0.0))))
        ...
        signal = float(np.clip(signal, -float(self.cfg.sac.dual_violation_clip), float(self.cfg.sac.dual_violation_clip)))
        return signal, error

    def _update_dual_lambda(self, lambda_tensor: torch.Tensor, signal: float, min_value: float, max_value: float, scale: float = 1.0) -> float:
        updated = float(lambda_tensor.item()) + float(self.cfg.sac.lambda_lr) * float(scale) * float(signal)
        updated = float(np.clip(updated, min_value, max_value))
    ...
    def update(self):
        ...
        collision_signal, collision_error = self._dual_signal("collision", float(self.ema_collision_cost), batch_collision_mean, self.cfg.sac.cost_limit_collision)
        headway_signal, headway_error = self._dual_signal("headway", float(self.ema_headway_cost), batch_headway_mean, self.cfg.sac.cost_limit_headway)
        overspeed_signal, overspeed_error = self._dual_signal("overspeed", float(self.ema_overspeed_cost), batch_overspeed_mean, self.cfg.sac.cost_limit_overspeed)
        comfort_signal, comfort_error = self._dual_signal("comfort", float(self.ema_comfort_cost), batch_comfort_mean, self.cfg.sac.cost_limit_comfort)
        lane_signal, lane_error = self._dual_signal("lane", float(self.ema_lane_cost), batch_lane_mean, self.cfg.sac.cost_limit_lane)

        self._update_dual_lambda(
        ...
        for ep in range(cfg.train.episodes):
            ...
            for t in range(cfg.train.max_steps_per_episode):
                ...
                if total_steps < cfg.sac.start_steps:
                    true_raw_action = _sample_teacher_guided_warmup_action(cfg, env, constraints, total_steps)
                else:
                    true_raw_action = agent.select_action(state, evaluate=False).astype(np.float32)
                if _should_use_safe_exploration_filter(cfg, total_steps):
                    true_raw_action = _apply_safe_exploration_filter(cfg, scene, true_raw_action, constraints, verifier, exploration_shield)
