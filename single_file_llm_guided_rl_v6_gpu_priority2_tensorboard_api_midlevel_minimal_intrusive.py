...
class PlannerConfig:
    ...
    query_min_interval_steps: int = 4
...
def compute_scene_risk_vector(cfg, scene: dict, novelty: float = 0.0) -> dict:
    ...
    left_change = _lane_change_risk("left") if scene.get("left_lane_id") is not None else 1.0
    right_change = _lane_change_risk("right") if scene.get("right_lane_id") is not None else 1.0
    ...
    risks["max_risk"] = float(max(risks["front_collision"], risks["headway"], risks["left_change"], risks["right_change"], risks["overspeed"], risks["lane_stability"]))
    return risks
...
class TeacherQueryGate:
    ...
    def evaluate(self, scene: dict, step_idx: int, last_plan_step: Optional[int], last_command_confidence: float, last_scene_signature: str, current_signature: str) -> dict:
        ...
        if last_scene_signature and current_signature != last_scene_signature and risk_vector["max_risk"] >= 0.35:
            reasons.append("scene_shift")
        high_priority = any(tag in reasons for tag in ["risk", "ttc", "front_distance"])
        if last_plan_step is not None and (step_idx - last_plan_step) < int(self.cfg.planner.query_min_interval_steps) and not high_priority:
            reasons = []
        return {"should_query": bool(reasons), "reasons": reasons, "risk_vector": risk_vector, "memory_context": memory_context, "novelty": novelty}
...
class LLMPlanner:
    ...
    def _build_query_context(self, scene: dict, step_idx: Optional[int]) -> dict:
        memory_context = self.memory.retrieve(scene, top_k=self.cfg.llm.memory_top_k)
        novelty = float(memory_context.get("novelty", 0.0))
        return {"memory_context": memory_context, "risk_vector": compute_scene_risk_vector(self.cfg, scene, novelty=novelty), "novelty": novelty, "reasons": ["initial_plan"] if step_idx is None else []}

    def should_replan(self, scene: dict, step_idx: int) -> bool:
        if step_idx <= 0:
            return False
        current_signature = self._scene_signature(scene)
        gate = self.query_gate.evaluate(scene=scene, step_idx=step_idx, last_plan_step=self.last_plan_step, last_command_confidence=self.last_command_confidence, last_scene_signature=self.last_scene_signature, current_signature=current_signature)
        self.last_trigger_reason = "+".join(gate["reasons"]) if gate["reasons"] else "none"
        self.last_query_context = {"memory_context": gate.get("memory_context", {}), "risk_vector": gate.get("risk_vector", {}), "novelty": float(gate.get("novelty", 0.0)), "reasons": list(gate.get("reasons", []))}
    ...
    def plan(self, scene: dict, step_idx: Optional[int] = None) -> dict:
        ...
        if parsed.get("recovery_source") in {"direct", "json_repair"}:
        ...
        if not self.memory_readonly and recovery_source in {"direct", "json_repair", "semantic_cache_exact", "semantic_cache_near", "low_confidence_cache"}:
