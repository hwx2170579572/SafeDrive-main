...
class LLMPlanner:
    ...
    def __init__(self, cfg, backend: BaseLLMBackend):
        ...
        self.last_valid_plan: Optional[dict] = None
    ...
    def _scene_signature(self, scene: dict) -> str:
        fd_bucket = int(min(12, max(0, float(scene.get("front_distance", 0.0)) // 5)))
        ttc_raw = float(scene.get("ttc", 99.0))
        ttc_bucket = 12 if ttc_raw >= 24.0 else int(max(0, ttc_raw // 2))
        speed_bucket = int(min(10, max(0, float(scene.get("ego_speed", 0.0)) // 3)))
        risk_mode = _scene_risk_mode(self.cfg, scene)
        return "|".join([
            f"lane={int(scene.get('lane_id', 0))}",
            f"fd={fd_bucket}",
            f"ttc={ttc_bucket}",
            f"spd={speed_bucket}",
            f"L={int(bool(scene.get('left_clear', False)))}",
            f"R={int(bool(scene.get('right_clear', False)))}",
            f"risk={risk_mode}",
        ])
    ...
    def _cache_put(self, scene: dict, parsed: dict, step_idx: Optional[int]) -> None:
        ...
        self.last_valid_plan = copy.deepcopy(payload)
    ...
    def _cache_get(self, scene: dict, step_idx: Optional[int]) -> Optional[dict]:
        ...
        if payload is None and self.last_valid_plan is not None:
            payload = self.last_valid_plan
        if payload is None:
            return None
        cached_step = payload.get("step_idx")
        if step_idx is not None and cached_step is not None:
            if (int(step_idx) - int(cached_step)) > int(self.cfg.llm.cache_reuse_max_age_steps):
                return None
        out = copy.deepcopy(payload.get("parsed", {}))
        out["recovery_source"] = "semantic_cache"
        out["cache_hit"] = True
        return out
