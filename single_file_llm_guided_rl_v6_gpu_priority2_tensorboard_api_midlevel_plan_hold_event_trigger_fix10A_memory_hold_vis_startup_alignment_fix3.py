...
class PlannerConfig:
    ...
    startup_alignment_hard_steps: int = 8
    ...
    startup_alignment_min_blend: float = 0.65
    startup_alignment_emergency_front_distance: float = 18.0
    startup_alignment_emergency_ttc: float = 4.0
    startup_alignment_target_speed_cap: float = 20.0
...
class ScenarioMemory:
    ...
    def _is_caution_case(self, compact: dict) -> bool:
        return bool(
            ...
            or int(compact.get("interventions", 0)) >= 1
        )
    ...
    def attach_step_outcome(
        ...
        cost_dict: Optional[dict],
    ) -> None:
        if self.readonly or step_idx is None or not episode_records:
            return
        target = None
        for record in reversed(episode_records):
            if record.get("step_idx") == int(step_idx) and record.get("outcome") is None:
                target = record
                break
        if target is None:
            return
        verify_info = dict(verify_info or {})
        shield_info = dict(shield_info or {})
        cost_dict = dict(cost_dict or {})
        target["outcome"] = {
            "success": int(not bool(next_scene.get("collision", False))),
            "collision": int(bool(next_scene.get("collision", False))),
            "episode_return": 0.0,
            "min_ttc": float(next_scene.get("ttc", 99.0)),
            "unsafe_headway_rate": float(1.0 if verify_info.get("unsafe_headway", False) else 0.0),
            "mean_cost_total": float(cost_dict.get("total_cost", 0.0)),
            "interventions": int(
                bool(shield_info.get("shield_modified", False))
                or bool(verify_info.get("needs_intervention", False))
                or bool(shield_info.get("recovery_active", False))
            ),
            "outcome_source": "step_local",
        }
...
class LLMPlanner:
    ...
    def observe_execution(self, scene: dict, verify_info: Optional[dict], shield_info: Optional[dict], step_idx: int, cost_dict: Optional[dict] = None) -> None:
        ...
        if not self.memory_readonly:
            self.memory.attach_step_outcome(self.episode_records, step_idx, scene, verify_info, shield_info, cost_dict)
    ...
    def plan(self, scene: dict, step_idx: Optional[int] = None) -> dict:
        ...
        if (not self.memory_readonly) and (step_idx is not None) and recovery_source in {"direct", "json_repair"}:
...
def apply_startup_alignment_override(cfg, scene: dict, action, constraints: dict, planner_info: Optional[dict], step_idx: Optional[int]):
    ...
    emergency_close = bool(
        front_distance < float(cfg.planner.startup_alignment_emergency_front_distance)
        or ttc < float(cfg.planner.startup_alignment_emergency_ttc)
    )

    # fix3: startup alignment always references the current lane center during the startup window.
    target_speed_plan = float(planner_info.get("target_speed", min(float(constraints.get("speed_max", cfg.planner.target_speed_free)), cfg.planner.target_speed_free)))
    startup_speed_cap = float(np.clip(
        float(cfg.planner.startup_alignment_target_speed_cap),
        float(cfg.env.min_speed) + 1.0,
        float(constraints.get("speed_max", cfg.planner.target_speed_free)),
    ))
    if emergency_close:
        desired_speed = float(np.clip(min(target_speed_plan, ego_speed + 0.5), cfg.env.min_speed, startup_speed_cap))
    else:
        desired_speed = float(np.clip(
            min(target_speed_plan, startup_speed_cap, max(float(cfg.planner.target_speed_cautious), ego_speed + 2.0)),
            cfg.env.min_speed,
            startup_speed_cap,
        ))
    ...
    if (not emergency_close) and bool(cfg.planner.startup_alignment_force_positive_accel) and desired_speed >= ego_speed + 0.5:
    ...
    raw_brake_bad = float(raw_action[0]) < -float(cfg.planner.startup_alignment_raw_brake_guard_ratio) * acc_bound and (not emergency_close)
    ...
    if hard_phase or startup_guard:
        ...
        blend = float(np.clip(
            ...
            float(cfg.planner.startup_alignment_min_blend),
            1.0,
        ))
        mode = "cautious_blend" if emergency_close else "blend"
    ...
    info.update({
        ...
        "startup_alignment_reason": "startup_lane_hold" if not emergency_close else "startup_lane_hold_cautious",
        ...
        try:
            ...
            for t in range(cfg.train.max_steps_per_episode):
                ...
                if planner is not None and hasattr(planner, "observe_execution"):
                    planner.observe_execution(next_scene, verify_info, shield_info, t, cost_dict=cost_dict)
