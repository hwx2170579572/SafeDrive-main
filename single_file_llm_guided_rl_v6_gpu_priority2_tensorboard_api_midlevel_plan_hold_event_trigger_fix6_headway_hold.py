...
class PlannerConfig:
    ...
    hold_min_steps_safe_cruise: int = 20
    hold_min_steps_very_safe: int = 28
    hold_max_steps: int = 36
...
def build_plan_validity(cfg, scene: dict, command: dict, decision: dict) -> dict:
    ...
    very_safe_cruise = bool((not lane_change_required) and front_distance >= float(cfg.planner.very_safe_front_distance) and ttc >= float(cfg.planner.very_safe_ttc) and risk_mode <= 0 and closing_speed < 2.0)
    safe_cruise = bool((not lane_change_required) and front_distance >= float(cfg.planner.safe_cruise_front_distance) and ttc >= float(cfg.planner.safe_cruise_ttc) and risk_mode <= 0 and closing_speed < float(cfg.planner.closing_speed_hard_threshold))
    near_risk = bool(risk_mode >= 1 or front_distance < float(cfg.planner.near_risk_front_distance) or ttc < float(cfg.planner.near_risk_ttc) or (front_distance < float(cfg.planner.closing_speed_near_front_distance) and closing_speed >= float(cfg.planner.closing_speed_soft_threshold)))
    ...
    if lane_change_required:
        ...
        hold_default = max(int(cfg.planner.hold_min_steps_very_safe), int(cfg.planner.hold_min_steps_default) + 12)
        hold_lb, hold_ub = max(24, int(cfg.planner.hold_min_steps_very_safe) - 2), min(int(cfg.planner.hold_max_steps), 36)
    elif safe_cruise:
        hold_default = max(int(cfg.planner.hold_min_steps_safe_cruise), int(cfg.planner.hold_min_steps_default) + 6)
        hold_lb, hold_ub = max(18, int(cfg.planner.hold_min_steps_safe_cruise) - 2), min(int(cfg.planner.hold_max_steps), 30)
    else:
        hold_default = max(12, int(cfg.planner.hold_min_steps_default))
        hold_lb, hold_ub = 10, min(int(cfg.planner.hold_max_steps), 20)
    ...
    if very_safe_cruise:
        fd_default = float(np.clip(max(90.0, front_distance * 0.40), 90.0, 180.0))
        ttc_default = float(np.clip(max(10.0, ttc * 0.28), 10.0, 18.0))
    elif safe_cruise:
        fd_default = float(np.clip(max(70.0, front_distance * 0.38), 70.0, 140.0))
        ttc_default = float(np.clip(max(8.0, ttc * 0.25), 8.0, 14.0))
...
def _teacher_weight_from_guidance(cfg, command: dict, risk_vector: dict, novelty: float, recovery_source: str) -> float:
    ...
    if front_collision >= 0.15 or headway >= 0.08:
        base = max(base, 0.65 * confidence + 0.20)
    elif risk_max < 0.15:
        base *= 0.30
    elif risk_max < 0.30:
        base *= 0.55
...
def translate_midlevel_command(cfg, scene: dict, command: dict, planner_state: dict, step_idx: Optional[int]) -> dict:
    ...
    if closing_pressure > 0.0:
        requested_speed = min(requested_speed, max(float(cfg.env.min_speed), float(scene.get("ego_speed", 0.0)) - closing_pressure))
