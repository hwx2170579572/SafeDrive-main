...
def enforce_workflow_stage_policy(cfg: Config, requested_mode: str, is_compare: bool) -> None:
    ...
    if stage == "dev":
        if is_compare:
            ...
            if any(m in REAL_LLM_MODES for m in candidate_modes) and not cfg.workflow.allow_real_llm_smoke:
                raise RuntimeError(
                    "Dev stage forbids real LLM modes in compare. Use --allow-real-llm-smoke for smoke tests only."
                )
        else:
            if requested_mode in REAL_LLM_MODES and not (cfg.workflow.allow_real_llm_smoke or cfg.workflow.smoke_test):
                raise RuntimeError(
...
class RealLLMBackend(BaseLLMBackend):
    ...
    def _call_real_api(self, prompt: str) -> str:
        ...
        if not api_key:
            raise RuntimeError("RealLLMBackend requires cfg.llm.api_key or OPENAI_API_KEY.")
        if not api_base:
            raise RuntimeError("RealLLMBackend requires cfg.llm.api_base or OPENAI_API_BASE.")
        headers = {
...
class ConstrainedSACAgent(SACAgent):
    """
    First-priority stabilized constrained SAC + lightweight diagnostics:
    - actor penalty uses only collision + headway
    - overspeed + comfort critics/lambdas are kept as diagnostic branches
    - lambda updates use batch immediate-cost mean, not critic predictions
    """
    ...
    def update(self):
        ...
        # Overspeed / comfort lambdas are updated only for diagnosis.
        lambda_loss = -(
            self.log_lambda_collision * (batch_collision_mean - self.cfg.sac.cost_limit_collision)
            + self.log_lambda_headway * (batch_headway_mean - self.cfg.sac.cost_limit_headway)
            + self.log_lambda_overspeed * (batch_overspeed_mean - self.cfg.sac.cost_limit_overspeed)
            + self.log_lambda_comfort * (batch_comfort_mean - self.cfg.sac.cost_limit_comfort)
        )
