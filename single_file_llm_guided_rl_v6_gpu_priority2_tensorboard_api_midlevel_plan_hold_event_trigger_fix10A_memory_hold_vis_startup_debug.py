    ...
    for ep in range(cfg.eval.episodes):
        ...
        try:
            ...
            for t in range(cfg.train.max_steps_per_episode):
                ...
                recovery_action, recovery_info = apply_local_recovery_override(cfg, scene, raw_action, constraints)

                verify_info = verifier.check(scene, recovery_action, constraints) if verifier is not None else {}
                shield_info = {"shield_modified": False, "shield_reasons": [], "recovery_active": bool(recovery_info.get("recovery_active", False)), "recovery_reason": str(recovery_info.get("recovery_reason", "none"))}
