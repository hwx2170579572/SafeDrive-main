...
class RewardConfig:
    ...
    w_overspeed = 0.9
    w_lane_center = 0.45
    w_lane_boundary = 1.25
    w_heading_align = 0.18
...
class SACConfig:
    ...
    min_lambda_lane: float = 0.010

    teacher_guidance_coef: float = 0.40
    teacher_action_coef: float = 1.10
    behavior_kl_coef: float = 0.10
    lane_kl_coef: float = 0.08
    actor_overspeed_penalty_coef: float = 0.10
    actor_lane_penalty_coef: float = 1.00
    corrective_teacher_weight_floor: float = 0.35
    corrective_teacher_weight_cap: float = 0.95
...
class SafetyConfig:
    ...
    road_boundary_soft_margin: float = 1.25
    road_boundary_hard_margin: float = 0.45
...
class HighwayEnvWrapper:
    ...
    def build_low_state(self, obs) -> np.ndarray:
        ...
        scene_feat = np.array([
            ...
            float(np.clip(scene.get("road_left_margin", 0.0) / max(self.cfg.env.lane_width * 2.0, 1e-6), 0.0, 1.5)),
            float(np.clip(scene.get("road_right_margin", 0.0) / max(self.cfg.env.lane_width * 2.0, 1e-6), 0.0, 1.5)),
    ...
    def get_scene_dict(self) -> dict:
        ...
        lane_width = max(float(self.cfg.env.lane_width), 1e-6)
        ...
        try:
            lateral_speed = float(getattr(ego, "velocity", np.zeros((2,), dtype=np.float32))[1])
        except Exception:
            lateral_speed = float(ego_speed * np.sin(heading))
        ...
        return {
            ...
            "ego_speed": ego_speed,
            "heading": heading,
            "heading_error": float(_wrap_to_pi(heading)),
...
def _lane_boundary_risk_terms(cfg, scene: dict) -> dict:
    ...
    lane_cost = _clip01(max(
        offroad_term,
        0.30 * center_soft_term + 0.65 * center_hard_term + 0.75 * road_soft_term + 1.10 * road_hard_term + 0.25 * heading_term + 0.20 * lateral_term,
    ))
    return {
...
def compute_scene_risk_vector(cfg, scene: dict, novelty: float = 0.0) -> dict:
    ...
    lane_stability = _clip01(
        0.55 * _clip01(max(0.0, lane_offset - 0.10) / 1.10)
        + 0.45 * _clip01(heading_abs / 1.20)
    )
    ...
    max_context_risk = _clip01(max(0.50 * scene_complexity, 0.35 * _clip01(novelty)))
    current_plan_risk = _clip01(max(max_longitudinal_risk, max_context_risk))
    global_scene_risk = _clip01(max(current_plan_risk, max_lane_change_risk))
    max_risk = float(current_plan_risk)
...
class ScenarioMemory:
    ...
    def retrieve(self, scene: dict, top_k: Optional[int] = None) -> dict:
        ...
        caution_pool = [c for c in compact_cases if str(c.get("tag", "success")) != "success"]
        ...
        # If memory is heavily skewed to low-speed keep-lane successes, reserve one slot for a faster but still similar safe success.
...
def translate_midlevel_command(cfg, scene: dict, command: dict, planner_state: dict, step_idx: Optional[int]) -> dict:
    ...
    closing_near = front_distance < float(cfg.planner.closing_speed_near_front_distance)
    closing_mid = front_distance < float(cfg.planner.closing_speed_mid_front_distance)
    closing_pressure = 0.0
    if closing_speed >= float(cfg.planner.closing_speed_soft_threshold) and (closing_near or ttc < float(cfg.planner.closing_speed_ttc)):
        closing_pressure = max(closing_pressure, (closing_speed - float(cfg.planner.closing_speed_soft_threshold)) * 1.2)
    if closing_speed >= float(cfg.planner.closing_speed_hard_threshold) and (closing_mid or ttc < float(cfg.planner.closing_speed_ttc) + 4.0):
        closing_pressure = max(closing_pressure, 2.5 + (closing_speed - float(cfg.planner.closing_speed_hard_threshold)) * 1.5)
    if front_distance < float(cfg.planner.near_risk_front_distance):
        closing_pressure = max(closing_pressure, max(0.0, float(cfg.planner.near_risk_front_distance) - front_distance) / 10.0)
    if ttc < float(cfg.planner.near_risk_ttc):
        closing_pressure = max(closing_pressure, max(0.0, float(cfg.planner.near_risk_ttc) - ttc) * 1.4)
    if closing_pressure > 0.0:
        requested_speed = min(requested_speed, max(float(cfg.env.min_speed), ego_speed - closing_pressure))
        behavior = "follow" if behavior in {"keep_lane", "follow"} else behavior
        headway_level = "conservative"
        if closing_pressure >= 3.5:
            speed_mode = "brake"
        elif speed_mode == "free":
            speed_mode = "cautious"
    ...
    if safe_cruise and current_overspeed_gap <= 0.0 and not near_overspeed:
        safe_recovery_floor = float(cfg.planner.safe_cruise_target_speed_floor)
        if front_distance >= float(cfg.planner.very_safe_front_distance) and ttc >= float(cfg.planner.very_safe_ttc):
            safe_recovery_floor = max(safe_recovery_floor, min(float(cfg.planner.target_speed_free), ego_speed + 2.5))
        else:
            safe_recovery_floor = max(safe_recovery_floor, min(float(cfg.planner.target_speed_free) - 1.0, ego_speed + 1.5))
        requested_speed = max(
            requested_speed,
            min(float(cfg.planner.target_speed_free), safe_recovery_floor)
        )
        if behavior == "follow":
            behavior = "keep_lane"
        if headway_level == "conservative" and closing_pressure <= 0.0:
            headway_level = "normal"
        if speed_mode == "brake" and closing_pressure <= 0.0:
            speed_mode = "cautious"
    ...
    if current_overspeed_gap > 0.0:
        ...
        speed_max = float(np.clip(max(target_speed + 1.5, min(ego_speed + 0.8, overspeed_hard_limit)), cfg.env.min_speed, overspeed_hard_limit))
    else:
...
def apply_startup_alignment_override(cfg, scene: dict, action, constraints: dict, planner_info: Optional[dict], step_idx: Optional[int]):
    ...
    planner_info = dict(planner_info or {})
    lane_id = int(scene.get("lane_id", 0))
    target_lane = int(planner_info.get("target_lane", lane_id))
    behavior = _normalize_behavior_token(planner_info.get("behavior", "keep_lane"), default="keep_lane")
    front_distance = float(scene.get("front_distance", 1e6))
    ttc = float(scene.get("ttc", 99.0))
    ego_speed = float(scene.get("ego_speed", 0.0))
    lane_width = max(float(cfg.env.lane_width), 1e-6)
    lane_offset_norm = (float(scene.get("ego_y", 0.0)) - float(scene.get("lane_center_y", 0.0))) / lane_width
    heading = _wrap_to_pi(float(scene.get("heading", 0.0)))
    info = {
        "startup_alignment_active": False,
        "startup_alignment_reason": "inactive",
        "startup_alignment_action": raw_action.copy(),
        "startup_alignment_blend": 0.0,
        "startup_alignment_target_speed": float(planner_info.get("target_speed", ego_speed)),
        "startup_alignment_mode": "inactive",
        "lane_offset_norm": float(lane_offset_norm),
    ...
    if (not bool(cfg.planner.startup_alignment_enabled)) or step_idx is None:
        info["startup_alignment_reason"] = "disabled_or_no_step"
        return raw_action, info
    if int(step_idx) >= int(cfg.planner.startup_alignment_steps):
        info["startup_alignment_reason"] = "after_window"
        return raw_action, info
    if behavior not in {"keep_lane", "follow"}:
        info["startup_alignment_reason"] = "behavior_excluded"
        return raw_action, info
    if target_lane != lane_id:
        info["startup_alignment_reason"] = "target_lane_mismatch"
        return raw_action, info
    if front_distance < float(cfg.planner.startup_alignment_min_front_distance) or ttc < float(cfg.planner.startup_alignment_min_ttc):
        info["startup_alignment_reason"] = "traffic_not_clear"
        return raw_action, info
    if abs(heading) > float(cfg.planner.startup_alignment_max_abs_heading):
        info["startup_alignment_reason"] = "heading_too_large"
        return raw_action, info
    if abs(lane_offset_norm) > float(cfg.planner.startup_alignment_max_abs_lane_offset):
        info["startup_alignment_reason"] = "offset_too_large"
        return raw_action, info

    acc_bound, steer_bound = physical_to_normalized_action_bounds(cfg, constraints)
    target_speed = float(planner_info.get("target_speed", min(float(constraints.get("speed_max", cfg.planner.target_speed_free)), cfg.planner.target_speed_free)))
    desired_speed = float(np.clip(target_speed, cfg.env.min_speed, float(constraints.get("speed_max", cfg.planner.target_speed_free))))
    desired_acc = float(np.clip(
        float(cfg.planner.startup_alignment_speed_k) * (desired_speed - ego_speed),
        -float(cfg.planner.startup_alignment_brake_clip) * acc_bound,
        float(cfg.planner.startup_alignment_acc_clip) * acc_bound,
    ))
    if bool(cfg.planner.startup_alignment_force_positive_accel) and desired_speed >= ego_speed + 1.0:
        desired_acc = max(desired_acc, 0.0)
    desired_steer = float(np.clip(
        -float(cfg.planner.startup_alignment_lane_k) * lane_offset_norm - float(cfg.planner.startup_alignment_heading_k) * heading,
        -float(cfg.planner.startup_alignment_steer_clip) * steer_bound,
        float(cfg.planner.startup_alignment_steer_clip) * steer_bound,
    ))

    hard_phase = int(step_idx) < int(cfg.planner.startup_alignment_hard_steps)
    raw_steer_bad = abs(float(raw_action[1])) > float(cfg.planner.startup_alignment_raw_steer_guard_ratio) * steer_bound
    raw_brake_bad = float(raw_action[0]) < -float(cfg.planner.startup_alignment_raw_brake_guard_ratio) * acc_bound and desired_speed >= ego_speed + 1.0
    startup_guard = bool(raw_steer_bad or raw_brake_bad)

    if hard_phase or startup_guard:
        blend = 1.0
        mode = "hard_override" if startup_guard else "hard_phase"
    else:
        progress = float(step_idx) / max(float(cfg.planner.startup_alignment_steps - 1), 1.0)
        blend = float(np.clip(
            float(cfg.planner.startup_alignment_blend_start)
            + (float(cfg.planner.startup_alignment_blend_end) - float(cfg.planner.startup_alignment_blend_start)) * progress,
            0.0,
            1.0,
        ))
        mode = "blend"

    aligned_action = (1.0 - blend) * raw_action + blend * np.asarray([desired_acc, desired_steer], dtype=np.float32)
    aligned_action = np.asarray([
        float(np.clip(aligned_action[0], -acc_bound, acc_bound)),
        float(np.clip(aligned_action[1], -steer_bound, steer_bound)),
    ], dtype=np.float32)

    info.update({
        "startup_alignment_active": bool(blend > 0.0),
        "startup_alignment_reason": "startup_keep_lane_alignment",
        "startup_alignment_action": aligned_action.copy(),
        "startup_alignment_blend": float(blend),
        "startup_alignment_target_speed": float(desired_speed),
        "startup_alignment_mode": str(mode),
        "raw_action_after_startup_alignment": aligned_action.copy(),
    })
    return aligned_action, info


def apply_local_recovery_override(cfg, scene: dict, action, constraints: dict):
    raw_action = np.asarray(action, dtype=np.float32).copy()
    lane_width = max(float(cfg.env.lane_width), 1e-6)
    lane_offset = (float(scene.get("ego_y", 0.0)) - float(scene.get("lane_center_y", 0.0))) / lane_width
    heading = _wrap_to_pi(float(scene.get("heading", 0.0)))
    offset_abs = abs(lane_offset)
    heading_abs = abs(heading)
    recovery_active = bool(offset_abs > 1.0 or heading_abs > 1.2)
    info = {
    ...
    acc_bound, steer_bound = physical_to_normalized_action_bounds(cfg, constraints)
    severity = max(0.0, offset_abs - 1.0, (heading_abs - 1.2) / 1.2)
    target_speed = float(np.clip(18.0 - 4.0 * severity, 12.0, 18.0))
    ego_speed = float(scene.get("ego_speed", 0.0))
    acc = float(np.clip(0.18 * (target_speed - ego_speed), -0.90 * acc_bound, 0.90 * acc_bound))
    if ego_speed < 0.0:
        acc = max(acc, min(0.60 * acc_bound, 0.35))
    steer = float(np.clip(-0.55 * lane_offset - 0.35 * heading, -0.90 * steer_bound, 0.90 * steer_bound))
    reason = "recenter"
    if offset_abs > 1.4 or heading_abs > 1.8:
        target_speed = 12.0
        acc = float(np.clip(0.18 * (target_speed - ego_speed), -0.95 * acc_bound, 0.95 * acc_bound))
        if ego_speed < 0.0:
            acc = max(acc, min(0.70 * acc_bound, 0.45))
        steer = float(np.clip(1.15 * steer, -0.95 * steer_bound, 0.95 * steer_bound))
        reason = "hard_recenter"
    recovery_action = np.asarray([acc, steer], dtype=np.float32)
    info.update({
        "recovery_reason": str(reason),
        "recovery_target_speed": float(target_speed),
        "recovery_action": recovery_action.copy(),
    })
    return recovery_action, info
...
def compute_costs(cfg, scene: dict, next_scene: dict, action, constraints: dict) -> dict:
    ...
    headway_hard = headway_gap / max(min_headway, 1e-6)
    ...
    headway_cost = max(headway_hard, 0.55 * headway_proactive + float(cfg.planner.headway_closing_speed_factor) * closing_penalty)

    speed_max = float(constraints["speed_max"])
    next_speed = float(next_scene["ego_speed"])
    overspeed_gap = max(0.0, next_speed - speed_max)
    overspeed_hard = overspeed_gap / max(speed_max, 1e-6)
    ...
    if next_speed > overspeed_proactive_threshold and float(action[0]) > 0.05:
        overspeed_proactive += 0.15 * float(np.clip(action[0], 0.0, 1.0))
    overspeed_cost = max(overspeed_hard, float(cfg.planner.overspeed_proactive_weight) * overspeed_proactive)

    lane_terms = _lane_boundary_risk_terms(cfg, next_scene)
    lane_cost = float(lane_terms["lane_cost"])
    ...
    return {
        ...
        "offroad_cost": float(lane_terms["offroad_term"]),
...
def compute_reward(
    ...
    lane_terms = _lane_boundary_risk_terms(cfg, next_scene)
    lane_center_penalty = float((lane_track_error ** 2) + 0.35 * lane_terms["center_soft_term"] + 0.55 * lane_terms["center_hard_term"])
    lane_boundary_penalty = float(max(lane_terms["road_soft_term"], 0.80 * lane_terms["road_hard_term"], lane_terms["offroad_term"]))
    heading_penalty = float(lane_terms["heading_term"] + 0.5 * lane_terms["lateral_term"])
    ...
    speed_max = float(constraints["speed_max"])
    next_speed = float(next_scene["ego_speed"])
    overspeed_hard = max(0.0, next_speed - speed_max) / max(speed_max, 1e-6)
    overspeed_proactive_threshold = float(cfg.planner.overspeed_proactive_ratio) * speed_max
    proactive_gap = max(0.0, next_speed - overspeed_proactive_threshold)
    proactive_denom = max(speed_max - overspeed_proactive_threshold, 1e-6)
    overspeed_proactive = float((proactive_gap / proactive_denom) ** 2)
    overspeed_penalty = max(overspeed_hard, 0.35 * overspeed_proactive)
...
class SACAgent:
    def __init__(self, state_dim, action_dim, cfg):
        ...
        self.replay = ReplayBuffer(state_dim, action_dim, cfg.sac.buffer_size, self.device, behavior_prior_dim=self.behavior_dim, lane_prior_dim=self.lane_dim)
    ...
    def _teacher_guidance_loss(self, batch: dict, new_action: torch.Tensor, behavior_logits: torch.Tensor, lane_logits: torch.Tensor) -> tuple[torch.Tensor, dict]:
        ...
        action_loss = (teacher_weight * action_mse).mean() * float(self.cfg.sac.teacher_guidance_coef) * float(self.cfg.sac.teacher_action_coef)
        ...
        behavior_loss = (teacher_weight * behavior_kl).mean() * float(self.cfg.sac.teacher_guidance_coef) * float(self.cfg.sac.behavior_kl_coef)
        lane_loss = (teacher_weight * lane_kl).mean() * float(self.cfg.sac.teacher_guidance_coef) * float(self.cfg.sac.lane_kl_coef)
...
class ConstrainedSACAgent(SACAgent):
    """
    Second-priority stabilized constrained SAC + stronger lane-boundary learning:
    - actor penalty uses collision + headway + overspeed + explicit lane-boundary critic
    - comfort critic/lambda are kept as diagnostic branches
    - lambda updates use EMA-smoothed immediate costs
    - collision/headway/lane lambdas have non-zero lower bounds
    - lane cost is dense and road-boundary aware before off-road happens
    """
    ...
    def __init__(self, state_dim, action_dim, cfg):
        ...
        self.log_lambda_lane = torch.tensor(np.log(init_lambda), requires_grad=True, device=self.device)
    ...
    def lambda_lane(self):
        return self.log_lambda_lane.exp()
    ...
    def update(self):
        ...
        actor_penalty = (
            ...
            + float(self.cfg.sac.actor_overspeed_penalty_coef) * self.lambda_overspeed.detach() * qc_overspeed_new
            + float(self.cfg.sac.actor_lane_penalty_coef) * self.lambda_lane.detach() * qc_lane_new
        )
        ...
        ema_lane_tensor = torch.tensor(self.ema_lane_cost, dtype=torch.float32, device=self.device)
        ...
        lambda_loss = -(
            ...
            + self.log_lambda_lane * (ema_lane_tensor - self.cfg.sac.cost_limit_lane)
        )
        ...
        self.log_lambda_lane.data.clamp_(min=float(np.log(max(self.cfg.sac.min_lambda_lane, 1e-8))), max=max_log_lambda)
        ...
        out = {
            ...
            "qc_lane": float(qc_lane_new.mean().item()),
            ...
            "qc_lane_target": float(qc_lane_target_mean.item()),
...
class EpisodeMetrics:
    ...
    def update(self, scene: dict, next_scene: dict, action, reward: float, replanned: bool,
        ...
        if next_scene["collision"]:
            self.collided = True
        if float(next_scene.get("road_boundary_margin", 1e6)) < 0.05:
    ...
    for ep in range(cfg.eval.episodes):
        ...
        try:
            ...
            for t in range(cfg.train.max_steps_per_episode):
                ...
                aligned_action, alignment_info = apply_startup_alignment_override(cfg, scene, raw_action, constraints, latest_plan_info, t)
                recovery_action, recovery_info = apply_local_recovery_override(cfg, scene, aligned_action, constraints)

                verify_info = verifier.check(scene, recovery_action, constraints) if verifier is not None else {}
                shield_info = {
                ...
                if shield is not None:
                    action, shield_info_core = shield.apply(scene, recovery_action, constraints, verify_info)
                ...
                if _startup_debug_log_enabled(cfg) and t < _startup_debug_steps(cfg):
                    ...
                    startup_record = {
                        ...
                        "raw_action_before_startup_alignment": _to_float_list(alignment_info.get("raw_action_before_startup_alignment", raw_action)),
...
def write_train_diagnostics_tensorboard(writer, row: dict, mode: str) -> None:
    ...
    if mode_is_constrained(mode):
        ...
        for key in ['ema_collision_cost', 'ema_headway_cost', 'ema_lane_cost']:
            value = float(row.get(key, float('nan')))
            if np.isfinite(value):
...
def build_augmented_teacher_guidance(
    ...
    if bool((alignment_info or {}).get("startup_alignment_active", False)):
        corrective_action = np.asarray(aligned_action, dtype=np.float32)
        corrective_source = "startup_alignment"
        weight_floor = max(weight_floor, float(cfg.sac.corrective_teacher_weight_floor) + 0.20 + 0.25 * correction_pressure)
    if bool((recovery_info or {}).get("recovery_active", False)):
        corrective_action = np.asarray(recovery_action, dtype=np.float32)
        corrective_source = "local_recovery"
        weight_floor = max(weight_floor, float(cfg.sac.corrective_teacher_weight_floor) + 0.35 + 0.30 * correction_pressure)
    shield_reasons = set((shield_info or {}).get("shield_reasons", []))
    if bool((shield_info or {}).get("shield_modified", False)) and ("steer_suppression" in shield_reasons or "emergency_brake" in shield_reasons or "overspeed_brake" in shield_reasons):
        corrective_action = np.asarray(final_action, dtype=np.float32)
        corrective_source = "safety_shield"
        weight_floor = max(weight_floor, float(cfg.sac.corrective_teacher_weight_floor) + 0.45 + 0.30 * correction_pressure)

    if corrective_action is None and guidance is None:
        return None
    ...
    if guidance is None:
        guidance = {
            ...
            "teacher_weight": 0.0,
            "teacher_target_speed": float(scene.get("ego_speed", 0.0)),
            "teacher_target_lane": float(scene.get("lane_id", 0)),
            "behavior_prior": behavior_prior,
            "lane_prior": lane_prior,
            "confidence": 1.0,
            "risk_vector": {"max_risk": float(max(road_proximity, lane_proximity))},
            "novelty": 0.0,
    ...
    guidance["teacher_weight"] = float(np.clip(max(base_weight, weight_floor), 0.0, float(cfg.sac.corrective_teacher_weight_cap)))
    ...
    try:
        for ep in range(cfg.train.episodes):
            ...
            for t in range(cfg.train.max_steps_per_episode):
                ...
                aligned_action, alignment_info = apply_startup_alignment_override(cfg, scene, raw_action, constraints, latest_plan_info_train if planner is not None else {}, t)
                recovery_action, recovery_info = apply_local_recovery_override(cfg, scene, aligned_action, constraints)
