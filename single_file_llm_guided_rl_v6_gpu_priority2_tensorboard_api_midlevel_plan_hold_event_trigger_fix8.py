...
def compute_scene_risk_vector(cfg, scene: dict, novelty: float = 0.0) -> dict:
    ...
    last_action = np.asarray(scene.get("last_action", np.zeros((2,), dtype=np.float32)), dtype=np.float32).reshape(-1)
    steer_mag = abs(float(last_action[1])) if last_action.shape[0] >= 2 else 0.0
    closing_speed = max(0.0, -float(scene.get("front_rel_speed", 0.0)))
    lane_stability = _clip01(max(0.0, steer_mag - 0.12) / 0.38)
    if front_distance < 30.0 and closing_speed > 3.0:
        lane_stability = max(lane_stability, 0.08)
    ...
    max_context_risk = _clip01(max(0.50 * scene_complexity, 0.20 * lane_stability, 0.35 * _clip01(novelty)))
    max_risk = _clip01(max(max_longitudinal_risk, max_lane_change_risk, max_context_risk))
...
def run_offline_tests() -> dict:
    ...
    _record("lane_change_risk_aggregates_into_max_risk", rv["max_lane_change_risk"] >= 0.70 and rv["max_risk"] >= rv["max_lane_change_risk"], f"risk={rv}")
