...
EVAL_VIDEO_RUNTIME_STATE = {'saved_count': 0, 'warned_dual_mode': False}
...
def _is_sensitive_config_key(key: Any) -> bool:
    ...
    if text in SENSITIVE_CONFIG_KEY_EXACT:
        return True
    if "api_key" in text or "authorization" in text:
        return True
    if "secret" in text:
        return True
    if text == "token" or text.endswith("_token"):
        return True
    return False
    ...
    if isinstance(value, dict):
        ...
        for k, v in value.items():
            if _is_sensitive_config_key(k):
                ...
                redacted[str(k)] = _redact_sensitive_payload(v)
        return redacted
    if isinstance(value, list):
        return [_redact_sensitive_payload(v) for v in value]
    if isinstance(value, tuple):
        return [_redact_sensitive_payload(v) for v in value]
    return _jsonable_plain(value)
...
class PlannerConfig:
    ...
    hold_min_steps_risky: int = 6
    hold_min_steps_safe_cruise: int = 36
    hold_min_steps_very_safe: int = 48
    hold_max_steps: int = 84
    ...
    safe_cruise_target_speed_floor: float = 22.0
    overspeed_soft_margin: float = 1.5
    overspeed_hard_margin: float = 0.5
    overspeed_recovery_speed_drop: float = 2.5
    overspeed_proactive_ratio: float = 0.92
    overspeed_proactive_weight: float = 0.40
    closing_speed_soft_threshold: float = 2.0
    closing_speed_hard_threshold: float = 4.0
    closing_speed_near_front_distance: float = 80.0
    closing_speed_mid_front_distance: float = 140.0
    closing_speed_ttc: float = 10.0
    headway_proactive_factor: float = 1.60
    headway_closing_speed_factor: float = 0.35
    headway_shape_closing_threshold: float = 1.5
    ...
    startup_alignment_enabled: bool = False
    startup_alignment_steps: int = 20
    startup_alignment_min_front_distance: float = 80.0
    startup_alignment_min_ttc: float = 12.0
    startup_alignment_max_abs_heading: float = 0.80
    startup_alignment_max_abs_lane_offset: float = 0.75
    startup_alignment_blend_start: float = 0.90
    startup_alignment_blend_end: float = 0.25
    startup_alignment_lane_k: float = 0.70
    startup_alignment_heading_k: float = 0.45
    startup_alignment_steer_clip: float = 0.60
    startup_alignment_speed_k: float = 0.22
    startup_alignment_brake_clip: float = 0.05
    startup_alignment_acc_clip: float = 0.45
    startup_alignment_hard_steps: int = 6
    startup_alignment_force_positive_accel: bool = True
    startup_alignment_raw_steer_guard_ratio: float = 0.35
    startup_alignment_raw_brake_guard_ratio: float = 0.10
...
class RewardConfig:
    ...
    constrained_use_env_reward: bool = False
    constrained_task_only: bool = True
    constrained_aux_safety_coef: float = 0.20
    constrained_terminal_failure_penalty: float = 2.5
    max_waypoint_pos_error: float = 2.5
    max_waypoint_speed_error: float = 1.5
    ...
    w_progress = 1.15
    w_waypoint_pos = 0.24
    w_waypoint_speed = 0.18

    w_collision = 6.0
    w_headway = 2.4
    w_overspeed = 0.9
    w_lane_center = 0.16
    w_lane_boundary = 0.42
    w_heading_align = 0.08
...
class SACConfig:
    ...
    cost_limit_lane = 0.35

    # constrained SAC: learn state-action cost critics so constraints can backprop to actor
    initial_lambda: float = 0.05
    lambda_max: float = 20.0
    lambda_overspeed_max: float = 12.0
    lambda_lane_max: float = 10.0
    lambda_overspeed_update_scale: float = 1.00
    lambda_lane_update_scale: float = 0.30
    ...
    min_lambda_overspeed: float = 0.030
    min_lambda_lane: float = 0.005
    dual_batch_mix: float = 0.25
    dual_policy_mix: float = 0.55
    dual_violation_clip: float = 4.0
    dual_pid_kp: float = 0.30
    dual_pid_ki: float = 0.08
    dual_pid_kd: float = 0.10
    dual_integral_decay: float = 0.995
    dual_integral_clip: float = 25.0
    dual_anti_windup_gain: float = 0.25
    dual_overspeed_release_margin: float = 0.05
    dual_overspeed_release_patience: int = 2
    dual_overspeed_release_scale: float = 1.50
    dual_overspeed_release_integral_decay: float = 0.50

    teacher_guidance_coef: float = 0.45
    teacher_action_coef: float = 1.10
    behavior_kl_coef: float = 0.10
    lane_kl_coef: float = 0.08
    actor_overspeed_penalty_coef: float = 1.00
    actor_comfort_penalty_coef: float = 0.12
    actor_lane_penalty_coef: float = 0.35
    corrective_teacher_weight_floor: float = 0.70
    corrective_teacher_weight_cap: float = 1.00
    teacher_base_weight_floor: float = 0.15
    teacher_decay_episodes_ratio: float = 0.70
    teacher_min_weight: float = 0.08
    guided_teacher_bc_coef: float = 0.65
    corrective_teacher_bc_coef: float = 1.60
    teacher_qfilter_enabled: bool = True
    teacher_qfilter_margin: float = 0.00
    teacher_warmup_enabled: bool = True
    teacher_warmup_prob_start: float = 1.0
    teacher_warmup_prob_end: float = 0.50
    teacher_warmup_noise_std: float = 0.06
    safe_random_accel_min: float = -0.25
    safe_random_accel_max: float = 0.45
    safe_random_steer_std: float = 0.06
    safe_exploration_filter: bool = True
    safe_exploration_steps: int = 4000
    risk_correction_enabled: bool = True
    risk_correction_min_severity: float = 0.18
    risk_correction_teacher_blend: float = 0.65
    risk_correction_severity_gain: float = 0.25
    risk_correction_shield_threshold: float = 0.55
    safety_priority_mix: float = 0.35
    safety_priority_power: float = 1.25
    safety_priority_min_cost: float = 0.18
...
class EvalConfig:
    ...
    video_dir: str = "results/eval_videos"
    video_fps: int = 15
    max_videos: int = 8
    video_prefix: str = ""
    startup_debug: bool = False
    startup_debug_steps: int = 20
    startup_debug_log_path: str = ""
...
def _sanitize_video_component(value: str) -> str:
...
def _build_eval_video_name(cfg, mode: str, report_tag: str, seed: int, episode_idx: int) -> str:
    prefix = _sanitize_video_component(str(getattr(cfg.eval, "video_prefix", "")).strip())
    mode_part = _sanitize_video_component(mode)
    report_part = _sanitize_video_component(report_tag)
    parts = []
    if prefix:
        parts.append(prefix)
    parts.extend([mode_part, f"seed{int(seed)}", report_part, f"ep{int(episode_idx)}"])
    return "_".join([p for p in parts if p])


def _should_record_eval_video(cfg) -> bool:
    if not bool(getattr(cfg.eval, "save_video", False)):
        return False
    max_videos = max(0, int(getattr(cfg.eval, "max_videos", 0)))
    return EVAL_VIDEO_RUNTIME_STATE["saved_count"] < max_videos
...
def _finite_int_or_none(value) -> Optional[int]:
    try:
        out = float(value)
    except Exception:
        return None
...
def _probe_video_metadata(path_hint: str) -> dict:
    path_hint = str(path_hint or "").strip()
    record = {
        "path": os.path.abspath(path_hint) if path_hint else "",
        "exists": False,
        "actual_fps": None,
        "duration_sec": None,
        "frame_count": None,
        "probe_status": "no_path" if not path_hint else "not_found",
        "probe_error": "",
    }
    if not path_hint:
        return record
    ...
    if not os.path.exists(video_path):
        root, ext = os.path.splitext(path_hint)
        candidates = sorted(glob.glob(root + "*.mp4"))
        if candidates:
            video_path = candidates[0]
    record["path"] = os.path.abspath(video_path)
    record["exists"] = bool(os.path.exists(video_path))
    ...
    try:
        ...
        record["actual_fps"] = _finite_float_or_none(meta.get("fps"))
        record["duration_sec"] = _finite_float_or_none(meta.get("duration"))
        record["frame_count"] = _finite_int_or_none(meta.get("nframes"))
        if record["frame_count"] is None and getattr(props, "shape", None):
            record["frame_count"] = _finite_int_or_none(props.shape[0])
        record["probe_status"] = "ok"
    except Exception as exc:
        record["probe_status"] = "probe_error"
        record["probe_error"] = f"{type(exc).__name__}: {exc}"
    return record


def _finalize_eval_video_metadata(video_meta: dict) -> Optional[dict]:
    if not bool((video_meta or {}).get("enabled", False)):
        return None
    record = {
        "report_tag": str(video_meta.get("report_tag", "")),
        "seed": int(video_meta.get("seed", 0)),
        "episode_idx": int(video_meta.get("episode_idx", 0)),
        "requested_fps": int(video_meta.get("requested_fps", 0)),
    }
    record.update(_probe_video_metadata(str(video_meta.get("path_hint", ""))))
    return record


def summarize_eval_video_fps(eval_payload: Optional[dict], cfg) -> dict:
    videos: List[dict] = []
    if isinstance(eval_payload, dict):
        for key in ("raw", "shielded"):
            section = eval_payload.get(key)
            if isinstance(section, dict):
                videos.extend([v for v in section.get("videos", []) if isinstance(v, dict)])
    observed = []
    probe_status = []
    paths = []
    for item in videos:
        paths.append(str(item.get("path", "")))
        if item.get("actual_fps") is not None:
            observed.append(float(item["actual_fps"]))
        probe_status.append(str(item.get("probe_status", "")))
    return {
        "requested_fps": int(getattr(cfg.eval, "video_fps", 0)),
        "observed_fps_values": observed,
        "paths": paths,
        "probe_status": probe_status,
    }


def _make_eval_env(cfg, mode: str, report_tag: str, seed: int, episode_idx: int):
    want_render = bool(getattr(cfg.eval, "render", False))
    want_video = _should_record_eval_video(cfg)
    render_mode = None
    if want_video:
        render_mode = "rgb_array"
        if want_render and not EVAL_VIDEO_RUNTIME_STATE["warned_dual_mode"]:
            print("[EVAL-VIDEO] eval_render and eval_save_video were both requested; prioritizing video capture (rgb_array) for compatibility.")
            EVAL_VIDEO_RUNTIME_STATE["warned_dual_mode"] = True
    elif want_render:
        render_mode = "human"

    wrapper = HighwayEnvWrapper(cfg, render=(render_mode == "human"), render_mode=render_mode)

    video_meta = {
        "enabled": False,
        "path_hint": "",
        "report_tag": str(report_tag),
        "seed": int(seed),
        "episode_idx": int(episode_idx),
    }
    if want_video:
        video_dir = str(getattr(cfg.eval, "video_dir", "results/eval_videos")).strip() or "results/eval_videos"
        os.makedirs(video_dir, exist_ok=True)
        name_prefix = _build_eval_video_name(cfg, mode=mode, report_tag=report_tag, seed=seed, episode_idx=episode_idx)
        fps = max(1, int(getattr(cfg.eval, "video_fps", 15)))
        try:
            wrapper.env.metadata = dict(getattr(wrapper.env, "metadata", {}) or {})
            wrapper.env.metadata["render_fps"] = fps
        except Exception:
            pass
        wrapper.env = gym.wrappers.RecordVideo(
            wrapper.env,
        ...
        try:
            ...
            pass
        EVAL_VIDEO_RUNTIME_STATE["saved_count"] += 1
        video_meta["enabled"] = True
        video_meta["requested_fps"] = int(fps)
        video_meta["path_hint"] = os.path.join(video_dir, f"{name_prefix}-episode-0.mp4")
        print(f"[EVAL-VIDEO] render={int(want_render)} save_video=1 dir={video_dir} report={report_tag} seed={seed} ep={episode_idx}")
    elif bool(getattr(cfg.eval, "save_video", False)):
        print("[EVAL-VIDEO] skip recording because eval_max_videos reached")

    return wrapper, video_meta


class HighwayEnvWrapper:
    def __init__(self, cfg, render: bool = False, render_mode: Optional[str] = None):
        self.cfg = cfg
        resolved_render_mode = str(render_mode) if render_mode is not None else ("human" if render else None)
        self.env = gym.make(
            cfg.env.env_id,
    ...
    def build_low_state(self, obs) -> np.ndarray:
        ...
        lane_width_norm = max(float(scene.get("lane_width_runtime", self.cfg.env.lane_width)), 1e-6)
        ...
        scene_feat = np.array([
            ...
            float(np.clip(scene.get("signed_lane_offset_norm", 0.0), -1.5, 1.5)),
            float(np.clip(scene.get("abs_lane_offset_norm", 0.0), 0.0, 1.5)),
            float(np.clip(scene.get("heading_error", 0.0) / 1.2, -1.5, 1.5)),
            float(np.clip(scene.get("lateral_speed", 0.0) / max(self.cfg.env.max_speed, 1e-6), -1.0, 1.0)),
            float(np.clip(scene.get("road_left_margin", 0.0) / max(lane_width_norm * 2.0, 1e-6), 0.0, 1.5)),
            float(np.clip(scene.get("road_right_margin", 0.0) / max(lane_width_norm * 2.0, 1e-6), 0.0, 1.5)),
            float(np.clip(scene.get("road_boundary_margin", 0.0) / max(self.cfg.safety.road_boundary_soft_margin, 1e-6), 0.0, 1.5)),
            float(np.clip(scene.get("road_boundary_proximity", 0.0), 0.0, 1.5)),
            float(np.clip(scene.get("lane_boundary_proximity", 0.0), 0.0, 1.5)),
        ], dtype=np.float32)
    ...
    def get_scene_dict(self) -> dict:
        ...
        closing_speed, ttc = self._front_closing_metrics(front_distance, front_rel_speed)
        rear_closing_speed, rear_ttc = self._rear_closing_metrics(rear_distance, rear_rel_speed)
        left_adj = self._adjacent_lane_safety_state(left_lane, left_front_distance, left_front_rel_speed, left_rear_distance, left_rear_rel_speed)
        right_adj = self._adjacent_lane_safety_state(right_lane, right_front_distance, right_front_rel_speed, right_rear_distance, right_rear_rel_speed)
        ...
        road_boundary_proximity = float(np.clip((float(self.cfg.safety.road_boundary_soft_margin) - road_boundary_margin) / max(float(self.cfg.safety.road_boundary_soft_margin), 1e-6), 0.0, 1.0))
        lane_boundary_proximity = float(np.clip((abs_lane_offset_norm - float(self.cfg.safety.lane_center_hard_ratio)) / max(1.0 - float(self.cfg.safety.lane_center_hard_ratio), 1e-6), 0.0, 1.0))
        try:
            velocity = np.asarray(getattr(ego, "velocity", np.zeros((2,), dtype=np.float32)), dtype=np.float32).reshape(-1)
            if velocity.shape[0] >= 2:
                lane_heading = float(geometry["lane_heading"])
                vx = float(velocity[0])
                vy = float(velocity[1])
                lateral_speed = float(-vx * np.sin(lane_heading) + vy * np.cos(lane_heading))
            else:
                lateral_speed = float(ego_speed * np.sin(heading - float(geometry["lane_heading"])))
        except Exception:
            lateral_speed = float(ego_speed * np.sin(heading - float(geometry["lane_heading"])))

        heading_error = float(_wrap_to_pi(heading - float(geometry["lane_heading"])))
        ...
        return {
            ...
            "geometry_source": str(geometry["source"]),
            ...
            "front_closing_speed": float(closing_speed),
            "front_ttc": float(ttc),
            ...
            "left_front_closing_speed": float(left_adj["front_closing_speed"]),
            "left_front_ttc": float(left_adj["front_ttc"]),
            "right_front_distance": right_front_distance,
            "right_front_rel_speed": right_front_rel_speed,
            "right_front_closing_speed": float(right_adj["front_closing_speed"]),
            "right_front_ttc": float(right_adj["front_ttc"]),
            "left_rear_distance": left_rear_distance,
            "left_rear_rel_speed": left_rear_rel_speed,
            "left_rear_closing_speed": float(left_adj["rear_closing_speed"]),
            "left_rear_ttc": float(left_adj["rear_ttc"]),
            "right_rear_distance": right_rear_distance,
            "right_rear_rel_speed": right_rear_rel_speed,
            "right_rear_closing_speed": float(right_adj["rear_closing_speed"]),
            "right_rear_ttc": float(right_adj["rear_ttc"]),
            "left_clear_geom": bool(left_adj["clear_geom"]),
            "left_clear_dyn": bool(left_adj["clear_dyn"]),
            "right_clear_geom": bool(right_adj["clear_geom"]),
            "right_clear_dyn": bool(right_adj["clear_dyn"]),
            "left_clear": bool(left_adj["clear_dyn"]),
            "right_clear": bool(right_adj["clear_dyn"]),
            "ttc": float(ttc),
    ...
    def get_runtime_env_snapshot(self) -> dict:
        return copy.deepcopy(self.runtime_env_snapshot)

    def _capture_runtime_env_snapshot(self, scene: Optional[dict] = None) -> dict:
        base_env = self.env.unwrapped
        runtime_cfg = _jsonable_plain(getattr(base_env, "config", {}) or {})
        ...
        if scene is None:
            try:
                ...
                scene = {}
        comparisons = {
            "lanes_count_match": runtime_cfg.get("lanes_count") == self.cfg.env.lanes_count if "lanes_count" in runtime_cfg else None,
            "vehicles_count_match": runtime_cfg.get("vehicles_count") == self.cfg.env.vehicles_count if "vehicles_count" in runtime_cfg else None,
            "duration_match": runtime_cfg.get("duration") == self.cfg.env.duration if "duration" in runtime_cfg else None,
            "simulation_frequency_match": runtime_cfg.get("simulation_frequency") == self.cfg.env.simulation_frequency if "simulation_frequency" in runtime_cfg else None,
            "policy_frequency_match": runtime_cfg.get("policy_frequency") == self.cfg.env.policy_frequency if "policy_frequency" in runtime_cfg else None,
        }
        return {
            "env_id": str(self.cfg.env.env_id),
            "runtime_env_config": runtime_cfg,
            "configured_metric_assumptions": {
                "lane_width_assumed": float(self.cfg.env.lane_width),
                "max_speed_assumed": float(self.cfg.env.max_speed),
                "min_speed_assumed": float(self.cfg.env.min_speed),
                "lanes_count_assumed": int(self.cfg.env.lanes_count),
                "vehicles_count_assumed": int(self.cfg.env.vehicles_count),
                "duration_assumed": int(self.cfg.env.duration),
                "simulation_frequency_assumed": int(self.cfg.env.simulation_frequency),
                "policy_frequency_assumed": int(self.cfg.env.policy_frequency),
            },
            "runtime_observation_space_shape": list(getattr(observation_space, "shape", ()) or ()),
            "runtime_action_space_shape": list(getattr(action_space, "shape", ()) or ()),
            "runtime_action_space_low": _jsonable_plain(getattr(action_space, "low", None)),
            "runtime_action_space_high": _jsonable_plain(getattr(action_space, "high", None)),
            "runtime_scene_probe": {
                ...
                "lane_heading_runtime": float(scene.get("lane_heading", 0.0)) if scene else 0.0,
                "geometry_source": str(scene.get("geometry_source", "unknown")) if scene else "unknown",
            },
            "assumption_runtime_comparisons": comparisons,
        }
    ...
    def _runtime_lane_geometry(self, vehicle, lane_id: int) -> dict:
        ...
        try:
            ...
            lane_width = fallback_lane_width
            if source != "config_assumption":
                source = "runtime_center_only"

        try:
            lane_heading = float(current_lane.heading_at(longitudinal))
        except Exception:
            lane_heading = 0.0
            if source != "config_assumption":
                source = "runtime_center_only"
        ...
        if len(road_edges) == 0:
            ...
            if source == "runtime_lane_geometry":
                source = "runtime_lane_only"
        else:
    ...
    def _adjacent_lane_safety_state(self, lane_id: int | None, front_distance: float, front_rel_speed: float, rear_distance: float, rear_rel_speed: float) -> dict:
        if lane_id is None:
            return {
                "front_closing_speed": 0.0,
                "front_ttc": 99.0,
                "rear_closing_speed": 0.0,
                "rear_ttc": 99.0,
                "clear_geom": False,
                "clear_dyn": False,
            }
        front_closing_speed, front_ttc = self._front_closing_metrics(front_distance, front_rel_speed)
        rear_closing_speed, rear_ttc = self._rear_closing_metrics(rear_distance, rear_rel_speed)
        clear_geom = bool(front_distance > 18.0 and rear_distance > 12.0)
        front_dyn_safe = bool(front_distance > 18.0 and (front_closing_speed <= 3.0 or front_ttc >= 4.5))
        rear_dyn_safe = bool(rear_distance > 12.0 and (rear_closing_speed <= 2.5 or rear_ttc >= 4.0))
        clear_dyn = bool(clear_geom and front_dyn_safe and rear_dyn_safe)
        return {
            "front_closing_speed": float(front_closing_speed),
            "front_ttc": float(front_ttc),
            "rear_closing_speed": float(rear_closing_speed),
            "rear_ttc": float(rear_ttc),
            "clear_geom": bool(clear_geom),
            "clear_dyn": bool(clear_dyn),
        }
...
def _rl_is_offroad_scene(cfg, scene: dict) -> bool:
    road_margin = float(scene.get("road_boundary_margin", float("inf")))
    lane_offset = abs(float(scene.get("signed_lane_offset_norm", scene.get("abs_lane_offset_norm", 0.0))))
    return bool(road_margin < float(cfg.safety.road_boundary_off_margin) or lane_offset > 1.10)


def _rl_terminal_failure(cfg, next_scene: dict) -> bool:
    return bool(next_scene.get("collision", False) or _rl_is_offroad_scene(cfg, next_scene))


def _clip_grad_norm_(parameters, max_norm: float) -> None:
...
def _teacher_warmup_probability(cfg, total_steps: int) -> float:
    horizon = max(1, int(cfg.sac.start_steps))
    progress = float(np.clip(total_steps / max(float(horizon), 1.0), 0.0, 1.0))
    start = float(np.clip(cfg.sac.teacher_warmup_prob_start, 0.0, 1.0))
    end = float(np.clip(cfg.sac.teacher_warmup_prob_end, 0.0, 1.0))
    return float(np.clip(start + (end - start) * progress, 0.0, 1.0))


def _sample_safe_random_action(cfg, constraints: dict) -> np.ndarray:
    acc_bound, steer_bound = physical_to_normalized_action_bounds(cfg, constraints)
    action = np.array([
        np.random.uniform(float(cfg.sac.safe_random_accel_min), float(cfg.sac.safe_random_accel_max)),
        np.random.normal(0.0, float(cfg.sac.safe_random_steer_std)),
    ], dtype=np.float32)
    action[0] = float(np.clip(action[0], -acc_bound, acc_bound))
    action[1] = float(np.clip(action[1], -steer_bound, steer_bound))
    return action


def _sample_teacher_guided_warmup_action(cfg, env, constraints: dict, total_steps: int) -> np.ndarray:
    guidance = getattr(env, "current_teacher_guidance", None) or {}
    teacher_action = guidance.get("teacher_action")
    use_teacher = bool(
        cfg.sac.teacher_warmup_enabled
        and teacher_action is not None
        and np.random.rand() < _teacher_warmup_probability(cfg, total_steps)
    )
    if not use_teacher:
        return _sample_safe_random_action(cfg, constraints)

    acc_bound, steer_bound = physical_to_normalized_action_bounds(cfg, constraints)
    action = np.asarray(teacher_action, dtype=np.float32).copy()
    noise = np.random.normal(0.0, float(cfg.sac.teacher_warmup_noise_std), size=action.shape).astype(np.float32)
    action = action + noise
    action[0] = float(np.clip(action[0], -acc_bound, acc_bound))
    action[1] = float(np.clip(action[1], -steer_bound, steer_bound))
...
def _apply_safe_exploration_filter(cfg, scene: dict, action, constraints: dict, verifier, shield):
    filtered_action = np.asarray(action, dtype=np.float32).copy()
    if not bool(cfg.sac.safe_exploration_filter) or verifier is None or shield is None:
        return filtered_action
    verify_info = verifier.check(scene, filtered_action, constraints)
    safe_action, _ = shield.apply(scene, filtered_action, constraints, verify_info)
    return np.asarray(safe_action, dtype=np.float32)


def _should_use_safe_exploration_filter(cfg, total_steps: int) -> bool:
    horizon = max(int(cfg.sac.start_steps), int(getattr(cfg.sac, "safe_exploration_steps", 0)))
    return bool(cfg.sac.safe_exploration_filter) and int(total_steps) < max(horizon, 0)


def _clip_action_to_constraints(cfg, action, constraints: dict) -> np.ndarray:
    clipped = np.asarray(action, dtype=np.float32).copy()
    acc_bound, steer_bound = physical_to_normalized_action_bounds(cfg, constraints)
    clipped[0] = float(np.clip(clipped[0], -acc_bound, acc_bound))
    clipped[1] = float(np.clip(clipped[1], -steer_bound, steer_bound))
    return clipped.astype(np.float32)


def _risk_correction_severity(verify_info: Optional[dict]) -> float:
    verify_info = verify_info or {}
    severity = float(verify_info.get("projected_boundary_risk", 0.0))
    if bool(verify_info.get("overspeed", False)):
        severity = max(severity, 0.35)
    if bool(verify_info.get("unsafe_headway", False)):
        severity = max(severity, 0.45)
    if bool(verify_info.get("low_ttc", False)):
        severity = max(severity, 0.55)
    if bool(verify_info.get("large_lane_offset", False)) or bool(verify_info.get("lane_instability_high", False)):
        severity = max(severity, 0.65)
    if bool(verify_info.get("emergency", False)):
        severity = max(severity, 0.80)
    return _clip01(severity)


def _apply_risk_aware_teacher_correction(cfg, env, scene: dict, action, constraints: dict, verifier, shield):
    corrected_action = _clip_action_to_constraints(cfg, action, constraints)
    info = {
        "correction_active": False,
        "correction_source": "inactive",
        "severity": 0.0,
        "teacher_blend": 0.0,
    ...
    if not bool(getattr(cfg.sac, "risk_correction_enabled", False)) or verifier is None:
        verify_info = verifier.check(scene, corrected_action, constraints) if verifier is not None else {}
        return corrected_action, info, verify_info

    pre_verify = verifier.check(scene, corrected_action, constraints)
    severity = _risk_correction_severity(pre_verify)
    info["severity"] = float(severity)
    if not bool(pre_verify.get("needs_intervention", False)) or severity < float(getattr(cfg.sac, "risk_correction_min_severity", 0.0)):
        return corrected_action, info, pre_verify

    guidance = getattr(env, "current_teacher_guidance", None) or {}
    teacher_action = guidance.get("teacher_action")
    if teacher_action is not None:
        blend = float(np.clip(
            float(getattr(cfg.sac, "risk_correction_teacher_blend", 0.0))
            + float(getattr(cfg.sac, "risk_correction_severity_gain", 0.0)) * severity,
            0.0,
            0.95,
        ))
        teacher_action = _clip_action_to_constraints(cfg, teacher_action, constraints)
        corrected_action = _clip_action_to_constraints(cfg, (1.0 - blend) * corrected_action + blend * teacher_action, constraints)
        info["correction_active"] = True
        info["correction_source"] = "teacher_blend"
        info["teacher_blend"] = float(blend)

    post_verify = verifier.check(scene, corrected_action, constraints)
    if shield is not None and (
        severity >= float(getattr(cfg.sac, "risk_correction_shield_threshold", 1.0))
        or bool(post_verify.get("needs_intervention", False))
    ):
        shielded_action, shield_info = shield.apply(scene, corrected_action, constraints, post_verify)
        corrected_action = _clip_action_to_constraints(cfg, shielded_action, constraints)
        post_verify = verifier.check(scene, corrected_action, constraints)
        info["correction_active"] = True
        info["correction_source"] = "teacher_blend_plus_shield" if info["teacher_blend"] > 0.0 else "shield_only"
        info["shield_modified"] = bool(shield_info.get("shield_modified", False))
        info["shield_reasons"] = list(shield_info.get("shield_reasons", []))

    info["corrected_action"] = corrected_action.copy()
    return corrected_action, info, post_verify
...
def _lane_center_tracking_error(cfg, scene: dict, reference_y: Optional[float] = None) -> tuple[float, float]:
    lane_width = max(float(cfg.env.lane_width), 1e-6)
    if reference_y is None:
        reference_y = float(scene.get("lane_center_y", 0.0))
    signed_error = float(scene.get("ego_y", 0.0) - float(reference_y)) / lane_width
    return float(signed_error), float(abs(signed_error))


def _lane_boundary_risk_terms(cfg, scene: dict) -> dict:
    abs_lane_offset = float(abs(scene.get("signed_lane_offset_norm", scene.get("abs_lane_offset_norm", 0.0))))
    road_margin = float(scene.get("road_boundary_margin", float("inf")))
    heading_abs = abs(float(scene.get("heading_error", scene.get("heading", 0.0))))
    lateral_speed = abs(float(scene.get("lateral_speed", 0.0)))

    center_soft = float(cfg.safety.lane_center_soft_ratio)
    center_hard = float(cfg.safety.lane_center_hard_ratio)
    road_soft = float(cfg.safety.road_boundary_soft_margin)
    road_hard = float(cfg.safety.road_boundary_hard_margin)
    road_off = float(cfg.safety.road_boundary_off_margin)

    center_soft_term = _clip01(max(0.0, abs_lane_offset - center_soft) / max(1.0 - center_soft, 1e-6))
    center_hard_term = _clip01(max(0.0, abs_lane_offset - center_hard) / max(1.0 - center_hard, 1e-6))
    road_soft_term = _squared_barrier_cost(road_margin, road_soft)
    road_hard_term = _squared_barrier_cost(road_margin, road_hard)
    offroad_term = 1.0 if road_margin < road_off else 0.0
    near_edge_factor = _clip01((road_soft - road_margin) / max(road_soft, 1e-6))
    heading_term = _clip01(heading_abs / 1.05) * max(near_edge_factor, 0.35 * center_hard_term)
    lateral_term = _clip01(lateral_speed / max(0.45 * float(cfg.env.max_speed), 1e-6)) * max(near_edge_factor, 0.35 * center_hard_term)

    lane_cost = _clip01(max(
        offroad_term,
        0.10 * center_soft_term + 0.30 * center_hard_term + 0.45 * road_soft_term + 0.75 * road_hard_term + 0.12 * heading_term + 0.08 * lateral_term,
    ))
    return {
...
def _scene_ttc_from_distance_closing(distance: float, closing_speed: float) -> float:
    if float(distance) < 1e5 and float(closing_speed) > 0.0:
        return float(distance) / max(float(closing_speed), 1e-3)
    return 99.0


def _side_merge_risk_primitives(cfg, scene: dict, side: str) -> dict:
    lane_exists = scene.get(f"{side}_lane_id") is not None
    if not lane_exists:
        return {
            "available": False,
            "front_distance": 1e6,
            ...
            "front_closing_speed": 0.0,
            "rear_closing_speed": 0.0,
            "lane_change_gap_risk": 0.0,
            "merge_conflict_risk": 0.0,
            "lateral_merge_risk": 0.0,
        }

    front_d = float(scene.get(f"{side}_front_distance", 1e6))
    rear_d = float(scene.get(f"{side}_rear_distance", 1e6))
    front_rel = float(scene.get(f"{side}_front_rel_speed", 0.0))
    rear_rel = float(scene.get(f"{side}_rear_rel_speed", 0.0))
    front_closing = float(scene.get(f"{side}_front_closing_speed", max(0.0, -front_rel)))
    rear_closing = float(scene.get(f"{side}_rear_closing_speed", max(0.0, -rear_rel)))
    front_ttc = float(scene.get(f"{side}_front_ttc", _scene_ttc_from_distance_closing(front_d, front_closing)))
    rear_ttc = float(scene.get(f"{side}_rear_ttc", _scene_ttc_from_distance_closing(rear_d, rear_closing)))
    geom_clear = bool(scene.get(f"{side}_clear_geom", scene.get(f"{side}_clear", False)))
    dyn_clear = bool(scene.get(f"{side}_clear_dyn", scene.get(f"{side}_clear", False)))

    front_gap_risk = _squared_barrier_cost(front_d, 18.0)
    rear_gap_risk = _squared_barrier_cost(rear_d, 14.0)
    front_ttc_risk = _squared_barrier_cost(front_ttc, 4.5)
    rear_ttc_risk = _squared_barrier_cost(rear_ttc, 4.0)
    front_closing_risk = _clip01(front_closing / 8.0)
    rear_closing_risk = _clip01(rear_closing / 7.0)
    lane_change_gap_risk = _clip01(max(front_gap_risk, rear_gap_risk))
    merge_conflict_risk = _clip01(max(
        front_ttc_risk,
        rear_ttc_risk,
        0.45 * front_closing_risk * _clip01((42.0 - front_d) / 42.0),
        0.75 * rear_closing_risk * _clip01((34.0 - rear_d) / 34.0),
    ))
    if not geom_clear:
        lane_change_gap_risk = max(lane_change_gap_risk, 0.45)
    if not dyn_clear:
        merge_conflict_risk = max(merge_conflict_risk, 0.70)
    lateral_merge_risk = _clip01(max(lane_change_gap_risk, merge_conflict_risk))
    return {
        "available": True,
        "front_distance": float(front_d),
        "rear_distance": float(rear_d),
        "front_ttc": float(front_ttc),
        "rear_ttc": float(rear_ttc),
        "front_closing_speed": float(front_closing),
        "rear_closing_speed": float(rear_closing),
        "lane_change_gap_risk": float(lane_change_gap_risk),
        "merge_conflict_risk": float(merge_conflict_risk),
        "lateral_merge_risk": float(lateral_merge_risk),
    }


def _infer_lateral_intent_side(
    cfg,
    scene: dict,
    action=None,
    ...
    reference_scene = previous_scene if previous_scene is not None else scene
    current_lane = int(reference_scene.get("lane_id", scene.get("lane_id", 0)))
    if target_lane is not None:
        try:
            target_lane_int = int(target_lane)
            if target_lane_int < current_lane:
                return "left", True
            if target_lane_int > current_lane:
                return "right", True
        except Exception:
    ...
    lateral_speed = float(scene.get("lateral_speed", 0.0))
    lane_offset = abs(float(scene.get("signed_lane_offset_norm", scene.get("abs_lane_offset_norm", 0.0))))
    if abs(lateral_speed) > max(0.15 * float(cfg.env.max_speed), 0.7):
        return ("right" if lateral_speed > 0.0 else "left"), True

    action_active = False
    try:
        action_arr = np.asarray(action, dtype=np.float32).reshape(-1)
        action_active = bool(action_arr.shape[0] >= 2 and abs(float(action_arr[1])) > 0.14)
    except Exception:
        action_active = False
    if action_active or lane_offset > 0.38:
        return None, True
    return None, False


def compute_interaction_risk_primitives(
    cfg,
    scene: dict,
    action=None,
    ...
    front_distance = float(scene.get("front_distance", 1e6))
    front_closing = float(scene.get("front_closing_speed", max(0.0, -float(scene.get("front_rel_speed", 0.0)))))
    front_ttc = float(scene.get("front_ttc", scene.get("ttc", _scene_ttc_from_distance_closing(front_distance, front_closing))))
    front_collision_risk = _clip01(max(
        _squared_barrier_cost(front_distance, float(cfg.safety.collision_risk_front_distance)),
        _squared_barrier_cost(front_ttc, float(cfg.safety.collision_risk_ttc_threshold)),
        0.35 * _clip01(front_closing / max(float(cfg.planner.closing_speed_hard_threshold) * 2.0, 1e-6))
        * _clip01((float(cfg.planner.closing_speed_near_front_distance) - front_distance) / max(float(cfg.planner.closing_speed_near_front_distance), 1e-6)),
    ))

    rear_distance = float(scene.get("rear_distance", 1e6))
    rear_rel = float(scene.get("rear_rel_speed", 0.0))
    rear_closing = float(scene.get("rear_closing_speed", max(0.0, -rear_rel)))
    rear_ttc = float(scene.get("rear_ttc", _scene_ttc_from_distance_closing(rear_distance, rear_closing)))
    brake_pressure = 0.0
    try:
        action_arr = np.asarray(action, dtype=np.float32).reshape(-1)
        if action_arr.shape[0] >= 1:
            brake_pressure = _clip01(-float(action_arr[0]))
    except Exception:
        brake_pressure = 0.0
    rear_gap_risk = _squared_barrier_cost(rear_distance, 12.0)
    rear_collision_risk = _clip01(max(
        _squared_barrier_cost(rear_ttc, 4.0),
        rear_gap_risk * (0.35 + 0.65 * brake_pressure),
        0.70 * _clip01(rear_closing / 7.0) * _clip01((36.0 - rear_distance) / 36.0),
    ))

    left = _side_merge_risk_primitives(cfg, scene, "left")
    right = _side_merge_risk_primitives(cfg, scene, "right")
    intended_side, lateral_active = _infer_lateral_intent_side(
        cfg,
        scene,
        action=action,
    ...
    if intended_side == "left":
        active_lateral_merge_risk = float(left["lateral_merge_risk"])
    elif intended_side == "right":
        active_lateral_merge_risk = float(right["lateral_merge_risk"])
    elif lateral_active:
        active_lateral_merge_risk = max(float(left["lateral_merge_risk"]), float(right["lateral_merge_risk"]))
    else:
        active_lateral_merge_risk = 0.25 * max(float(left["lateral_merge_risk"]), float(right["lateral_merge_risk"]))
    lateral_merge_risk = _clip01(max(float(left["lateral_merge_risk"]), float(right["lateral_merge_risk"])))

    lane_terms = _lane_boundary_risk_terms(cfg, scene)
    boundary_cost = _clip01(max(
        lane_terms["lane_cost"],
        lane_terms["road_soft_term"],
        lane_terms["road_hard_term"],
        lane_terms["offroad_term"],
        float(scene.get("lane_boundary_proximity", 0.0)),
        float(scene.get("road_boundary_proximity", 0.0)),
    ))

    return {
        "front_collision_risk": float(front_collision_risk),
        "rear_collision_risk": float(rear_collision_risk),
        "lateral_merge_risk": float(lateral_merge_risk),
        "active_lateral_merge_risk": float(_clip01(active_lateral_merge_risk)),
        "boundary_risk": float(boundary_cost),
        "boundary_cost": float(boundary_cost),
        "left_lane_change_gap_risk": float(left["lane_change_gap_risk"]),
        "right_lane_change_gap_risk": float(right["lane_change_gap_risk"]),
        "lane_change_gap_risk": float(max(left["lane_change_gap_risk"], right["lane_change_gap_risk"])),
        "left_merge_conflict_risk": float(left["merge_conflict_risk"]),
        "right_merge_conflict_risk": float(right["merge_conflict_risk"]),
        "merge_conflict_risk": float(max(left["merge_conflict_risk"], right["merge_conflict_risk"])),
        "left_front_ttc": float(left["front_ttc"]),
        "right_front_ttc": float(right["front_ttc"]),
        "left_rear_ttc": float(left["rear_ttc"]),
        "right_rear_ttc": float(right["rear_ttc"]),
        "rear_ttc": float(rear_ttc),
        "rear_closing_speed": float(rear_closing),
        "left_rear_closing_speed": float(left["rear_closing_speed"]),
        "right_rear_closing_speed": float(right["rear_closing_speed"]),
        "intended_lateral_side": str(intended_side or "none"),
...
def compute_scene_risk_vector(cfg, scene: dict, novelty: float = 0.0) -> dict:
    front_distance = float(scene.get("front_distance", 1e6))
    ego_speed = float(scene.get("ego_speed", 0.0))
    heading_abs = abs(_wrap_to_pi(float(scene.get("heading", 0.0))))
    lateral_speed = abs(float(scene.get("lateral_speed", 0.0)))
    interaction = compute_interaction_risk_primitives(cfg, scene)

    front_collision = float(interaction["front_collision_risk"])
    rear_collision = float(interaction["rear_collision_risk"])
    lateral_merge_risk = float(interaction["lateral_merge_risk"])
    active_lateral_merge_risk = float(interaction["active_lateral_merge_risk"])
    headway = _squared_barrier_cost(front_distance, float(cfg.planner.conservative_headway))
    overspeed = _clip01(
        max(0.0, ego_speed - (float(cfg.planner.speed_max) - float(cfg.planner.overspeed_soft_margin)))
        / max(float(cfg.planner.speed_max), 1e-6)
    )
    left_change = _clip01(max(
        float(interaction["left_lane_change_gap_risk"]),
        float(interaction["left_merge_conflict_risk"]),
    ))
    right_change = _clip01(max(
        float(interaction["right_lane_change_gap_risk"]),
        float(interaction["right_merge_conflict_risk"]),
    ))
    lane_width = max(float(cfg.env.lane_width), 1e-6)
    lane_offset = abs(float(scene.get("signed_lane_offset_norm", (float(scene.get("ego_y", 0.0)) - float(scene.get("lane_center_y", 0.0))) / lane_width)))
    lane_stability = _clip01(
        0.38 * _clip01(max(0.0, lane_offset - 0.08) / 1.05)
        + 0.30 * _clip01(heading_abs / 0.85)
        + 0.18 * _clip01(lateral_speed / max(0.25 * float(cfg.env.max_speed), 1e-6))
        + 0.14 * max(float(scene.get("lane_boundary_proximity", 0.0)), float(scene.get("road_boundary_proximity", 0.0)))
    )
    boundary_risk = _clip01(max(float(interaction["boundary_risk"]), 0.70 * lane_stability))
    ...
    max_longitudinal_risk = _clip01(max(front_collision, rear_collision, headway, overspeed))
    max_lane_change_risk = _clip01(max(left_change, right_change, lateral_merge_risk))
    max_context_risk = _clip01(max(
        0.45 * scene_complexity,
        0.35 * _clip01(novelty),
        lane_stability,
        boundary_risk,
        0.80 * float(interaction["merge_conflict_risk"]),
    ))
    current_plan_risk = _clip01(max(
        max_longitudinal_risk,
        active_lateral_merge_risk,
        0.80 * lane_stability,
        boundary_risk,
    ))
    global_scene_risk = _clip01(max(current_plan_risk, max_lane_change_risk, max_context_risk))
    max_risk = float(global_scene_risk)

    risks = {
        "front_collision": _clip01(front_collision),
        "front_collision_risk": _clip01(front_collision),
        "rear_collision_risk": float(rear_collision),
        "lateral_merge_risk": float(lateral_merge_risk),
        "active_lateral_merge_risk": float(active_lateral_merge_risk),
        ...
        "boundary_cost": float(interaction["boundary_cost"]),
        "left_lane_change_gap_risk": float(interaction["left_lane_change_gap_risk"]),
        "right_lane_change_gap_risk": float(interaction["right_lane_change_gap_risk"]),
        "lane_change_gap_risk": float(interaction["lane_change_gap_risk"]),
        "left_merge_conflict_risk": float(interaction["left_merge_conflict_risk"]),
        "right_merge_conflict_risk": float(interaction["right_merge_conflict_risk"]),
        "merge_conflict_risk": float(interaction["merge_conflict_risk"]),
        "left_front_ttc": float(interaction["left_front_ttc"]),
        "right_front_ttc": float(interaction["right_front_ttc"]),
        "left_rear_ttc": float(interaction["left_rear_ttc"]),
        "right_rear_ttc": float(interaction["right_rear_ttc"]),
        "rear_ttc": float(interaction["rear_ttc"]),
        "rear_closing_speed": float(interaction["rear_closing_speed"]),
        "left_rear_closing_speed": float(interaction["left_rear_closing_speed"]),
        "right_rear_closing_speed": float(interaction["right_rear_closing_speed"]),
        "scene_complexity": float(scene_complexity),
        "novelty": _clip01(novelty),
        "max_longitudinal_risk": float(max_longitudinal_risk),
        "max_lane_change_risk": float(max_lane_change_risk),
        "max_context_risk": float(max_context_risk),
        "current_plan_risk": float(current_plan_risk),
        "global_scene_risk": float(global_scene_risk),
        "max_risk": float(max_risk),
    }
    return risks


def _plan_regime_flags(cfg, scene: dict, target_lane: int, behavior: str, risk_mode: int) -> dict:
    current_lane = int(scene.get("lane_id", 0))
    front_distance = float(scene.get("front_distance", 1e6))
    ttc = float(scene.get("ttc", 99.0))
    closing_speed = float(max(0.0, -float(scene.get("front_rel_speed", 0.0))))
    lane_change_required = bool(target_lane != current_lane or behavior in {"change_left", "change_right"})
    very_safe_cruise = bool(
        (not lane_change_required)
        and front_distance >= float(cfg.planner.very_safe_front_distance)
        and ttc >= float(cfg.planner.very_safe_ttc)
        and risk_mode <= 0
        and closing_speed < 1.5
    )
    safe_cruise = bool(
        (not lane_change_required)
        and front_distance >= float(cfg.planner.safe_cruise_front_distance)
        and ttc >= float(cfg.planner.safe_cruise_ttc)
        and risk_mode <= 0
        and closing_speed < float(cfg.planner.closing_speed_hard_threshold)
    )
    near_risk = bool(
        risk_mode >= 1
        or front_distance < float(cfg.planner.near_risk_front_distance)
        or ttc < float(cfg.planner.near_risk_ttc)
        or (front_distance < float(cfg.planner.closing_speed_near_front_distance) and closing_speed >= float(cfg.planner.closing_speed_soft_threshold))
    )
    return {
        "lane_change_required": bool(lane_change_required),
        "safe_cruise": bool(safe_cruise),
        "very_safe_cruise": bool(very_safe_cruise),
        "near_risk": bool(near_risk),
...
def _plan_hold_bounds(cfg, regime: dict) -> tuple[int, int, int, str]:
    global_max = int(cfg.planner.hold_max_steps)
    if regime["lane_change_required"]:
        hold_default = max(10, int(cfg.planner.hold_min_steps_risky) + 3)
        hold_lb, local_max = 8, 16
        reason = "lane_change_commit"
    elif regime["near_risk"]:
        hold_default = max(8, int(cfg.planner.hold_min_steps_risky) + 1)
        hold_lb, local_max = 7, 14
        reason = "near_risk_hold"
    elif regime["very_safe_cruise"]:
        hold_default = max(int(cfg.planner.hold_min_steps_very_safe), int(cfg.planner.hold_min_steps_default) + 24)
        hold_lb, local_max = max(40, hold_default - 4), 84
        reason = "very_safe_cruise_hold"
    elif regime["safe_cruise"]:
        hold_default = max(int(cfg.planner.hold_min_steps_safe_cruise), int(cfg.planner.hold_min_steps_default) + 18)
        hold_lb, local_max = max(32, hold_default - 4), 72
        reason = "safe_cruise_hold"
    else:
        hold_default = max(12, int(cfg.planner.hold_min_steps_default))
        hold_lb, local_max = 10, 26
        reason = "default_hold"
    hold_max_steps = int(min(local_max, global_max))
    hold_min_steps = int(np.clip(hold_default, hold_lb, hold_max_steps))
    return hold_min_steps, hold_max_steps, hold_lb, reason


def _plan_expire_thresholds(cfg, scene: dict, regime: dict) -> tuple[float, float]:
    front_distance = float(regime["front_distance"])
    ttc = float(regime["ttc"])
    if regime["very_safe_cruise"]:
        fd_default = float(np.clip(max(140.0, front_distance * 0.42), 140.0, 320.0)) if front_distance < 1e5 else 160.0
        ttc_default = float(np.clip(max(16.0, ttc * 0.45), 16.0, 32.0))
    elif regime["safe_cruise"]:
        fd_default = float(np.clip(max(100.0, front_distance * 0.40), 100.0, 240.0)) if front_distance < 1e5 else 120.0
        ttc_default = float(np.clip(max(14.0, ttc * 0.40), 14.0, 26.0))
    elif regime["lane_change_required"]:
        fd_default = float(np.clip(min(front_distance * 0.70, front_distance - 6.0), 25.0, 80.0)) if front_distance < 1e5 else 40.0
        ttc_default = 8.0
    else:
        fd_default = float(np.clip(min(front_distance * 0.72, front_distance - 6.0), max(cfg.planner.replan_front_distance, 20.0), 75.0)) if front_distance < 1e5 else float(cfg.planner.replan_front_distance)
        ttc_default = 6.0 if regime["near_risk"] else 8.0
    if front_distance <= float(cfg.planner.replan_front_distance) + 2.0:
        fd_default = float(cfg.planner.replan_front_distance)
    if ttc >= 90.0:
        ttc_default = max(ttc_default, float(cfg.planner.replan_ttc_threshold + 1.5))
    else:
        ttc_default = max(float(cfg.planner.replan_ttc_threshold), min(ttc_default, ttc - 0.75))
    return float(fd_default), float(ttc_default)


def _plan_reference_lane_feasibility(scene: dict, dependent_lane_side: str):
    dependent = dependent_lane_feasibility(scene, dependent_lane_side)
    if dependent is not None:
        return dependent
    return lane_feasibility_signature(scene)
...
def build_plan_validity(cfg, scene: dict, command: dict, decision: dict) -> dict:
    ...
    regime = _plan_regime_flags(cfg, scene, target_lane, behavior, risk_mode)

    requested_hold = int(command.get("hold_min_steps", 0))
    hold_default, hold_max_steps, hold_lb, reason = _plan_hold_bounds(cfg, regime)
    if requested_hold > 0:
        if regime["safe_cruise"] or regime["very_safe_cruise"]:
            requested_hold = max(requested_hold, hold_default)
        hold_min_steps = int(np.clip(requested_hold, hold_lb, hold_max_steps))
    else:
        hold_min_steps = int(hold_default)

    fd_default, ttc_default = _plan_expire_thresholds(cfg, scene, regime)
    requested_fd = float(command.get("expire_if_front_distance_below", fd_default))
    requested_ttc = float(command.get("expire_if_ttc_below", ttc_default))
    if regime["safe_cruise"] or regime["very_safe_cruise"]:
        requested_fd = max(requested_fd, fd_default)
        requested_ttc = max(requested_ttc, ttc_default)
    expire_if_front_distance_below = float(np.clip(requested_fd, cfg.planner.replan_front_distance, 1e6))
    expire_if_ttc_below = float(np.clip(requested_ttc, cfg.planner.replan_ttc_threshold, 99.0))
    ...
    expire_if_lane_feasibility_changes = bool(command.get("expire_if_lane_feasibility_changes", regime["lane_change_required"])) and regime["lane_change_required"]
    expire_if_rear_gap_changes = bool(command.get("expire_if_rear_gap_changes", regime["lane_change_required"])) and regime["lane_change_required"]
    reference_lane_feasibility = _plan_reference_lane_feasibility(scene, dependent_lane_side)

    return {
        "reason": str(reason),
        "hold_min_steps": int(hold_min_steps),
        "hold_max_steps": int(hold_max_steps),
        "expire_if_front_distance_below": float(expire_if_front_distance_below),
        "expire_if_ttc_below": float(expire_if_ttc_below),
        "expire_if_lane_feasibility_changes": bool(expire_if_lane_feasibility_changes),
        "expire_if_rear_gap_changes": bool(expire_if_rear_gap_changes),
        "lane_change_required": bool(regime["lane_change_required"]),
        ...
        "reference_lane_feasibility": reference_lane_feasibility,
        "safe_cruise": bool(regime["safe_cruise"] or regime["very_safe_cruise"]),
        "very_safe_cruise": bool(regime["very_safe_cruise"]),
    }
...
def lane_feasibility_signature(scene: dict) -> tuple:
    return (
        ...
        bool(scene.get("left_clear_dyn", scene.get("left_clear", False))),
        scene.get("right_lane_id") is not None,
        bool(scene.get("right_clear_dyn", scene.get("right_clear", False))),
    )
...
def plan_event_state(cfg, plan_commit: Optional[dict], current_scene: dict, previous_scene: Optional[dict], shield_intervened: bool, force_replan: bool, step_idx: int) -> dict:
    ...
    reference_lane_feasibility = validity.get("reference_lane_feasibility", plan_commit.get("reference_lane_feasibility"))
    current_dep_feas = dependent_lane_feasibility(current_scene, dependent_side)
    if dependent_side in {"left", "right"} and current_dep_feas != reference_lane_feasibility:
        hard_events.append("dependent_lane_feasibility_flip")

    prev_ttc = float(prev_scene.get("ttc", 99.0))
    ...
    if crossing_below(prev_ttc, curr_ttc, max(cfg.planner.replan_ttc_threshold, ttc_thr + cfg.planner.event_ttc_margin)) or curr_ttc < ttc_thr:
        replan_events.append("ttc_cross")

    prev_fd = float(prev_scene.get("front_distance", 1e6))
    ...
    if crossing_below(prev_fd, curr_fd, fd_thr + cfg.planner.event_front_distance_margin) or curr_fd < fd_thr:
        replan_events.append("front_distance_cross")
    ...
    if bool(validity.get("expire_if_rear_gap_changes", False)):
        ...
        if abs(float(current_scene.get("left_rear_distance", 1e6)) - ref_left_rear) >= float(cfg.planner.event_rear_gap_delta) or abs(float(current_scene.get("right_rear_distance", 1e6)) - ref_right_rear) >= float(cfg.planner.event_rear_gap_delta):
            replan_events.append("rear_gap_change")

    ref_closing_speed = float(validity.get("reference_closing_speed", max(0.0, -float(ref_scene.get("front_rel_speed", 0.0)))))
    prev_closing_speed = float(max(0.0, -float(prev_scene.get("front_rel_speed", 0.0))))
    ...
    closing_speed_became_dangerous = bool(
        curr_closing_speed >= max(float(cfg.planner.event_closing_speed_high), ref_closing_speed + float(cfg.planner.event_closing_speed_delta))
        and curr_closing_speed >= prev_closing_speed
        and closing_speed_delta >= float(cfg.planner.event_closing_speed_delta)
    )
    if closing_speed_became_dangerous:
        if curr_fd <= float(cfg.planner.closing_speed_near_front_distance) or curr_ttc <= float(cfg.planner.event_closing_speed_ttc):
            replan_events.append("closing_speed_rise_near")
        else:
            monitor_events.append("closing_speed_rise_far")
    ...
    if age >= hold_max_steps:
        replan_events.append("plan_age_expired")

    all_events = hard_events + [e for e in replan_events if e not in hard_events] + [e for e in monitor_events if e not in hard_events and e not in replan_events]
    if hard_events:
        return {
            "valid": False,
            "state": "HARD_REPLAN",
            ...
            "replan_events": list(replan_events),
            "monitor_events": list(monitor_events),
            "age": age,
        }
    if not replan_events and not monitor_events:
        return {
            "valid": True,
            "state": "COMMIT",
            "events": [],
    ...
    if age < hold_min_steps:
        return {
            "valid": True,
            "state": "MONITOR",
            "events": list(replan_events) + [e for e in monitor_events if e not in replan_events],
            "hard_events": [],
            "replan_events": list(replan_events),
            "monitor_events": list(monitor_events),
            "age": age,
...
def _teacher_weight_from_guidance(cfg, command: dict, risk_vector: dict, novelty: float, recovery_source: str) -> float:
    ...
    front_collision = float(risk_vector.get("front_collision", 0.0))
    headway = float(risk_vector.get("headway", 0.0))
    overspeed = float(risk_vector.get("overspeed", 0.0))
    base = confidence * (
        0.05
        + 0.95 * float(cfg.llm.teacher_weight_risk_scale) * risk_max
        + 0.45 * front_collision
        + 0.35 * headway
        + 0.45 * overspeed
        + 0.20 * float(cfg.llm.teacher_weight_novelty_scale) * float(novelty)
    )
    if front_collision >= 0.15 or headway >= 0.08 or overspeed >= 0.08:
        bonus = 0.20 if (front_collision >= 0.15 or headway >= 0.08) else 0.16
        base = max(base, 0.60 * confidence + bonus)
    elif risk_max < 0.15 and overspeed < 0.05:
        base *= 0.30
    elif risk_max < 0.30 and overspeed < 0.08:
        base *= 0.55
    if abstain:
        base *= 0.90
...
class ScenarioMemory:
    ...
    def _case_dedup_key(self, compact: dict) -> tuple:
        return (
            str(compact.get("tag", "success")),
            str(compact.get("behavior", "keep_lane")),
            int(compact.get("target_lane", 0)),
            int(float(compact.get("target_speed", 0.0)) // 3),
            int(min(25, float(compact.get("front_distance", 0.0)) // 40)),
            20 if float(compact.get("ttc", 99.0)) >= 80.0 else int(max(0, float(compact.get("ttc", 99.0)) // 8)),
        )

    def _is_caution_case(self, compact: dict) -> bool:
        return bool(
            int(compact.get("collision", 0)) == 1
            or int(compact.get("success", 0)) == 0
            or float(compact.get("unsafe_headway_rate", 0.0)) >= 0.02
            or float(compact.get("mean_cost_total", 0.0)) >= 0.12
            or float(compact.get("min_ttc", 99.0)) <= 6.0
            or int(compact.get("interventions", 0)) >= 18
        )

    def _build_compact_case(self, sim: float, item: dict) -> dict:
        ...
        compact = {
            ...
            "unsafe_headway_rate": float(outcome.get("unsafe_headway_rate", 0.0)),
            "mean_cost_total": float(outcome.get("mean_cost_total", 0.0)),
            "interventions": int(outcome.get("interventions", 0)),
        }
        compact["tag"] = "caution" if self._is_caution_case(compact) else "success"
        return compact

    def _select_diverse_cases(self, candidates: List[dict], limit: int, prefer_speed: bool = False) -> List[dict]:
        if limit <= 0:
            return []
        ordered = sorted(candidates, key=lambda c: (float(c.get("sim", 0.0)), float(c.get("target_speed", 0.0)) if prefer_speed else 0.0), reverse=True)
        selected: List[dict] = []
        seen = set()
        seen_behaviors = set()
        for case in ordered:
            key = self._case_dedup_key(case)
            if key in seen:
                continue
            behavior = str(case.get("behavior", "keep_lane"))
            if len(selected) >= 1 and behavior in seen_behaviors and float(case.get("sim", 0.0)) < 0.90:
                continue
    ...
    def retrieve(self, scene: dict, top_k: Optional[int] = None) -> dict:
        ...
        valid_items = [
            item for item in list(self.store.get("items", []))
            if str(item.get("recovery_source", "direct")) not in {"safe_fallback", "low_confidence_safe_fallback"}
        ]
        scored = [(self.similarity(query_desc, dict(item.get("descriptor", {}))), item) for item in valid_items]
        scored.sort(key=lambda x: x[0], reverse=True)
        compact_cases = [self._build_compact_case(sim, item) for sim, item in scored]
        success_pool = [c for c in compact_cases if str(c.get("tag", "success")) == "success"]
        expensive_success_pool = [c for c in compact_cases if str(c.get("tag", "success")) == "expensive_success"]
        caution_pool = [c for c in compact_cases if str(c.get("tag", "success")) == "caution"]

        failure_like_budget = 1 if top_k >= 2 else 0
        success_budget = max(1, top_k - failure_like_budget)
        success_cases = self._select_diverse_cases(success_pool, success_budget, prefer_speed=True)
        if len(success_cases) < success_budget:
            success_cases.extend(self._select_diverse_cases(expensive_success_pool, success_budget - len(success_cases), prefer_speed=True))
        failure_cases = self._select_diverse_cases(caution_pool, failure_like_budget, prefer_speed=False)

        if len(success_cases) >= 1 and top_k >= 3:
            low_speed_bias = all(float(c.get("target_speed", 0.0)) <= 20.5 for c in success_cases)
            if low_speed_bias:
                faster_pool = [c for c in success_pool if float(c.get("target_speed", 0.0)) >= 22.0]
                alt = self._select_diverse_cases(faster_pool, 1, prefer_speed=True)
                if alt:
                    replacement_key = self._case_dedup_key(alt[0])
                    if all(self._case_dedup_key(c) != replacement_key for c in success_cases):
                        success_cases[-1] = alt[0]

        best_similarity = float(scored[0][0]) if scored else 0.0
        novelty = float(np.clip(1.0 - best_similarity, 0.0, 1.0))
        return {
            "success_cases": success_cases[:success_budget],
            "failure_cases": failure_cases[:failure_like_budget],
    ...
    def make_episode_record(self, scene: dict, command: dict, step_idx: Optional[int], recovery_source: str, novelty: float) -> dict:
        return {
            "descriptor": self._descriptor(scene),
    ...
    def finalize_episode(self, episode_records: List[dict], episode_summary: dict) -> None:
        ...
        total_steps = max(1, int(episode_summary.get("steps", len(episode_records))))
        base_outcome = {
            ...
            "interventions": int(episode_summary.get("interventions", 0)),
            "max_abs_lane_offset": float(episode_summary.get("max_abs_lane_offset", 0.0)),
            "offroad_rate": float(episode_summary.get("offroad_rate", 0.0)),
            "min_road_boundary_margin": float(episode_summary.get("min_road_boundary_margin", 1e6)),
        }
        for record in episode_records:
            item = copy.deepcopy(record)
            step_idx = int(item.get("step_idx", total_steps - 1) or 0)
            temporal = float(np.clip((step_idx + 1) / max(total_steps, 1), 0.0, 1.0))
            failure_weight = 0.0
            if base_outcome["collision"] == 1 or base_outcome["success"] == 0 or base_outcome["offroad_rate"] > 0.0:
                failure_weight = temporal ** 2
            outcome = dict(base_outcome)
            outcome["failure_weight"] = float(failure_weight)
            outcome["unsafe_headway_rate"] = float(base_outcome["unsafe_headway_rate"] * (0.35 + 0.65 * temporal))
            outcome["mean_cost_total"] = float(base_outcome["mean_cost_total"] * (0.35 + 0.65 * temporal))
            outcome["max_abs_lane_offset"] = float(base_outcome["max_abs_lane_offset"] * (0.50 + 0.50 * temporal))
            outcome["offroad_rate"] = float(base_outcome["offroad_rate"] * (0.25 + 0.75 * temporal))
            item["id"] = int(next_id)
...
class TeacherQueryGate:
    ...
    def evaluate(self, scene: dict, step_idx: int, plan_commit: Optional[dict], previous_scene: Optional[dict], shield_intervened: bool, force_replan: bool) -> dict:
        ...
        return {
            ...
            "events": list(commit_state.get("events", [])),
            "shield_intervened": bool(shield_intervened),
            "force_replan_next": bool(force_replan),
            "hard_events": list(commit_state.get("hard_events", [])),
            "monitor_events": list(commit_state.get("monitor_events", [])),
            "replan_events": list(commit_state.get("replan_events", [])),
        }
...
def translate_midlevel_command(cfg, scene: dict, command: dict, planner_state: dict, step_idx: Optional[int]) -> dict:
    ...
    ego_speed = float(scene.get("ego_speed", 0.0))
    closing_speed = float(max(0.0, -float(scene.get("front_rel_speed", 0.0))))
    lane_stability = float(compute_scene_risk_vector(cfg, scene, novelty=0.0).get("lane_stability", 0.0))
    boundary_risk = float(compute_scene_risk_vector(cfg, scene, novelty=0.0).get("boundary_risk", 0.0))

    requested_lane = lane_id
    if behavior == "change_left":
        requested_lane = int(scene.get("left_lane_id", lane_id) if scene.get("left_lane_id") is not None else lane_id)
    elif behavior == "change_right":
        requested_lane = int(scene.get("right_lane_id", lane_id) if scene.get("right_lane_id") is not None else lane_id)
    else:
        requested_lane = lane_id
    ...
    lane_change_requested = requested_lane != lane_id and behavior in {"change_left", "change_right"}
    adjacent_safe = False
    if lane_change_requested:
        side = "left" if requested_lane < lane_id else "right"
        adjacent_safe = bool(scene.get(f"{side}_clear_dyn", scene.get(f"{side}_clear", False)))
        adjacent_safe = adjacent_safe and float(scene.get(f"{side}_rear_ttc", 99.0)) >= 4.0 and float(scene.get(f"{side}_front_ttc", 99.0)) >= 4.5
        front_gain = lane_front_distance(scene, requested_lane) - lane_front_distance(scene, lane_id)
        if (not adjacent_safe) or front_gain < float(cfg.planner.lane_change_min_improvement) or lane_stability > 0.55 or boundary_risk > 0.45:
            requested_lane = lane_id
            lane_change_requested = False
            behavior = "follow" if (risk_mode >= 1 or front_distance < float(cfg.planner.safe_cruise_front_distance)) else "keep_lane"
            speed_mode = "cautious" if speed_mode == "free" else speed_mode
            headway_level = "conservative" if risk_mode >= 1 else headway_level
            reason = reason + "; defer lane change and stabilize current lane"

    if not lane_change_requested and behavior in {"keep_lane", "follow", "brake", "yield"}:
        requested_lane = lane_id
        planner_state["active_target_lane"] = lane_id

    target_lane, stabilization_tag = stabilize_lane_decision(cfg, scene, int(requested_lane), planner_state, step_idx)
    if behavior in {"keep_lane", "follow", "brake", "yield"}:
        target_lane = lane_id
        planner_state["active_target_lane"] = lane_id
    ...
    planner_speed_max = float(cfg.planner.speed_max)
    overspeed_soft_limit = max(float(cfg.env.min_speed), planner_speed_max - float(cfg.planner.overspeed_soft_margin))
    overspeed_hard_limit = max(float(cfg.env.min_speed), planner_speed_max - float(cfg.planner.overspeed_hard_margin))
    current_overspeed_gap = max(0.0, ego_speed - planner_speed_max)
    near_overspeed = bool(ego_speed >= overspeed_soft_limit)

    if front_distance < float(cfg.planner.near_risk_front_distance) or ttc < float(cfg.planner.near_risk_ttc) or closing_speed >= float(cfg.planner.closing_speed_soft_threshold):
        requested_speed = min(requested_speed, max(float(cfg.env.min_speed), ego_speed - max(0.5, 0.45 * closing_speed)))
        if behavior == "keep_lane":
            behavior = "follow"
        headway_level = "conservative"
        speed_mode = "brake" if (ttc < float(cfg.planner.replan_ttc_threshold) or front_distance < float(cfg.planner.replan_front_distance)) else "cautious"

    if current_overspeed_gap > 0.0:
        requested_speed = min(requested_speed, max(float(cfg.env.min_speed), overspeed_soft_limit - float(cfg.planner.overspeed_recovery_speed_drop)))
        if behavior == "keep_lane":
            behavior = "follow"
        headway_level = "conservative"
        speed_mode = "brake" if current_overspeed_gap >= float(cfg.planner.overspeed_hard_margin) else "cautious"
    elif near_overspeed:
        requested_speed = min(requested_speed, overspeed_soft_limit)
        if behavior == "keep_lane":
            behavior = "follow"
        if speed_mode == "free":
            speed_mode = "cautious"
    ...
    safe_cruise = bool(
        ...
        and front_distance >= float(cfg.planner.safe_cruise_front_distance)
        and ttc >= float(cfg.planner.safe_cruise_ttc)
        and lane_stability < 0.20
        and boundary_risk < 0.15
        and closing_speed < float(cfg.planner.closing_speed_hard_threshold)
    )
    if safe_cruise and current_overspeed_gap <= 0.0 and not near_overspeed:
        requested_speed = max(requested_speed, min(float(cfg.planner.target_speed_free), max(ego_speed + 0.8, float(cfg.planner.safe_cruise_target_speed_floor))))
        if behavior == "follow":
            behavior = "keep_lane"
    ...
    if boundary_risk > 0.35 or lane_stability > 0.45:
        requested_speed = min(requested_speed, max(float(cfg.env.min_speed), ego_speed + 0.5))
        target_lane = lane_id
        if behavior in {"change_left", "change_right"}:
            behavior = "keep_lane"
        headway_level = "conservative" if headway_level != "normal" else headway_level
        if speed_mode == "free":
            speed_mode = "cautious"

    if speed_mode == "free":
        speed_cap = float(overspeed_hard_limit)
    elif speed_mode == "cautious":
        speed_cap = float(min(overspeed_soft_limit, cfg.planner.target_speed_cautious + 5.0))
    else:
        speed_cap = float(min(max(float(cfg.env.min_speed), overspeed_soft_limit - 1.0), cfg.planner.target_speed_brake + 4.0))
    speed_cap = max(float(cfg.env.min_speed) + 0.5, speed_cap)
    ...
    if current_overspeed_gap > 0.0:
        speed_max = float(np.clip(max(target_speed + 0.4, overspeed_soft_limit - 0.8), cfg.env.min_speed, overspeed_soft_limit))
    elif safe_cruise and not near_overspeed:
        speed_max = float(np.clip(max(target_speed + 1.2, min(ego_speed + 0.8, overspeed_hard_limit)), cfg.env.min_speed, overspeed_hard_limit))
    else:
        speed_buffer = 1.0 if speed_mode == "free" else (0.8 if speed_mode == "cautious" else 0.5)
        hard_cap = overspeed_hard_limit if speed_mode == "free" else overspeed_soft_limit
        speed_max = float(np.clip(target_speed + speed_buffer, cfg.env.min_speed, hard_cap))
    constraints = {
    ...
    plan_validity = build_plan_validity(cfg, scene, parsed_command, {"target_lane": int(target_lane), "target_speed": float(target_speed), "behavior": behavior})
    return {
    ...
    def _format_memory_cases(self, cases: List[dict]) -> str:
        ...
        for item in cases:
            compact.append({
                ...
                "unsafe_headway_rate": round(float(item.get("unsafe_headway_rate", 0.0)), 3),
                "mean_cost_total": round(float(item.get("mean_cost_total", 0.0)), 3),
                "interventions": int(item.get("interventions", 0)),
            })
...
- signed_lane_offset_norm: {scene.get('signed_lane_offset_norm', 0.0):.3f}
- heading_error: {scene.get('heading_error', scene.get('heading', 0.0)):.3f}
- road_boundary_margin: {scene.get('road_boundary_margin', 1e6):.3f}
- road_boundary_proximity: {scene.get('road_boundary_proximity', 0.0):.3f}
- lane_boundary_proximity: {scene.get('lane_boundary_proximity', 0.0):.3f}
...
- Use both successful cases and caution/failure-like cases when they are provided; do not anchor on repeated low-speed templates.
- keep_lane, follow, brake, and yield must keep target_lane equal to the current lane_id.
- If lane_stability or boundary_risk is elevated, prioritize stabilizing in the current lane; do not request a lane change unless the adjacent lane is dynamically clear and clearly better.
- Treat expensive-success memories as weak context only, not as failures.
...
- For safe cruising keep_lane/follow scenes with front_distance >= 80m and ttc >= 15s, set hold_min_steps to 36-48, unless there is a concrete nearby risk event.
- Only when following closely, changing lane, or under near-risk conditions (for example front_distance < 50m or ttc < 10s) may hold_min_steps be 6-12.
- If front_distance >= 120m, ttc >= 15s, and there is no overspeed or lane instability issue, avoid unnecessarily low target speeds; target_speed should usually stay in a reasonable cruise range.
- If ego speed is close to or above the local speed limit, reduce target_speed instead of maintaining cruise speed.
...
class LLMPlanner:
    ...
    def __init__(self, cfg, backend: BaseLLMBackend, memory_readonly: bool = False):
        ...
        self.last_replan_debug: Dict[str, Any] = {"plan_age": 0, "planner_state": "INIT", "events": [], "shield_intervened": False, "force_replan_next": False}
    ...
    def observe_execution(self, scene: dict, verify_info: Optional[dict], shield_info: Optional[dict], step_idx: int) -> None:
        ...
        self.force_replan_next = bool(self.last_observed_shield_intervened) or bool((verify_info or {}).get("emergency", False))

    def should_replan(self, scene: dict, step_idx: int) -> bool:
        if step_idx <= 0:
            self.last_replan_debug = {
                "plan_age": 0,
                "planner_state": "INIT",
                "events": [],
                "shield_intervened": bool(self.last_observed_shield_intervened),
                "force_replan_next": bool(self.force_replan_next),
            }
            return False
        ...
        self.last_replan_debug = {
            "plan_age": int(gate.get("plan_age", 0)),
            "planner_state": str(gate.get("planner_state", "REPLAN")),
            "events": list(gate.get("events", gate.get("reasons", []))),
            "shield_intervened": bool(gate.get("shield_intervened", False)),
            "force_replan_next": bool(gate.get("force_replan_next", False)),
            "hard_events": list(gate.get("hard_events", [])),
            "replan_events": list(gate.get("replan_events", [])),
            "monitor_events": list(gate.get("monitor_events", [])),
        }
        ...
        return bool(gate.get("should_query", False))
    ...
    def _set_plan_commit(self, scene: dict, parsed: dict, step_idx: Optional[int]) -> None:
        validity = copy.deepcopy(parsed.get("plan_validity", {}))
        self.plan_commit = {
            "created_step": int(0 if step_idx is None else step_idx),
            "reference_scene": copy.deepcopy(scene),
            "reference_lane_feasibility": validity.get("reference_lane_feasibility", lane_feasibility_signature(scene)),
    ...
    def plan(self, scene: dict, step_idx: Optional[int] = None) -> dict:
        ...
        return {
            ...
            "planner_info": {
                ...
                "plan_age": int((self.last_query_context or {}).get("plan_age", self.last_replan_debug.get("plan_age", 0))) if step_idx is not None else 0,
                "events": list((self.last_query_context or {}).get("events", self.last_replan_debug.get("events", []))) if step_idx is not None else ["initial_plan"],
                "shield_intervened": bool((self.last_query_context or {}).get("shield_intervened", self.last_replan_debug.get("shield_intervened", False))) if step_idx is not None else False,
                "force_replan_next": bool((self.last_query_context or {}).get("force_replan_next", self.last_replan_debug.get("force_replan_next", False))) if step_idx is not None else False,
...
def apply_startup_alignment_override(cfg, scene: dict, action, constraints: dict, planner_info: Optional[dict], step_idx: Optional[int]):
    raw_action = np.asarray(action, dtype=np.float32).copy()
    info = {
        "startup_alignment_active": False,
        "startup_alignment_reason": "removed_fix10b",
        "startup_alignment_action": raw_action.copy(),
        "startup_alignment_blend": 0.0,
        "startup_alignment_target_speed": float((planner_info or {}).get("target_speed", scene.get("ego_speed", 0.0))),
        "startup_alignment_mode": "disabled",
        "lane_offset_norm": float(scene.get("signed_lane_offset_norm", scene.get("abs_lane_offset_norm", 0.0))),
        "heading_error": float(scene.get("heading_error", scene.get("heading", 0.0))),
        "raw_action_before_startup_alignment": raw_action.copy(),
        "raw_action_after_startup_alignment": raw_action.copy(),
    }
    return raw_action, info


def apply_local_recovery_override(cfg, scene: dict, action, constraints: dict):
    raw_action = np.asarray(action, dtype=np.float32).copy()
    info = {
        "recovery_active": False,
        "lane_offset": float(scene.get("signed_lane_offset_norm", scene.get("abs_lane_offset_norm", 0.0))),
        "heading_error": float(scene.get("heading_error", scene.get("heading", 0.0))),
        "raw_action": raw_action.copy(),
        "recovery_reason": "removed_fix10b",
        "recovery_action": raw_action.copy(),
    }
    return raw_action, info
...
class SafetyVerifier:
    ...
    def check(self, scene: dict, action, constraints: dict) -> dict:
        ...
        interaction = compute_interaction_risk_primitives(
            self.cfg,
            scene,
            action=action,
            constraints=constraints,
        ...
        ttc = float(scene["ttc"])
        signed_lane_offset = float(scene.get("signed_lane_offset_norm", 0.0))
        lane_offset = abs(signed_lane_offset)
        heading_error = float(scene.get("heading_error", scene.get("heading", 0.0)))
        road_proximity = float(scene.get("road_boundary_proximity", 0.0))
        lane_proximity = float(scene.get("lane_boundary_proximity", 0.0))
        lateral_speed = abs(float(scene.get("lateral_speed", 0.0))) / max(float(self.cfg.env.max_speed), 1e-6)
        steer_outward = bool(abs(steer) > 1e-4 and np.sign(steer) == np.sign(signed_lane_offset) and lane_offset > 0.08)
        ...
        front_collision_risk = float(interaction["front_collision_risk"])
        rear_collision_risk = float(interaction["rear_collision_risk"])
        active_lateral_merge_risk = float(interaction["active_lateral_merge_risk"])
        lateral_merge_risk = float(interaction["lateral_merge_risk"])
        merge_conflict_risk = float(interaction["merge_conflict_risk"])
        boundary_risk = float(interaction["boundary_risk"])
        projected_boundary_risk = _clip01(max(
            boundary_risk,
            road_proximity,
            lane_proximity,
            max(0.0, lane_offset - 0.25) / 0.75,
            abs(heading_error) / 0.85,
            0.65 * lateral_speed,
            0.45 if steer_outward else 0.0,
        ))
        lane_instability_high = bool(projected_boundary_risk > 0.35 or lane_offset > 0.40 or abs(heading_error) > 0.35)
        large_lane_offset = bool(lane_offset > self.cfg.safety.lane_offset_hard or projected_boundary_risk > 0.60)
        ...
        front_collision_risk_high = bool(front_collision_risk > 0.55)
        rear_collision_risk_high = bool(rear_collision_risk > 0.55)
        lateral_merge_risk_high = bool(active_lateral_merge_risk > 0.50)
        merge_conflict_risk_high = bool(merge_conflict_risk > 0.70 and bool(interaction.get("lateral_active", False)))
        boundary_risk_high = bool(boundary_risk > 0.45)
        emergency = hard_brake_zone or (low_ttc and unsafe_headway) or front_collision_risk > 0.85
        ...
        return {
            ...
            "front_collision_risk_high": bool(front_collision_risk_high),
            "rear_collision_risk_high": bool(rear_collision_risk_high),
            "lateral_merge_risk_high": bool(lateral_merge_risk_high),
            "merge_conflict_risk_high": bool(merge_conflict_risk_high),
            "boundary_risk_high": bool(boundary_risk_high),
            "front_collision_risk": float(front_collision_risk),
            "rear_collision_risk": float(rear_collision_risk),
            "lateral_merge_risk": float(lateral_merge_risk),
            "active_lateral_merge_risk": float(active_lateral_merge_risk),
            ...
            "left_rear_closing_speed": float(interaction["left_rear_closing_speed"]),
            "right_rear_closing_speed": float(interaction["right_rear_closing_speed"]),
            "intended_lateral_side": str(interaction["intended_lateral_side"]),
            "lateral_active": bool(interaction["lateral_active"]),
            "large_lane_offset": bool(large_lane_offset),
            "lane_instability_high": bool(lane_instability_high),
            "projected_boundary_risk": float(projected_boundary_risk),
            "signed_lane_offset": float(signed_lane_offset),
            "heading_error": float(heading_error),
            "road_boundary_proximity": float(road_proximity),
            ...
            "needs_intervention": bool(
                emergency
                or overspeed
                or large_lane_offset
                or lane_instability_high
                or lateral_merge_risk_high
                or merge_conflict_risk_high
                or boundary_risk_high
                or rear_collision_risk_high
                or over_acc
                or over_steer
            ),
        }
...
class SafetyShield:
    ...
    def apply(self, scene: dict, action, constraints: dict, verify_info: dict):
        ...
        signed_lane_offset = float(verify_info.get("signed_lane_offset", scene.get("signed_lane_offset_norm", 0.0)))
        heading_error = float(verify_info.get("heading_error", scene.get("heading_error", scene.get("heading", 0.0))))
        projected_boundary_risk = float(verify_info.get("projected_boundary_risk", 0.0))
        rear_collision_risk = float(verify_info.get("rear_collision_risk", 0.0))
        active_lateral_merge_risk = float(verify_info.get("active_lateral_merge_risk", 0.0))
        intended_lateral_side = str(verify_info.get("intended_lateral_side", "none"))
        acc_bound, steer_bound = physical_to_normalized_action_bounds(self.cfg, constraints)

        if projected_boundary_risk > 0.30:
            inward_steer = float(np.clip(-0.65 * signed_lane_offset - 0.40 * heading_error, -0.95 * steer_bound, 0.95 * steer_bound))
            if abs(inward_steer) > abs(steer) or (abs(steer) > 1e-4 and np.sign(steer) == np.sign(signed_lane_offset)):
                blend = 0.70 if projected_boundary_risk > 0.55 else 0.45
                steer = float((1.0 - blend) * steer + blend * inward_steer)
                modified = True
                reasons.append("boundary_steer_guard")
            if projected_boundary_risk > 0.55 and acc > 0.20 * acc_bound:
                acc = min(acc, 0.20 * acc_bound)
                modified = True
                reasons.append("boundary_acc_soft_limit")

        if active_lateral_merge_risk > 0.45:
            if intended_lateral_side == "left":
                blocked_steer_sign = -1.0
            elif intended_lateral_side == "right":
                blocked_steer_sign = 1.0
            else:
                blocked_steer_sign = float(np.sign(steer))
            if abs(blocked_steer_sign) > 0.0 and abs(steer) > 1e-4 and np.sign(steer) == np.sign(blocked_steer_sign):
                steer *= 0.25 if active_lateral_merge_risk > 0.70 else 0.55
                modified = True
                reasons.append("lateral_merge_steer_guard")
            if acc > 0.18 * acc_bound:
                acc = min(acc, 0.18 * acc_bound)
                modified = True
                reasons.append("lateral_merge_acc_soft_limit")

        if rear_collision_risk > 0.50 and not bool(verify_info.get("emergency", False)) and acc < -0.10 * acc_bound:
            acc = max(acc, -0.10 * acc_bound)
            modified = True
            reasons.append("rear_risk_brake_release")

        if verify_info["large_lane_offset"] and abs(steer) > 1e-6 and np.sign(steer) == np.sign(signed_lane_offset):
...
def compute_costs(cfg, scene: dict, next_scene: dict, action, constraints: dict) -> dict:
    ...
    if cfg.ablation.disable_constraint_costs:
        return {
            "collision_cost": 0.0,
            "collision_cost_front": 0.0,
            "collision_cost_rear": 0.0,
            "collision_cost_lateral": 0.0,
            "headway_cost": 0.0,
            "overspeed_cost": 0.0,
            "comfort_cost": 0.0,
            "lane_cost": 0.0,
    ...
    target_lane = next_scene.get("lane_id", scene.get("lane_id", None))
    current_interaction = compute_interaction_risk_primitives(
        cfg,
        scene,
        action=action,
        constraints=constraints,
    ...
    next_interaction = compute_interaction_risk_primitives(
        cfg,
        next_scene,
        action=action,
        constraints=constraints,
    ...
    collision_cost_front = _clip01(max(
        float(current_interaction["front_collision_risk"]),
        float(next_interaction["front_collision_risk"]),
    ))
    collision_cost_rear = _clip01(max(
        float(current_interaction["rear_collision_risk"]),
        float(next_interaction["rear_collision_risk"]),
    ))
    collision_cost_lateral = _clip01(max(
        float(current_interaction["active_lateral_merge_risk"]),
        float(next_interaction["active_lateral_merge_risk"]),
    ))
    collision_cost = _clip01(max(
        crash_cost,
    ...
    min_headway = float(constraints["min_headway"])
    front_distance = float(next_scene["front_distance"])
    closing_speed = float(max(0.0, -float(next_scene.get("front_rel_speed", 0.0))))
    headway_gap = max(0.0, min_headway - front_distance)
    headway_hard = _clip01(headway_gap / max(min_headway, 1e-6))
    proactive_threshold = float(cfg.planner.headway_proactive_factor) * min_headway
    headway_proactive = _squared_barrier_cost(front_distance, proactive_threshold)
    closing_zone = max(0.0, (3.0 * min_headway - front_distance) / max(3.0 * min_headway, 1e-6))
    closing_penalty = 0.0
    if closing_speed > float(cfg.planner.headway_shape_closing_threshold):
        closing_penalty = float(np.clip((closing_speed - float(cfg.planner.headway_shape_closing_threshold)) / 8.0, 0.0, 1.0)) * closing_zone
    headway_cost = _clip01(max(headway_hard, 0.55 * headway_proactive + float(cfg.planner.headway_closing_speed_factor) * closing_penalty))

    speed_max = float(constraints["speed_max"])
    next_speed = float(next_scene["ego_speed"])
    overspeed_gap = max(0.0, next_speed - speed_max)
    overspeed_hard = _clip01(overspeed_gap / max(0.06 * speed_max, 0.75))
    overspeed_proactive_threshold = float(cfg.planner.overspeed_proactive_ratio) * speed_max
    proactive_gap = max(0.0, next_speed - overspeed_proactive_threshold)
    proactive_denom = max(speed_max - overspeed_proactive_threshold, 0.75)
    overspeed_proactive = _clip01((proactive_gap / proactive_denom) ** 2)
    overspeed_push = 0.0
    if next_speed > overspeed_proactive_threshold:
        overspeed_push = _clip01((float(action[0]) - 0.02) / 0.35)
    overspeed_cost = _clip01(max(overspeed_hard, 0.70 * overspeed_proactive + 0.30 * overspeed_push))

    lane_terms = _lane_boundary_risk_terms(cfg, next_scene)
    offroad_cost = 1.0 if _rl_is_offroad_scene(cfg, next_scene) else 0.0
    boundary_cost = _clip01(max(
        float(current_interaction["boundary_cost"]),
        float(next_interaction["boundary_cost"]),
        float(lane_terms["lane_cost"]),
        offroad_cost,
    ))
    lane_cost = float(boundary_cost)

    acc_cmd = abs(float(action[0])) * float(cfg.safety.action_acc_scale)
    steer_cmd = abs(float(action[1])) * float(cfg.safety.action_steer_scale)
    acc_delta = abs(float(action[0] - last_action[0])) * float(cfg.safety.action_acc_scale)
    steer_delta = abs(float(action[1] - last_action[1])) * float(cfg.safety.action_steer_scale)
    acc_max_ref = max(float(constraints.get("acc_max", cfg.planner.acc_max)), 1e-6)
    comfort_acc_ref = max(float(constraints.get("comfort_acc_delta", cfg.planner.comfort_acc_delta)), 1e-6)
    comfort_steer_ref = max(float(constraints.get("steer_max", cfg.planner.steer_max)), 1e-6)
    acc_mag_cost = _upper_quadratic_cost(acc_cmd, 0.72 * acc_max_ref, 1.05 * acc_max_ref)
    acc_delta_cost = _upper_quadratic_cost(acc_delta, 0.85 * comfort_acc_ref, 1.80 * comfort_acc_ref)
    steer_mag_cost = _upper_quadratic_cost(steer_cmd, 0.72 * comfort_steer_ref, 1.20 * comfort_steer_ref)
    steer_delta_cost = _upper_quadratic_cost(steer_delta, 0.80 * comfort_steer_ref, 1.65 * comfort_steer_ref)
    comfort_cost = _clip01(
        0.18 * acc_mag_cost
        + 0.37 * acc_delta_cost
        + 0.15 * steer_mag_cost
        + 0.30 * steer_delta_cost
    )
    ...
    total_cost = (
        ...
        + cfg.safety.cost_lane_weight * lane_cost
    )

    return {
        "collision_cost": float(collision_cost),
        "collision_cost_front": float(collision_cost_front),
        "collision_cost_rear": float(collision_cost_rear),
        "collision_cost_lateral": float(collision_cost_lateral),
        "headway_cost": float(headway_cost),
        "overspeed_cost": float(overspeed_cost),
        "comfort_cost": float(comfort_cost),
        "lane_cost": float(lane_cost),
        "boundary_cost": float(boundary_cost),
        "front_collision_risk": float(next_interaction["front_collision_risk"]),
        "rear_collision_risk": float(next_interaction["rear_collision_risk"]),
        "lateral_merge_risk": float(next_interaction["lateral_merge_risk"]),
        "active_lateral_merge_risk": float(next_interaction["active_lateral_merge_risk"]),
        "lane_change_gap_risk": float(next_interaction["lane_change_gap_risk"]),
        "merge_conflict_risk": float(next_interaction["merge_conflict_risk"]),
        "lane_center_soft_cost": float(lane_terms["center_soft_term"]),
        "lane_center_hard_cost": float(lane_terms["center_hard_term"]),
        "road_boundary_soft_cost": float(lane_terms["road_soft_term"]),
        "road_boundary_hard_cost": float(lane_terms["road_hard_term"]),
...
def compute_reward(
    ...
    lane_track_error = 0.0
    reference_y = float(next_scene.get("lane_center_y", 0.0))
    if planner_used and current_waypoints is not None and len(current_waypoints) > 0:
        ...
        reference_y = float(target_wp[1])
        target_v = float(target_wp[2])
        waypoint_pos_error = abs(next_scene["ego_y"] - reference_y) / max(cfg.env.lane_width, 1e-6)
    ...
    waypoint_pos_error = float(np.clip(waypoint_pos_error, 0.0, float(cfg.reward.max_waypoint_pos_error)))
    waypoint_speed_error = float(np.clip(waypoint_speed_error, 0.0, float(cfg.reward.max_waypoint_speed_error)))

    _, lane_track_error = _lane_center_tracking_error(cfg, next_scene, reference_y=reference_y)
    lane_terms = _lane_boundary_risk_terms(cfg, next_scene)
    lane_center_penalty = float(
        np.clip(
            0.55 * (lane_track_error ** 2) + 0.12 * lane_terms["center_soft_term"] + 0.24 * lane_terms["center_hard_term"],
            0.0,
            float(cfg.reward.max_lane_center_penalty),
        )
    )
    lane_boundary_penalty = float(lane_terms["lane_cost"])
    heading_penalty = float(np.clip(0.60 * lane_terms["heading_term"] + 0.25 * lane_terms["lateral_term"], 0.0, float(cfg.reward.max_heading_penalty)))
    ...
    headway_penalty = _clip01(max(0.0, constraints["min_headway"] - next_scene["front_distance"]) / max(
        constraints["min_headway"], 1e-6
    ))
    speed_max = float(constraints["speed_max"])
    next_speed = float(next_scene["ego_speed"])
    overspeed_hard = _clip01(max(0.0, next_speed - speed_max) / max(speed_max, 1e-6))
    overspeed_proactive_threshold = float(cfg.planner.overspeed_proactive_ratio) * speed_max
    proactive_gap = max(0.0, next_speed - overspeed_proactive_threshold)
    proactive_denom = max(speed_max - overspeed_proactive_threshold, 1e-6)
    overspeed_proactive = float((proactive_gap / proactive_denom) ** 2)
    overspeed_penalty = _clip01(max(overspeed_hard, 0.35 * overspeed_proactive))
    action_penalty = float(np.clip(abs(float(action[0])) + abs(float(action[1])), 0.0, float(cfg.reward.max_action_penalty)))
    smooth_penalty = float(
        np.clip(
            abs(float(action[0] - last_action[0])) + abs(float(action[1] - last_action[1])),
            0.0,
            float(cfg.reward.max_smooth_penalty),
        )
    )
    lane_change_penalty = abs(next_scene["lane_id"] - scene["lane_id"])

    env_reward_term = cfg.reward.env_reward_scale * float(env_reward) if bool(use_env_reward) else 0.0
    task_return_core = float(cfg.reward.w_progress * progress_reward + env_reward_term)
    ...
    task_reward = (
        ...
        - cfg.reward.w_lane_center * lane_center_penalty
        - cfg.reward.w_heading_align * heading_penalty
    ...
    safety_penalty = (
        ...
        + cfg.reward.w_lane_boundary * lane_boundary_penalty
    )
    if task_only_safety:
        reward = task_reward - float(getattr(cfg.reward, "constrained_aux_safety_coef", 0.0)) * safety_penalty
        if _rl_terminal_failure(cfg, next_scene):
            reward -= float(cfg.reward.constrained_terminal_failure_penalty)
    else:
        reward = task_reward - safety_penalty

    if bool(use_env_reward):
        reward += env_reward_term

    terms = {
        "progress_reward": float(progress_reward),
        "waypoint_pos_error": float(waypoint_pos_error),
        "waypoint_speed_error": float(waypoint_speed_error),
        "lane_track_error": float(lane_track_error),
        "lane_center_penalty": float(lane_center_penalty),
        "lane_boundary_penalty": float(lane_boundary_penalty),
        ...
        "task_return_core": float(task_return_core),
...
def choose_reward(
    ...
    use_env_reward = bool(cfg.reward.use_env_reward)
    if mode_is_constrained(mode) and not bool(getattr(cfg.reward, "constrained_use_env_reward", False)):
    ...
    reward, terms = compute_reward(
        ...
        task_only_safety=bool(mode_is_constrained(mode) and cfg.reward.constrained_task_only),
        use_env_reward=bool(use_env_reward),
    )
    terms["source"] = "shaped"
    return reward, terms
...
class ReplayBuffer:
    def __init__(
        ...
        capacity,
        device,
        behavior_prior_dim: int,
        lane_prior_dim: int,
        safety_priority_mix: float = 0.0,
        safety_priority_power: float = 1.0,
        safety_priority_min_cost: float = 0.0,
    ):
        ...
        self.safety_priority_mix = float(np.clip(safety_priority_mix, 0.0, 1.0))
        self.safety_priority_power = float(max(safety_priority_power, 1e-6))
        self.safety_priority_min_cost = float(max(safety_priority_min_cost, 0.0))
        ...
        self.teacher_is_corrective = np.zeros((capacity, 1), dtype=np.float32)
        self.hazard_score = np.zeros((capacity,), dtype=np.float32)
        self.sample_priority = np.ones((capacity,), dtype=np.float32)

    def add(self, state, action, reward, next_state, done, cost_dict=None, teacher_dict=None):
        if cost_dict is None:
            cost_dict = {"collision_cost": 0.0, "headway_cost": 0.0, "overspeed_cost": 0.0, "comfort_cost": 0.0, "lane_cost": 0.0, "total_cost": 0.0}
        ...
        self.cost_lane[self.ptr] = cost_dict.get("lane_cost", 0.0)
        ...
        self.teacher_is_corrective[self.ptr] = float(teacher_dict.get("teacher_is_corrective", 0.0))
        ...
        collision_cost = float(cost_dict.get("collision_cost", 0.0))
        headway_cost = float(cost_dict.get("headway_cost", 0.0))
        overspeed_cost = float(cost_dict.get("overspeed_cost", 0.0))
        comfort_cost = float(cost_dict.get("comfort_cost", 0.0))
        lane_cost = float(cost_dict.get("lane_cost", 0.0))
        total_cost = float(cost_dict.get("total_cost", 0.0))
        hazard_score = max(collision_cost, headway_cost, overspeed_cost, lane_cost, 0.75 * total_cost, 0.40 * comfort_cost)
        priority = (
            1.0
            + 4.0 * collision_cost
            + 2.4 * lane_cost
        ...
        if float(teacher_dict.get("teacher_is_corrective", 0.0)) > 0.5:
            priority += 1.25
        if float(done) > 0.5 and hazard_score >= self.safety_priority_min_cost:
            priority += 0.6
        self.hazard_score[self.ptr] = float(hazard_score)
        self.sample_priority[self.ptr] = float(max(priority, 1e-3))
    ...
    def sample(self, batch_size):
        ...
        idx = None
        safety_batch = int(round(float(batch_size) * self.safety_priority_mix))
        safety_batch = int(np.clip(safety_batch, 0, batch_size))
        if safety_batch > 0:
            hazard_pool = np.flatnonzero(self.hazard_score[:self.size] >= self.safety_priority_min_cost)
            if hazard_pool.size > 0:
                weights = np.power(np.maximum(self.sample_priority[hazard_pool], 1e-6), self.safety_priority_power)
                weights = weights / max(float(weights.sum()), 1e-6)
                hazard_idx = np.random.choice(hazard_pool, size=safety_batch, replace=hazard_pool.size < safety_batch, p=weights)
                uniform_idx = np.random.randint(0, self.size, size=batch_size - safety_batch)
                idx = np.concatenate([hazard_idx, uniform_idx], axis=0)
        ...
        return dict(
            ...
            teacher_is_corrective=torch.as_tensor(self.teacher_is_corrective[idx], device=self.device),
        )
...
class Actor(nn.Module):
    def __init__(self, state_dim, action_dim, hidden_dim, behavior_dim: int, lane_dim: int):
        super().__init__()
        ...
        self.behavior_head = nn.Linear(hidden_dim, int(behavior_dim))
        self.lane_head = nn.Linear(hidden_dim, int(lane_dim))

    def forward(self, state):
        h = self.backbone(state)
        mu = self.mu(h)
        log_std = torch.clamp(self.log_std(h), LOG_STD_MIN, LOG_STD_MAX)
        behavior_logits = self.behavior_head(h)
        lane_logits = self.lane_head(h)
        return mu, log_std, behavior_logits, lane_logits

    def sample(self, state):
        mu, log_std, behavior_logits, lane_logits = self(state)
        std = log_std.exp()
        dist = Normal(mu, std)
        ...
        return action, log_prob, mu_action, behavior_logits, lane_logits
...
class SACAgent:
    def __init__(self, state_dim, action_dim, cfg):
        ...
        self.behavior_dim = len(BEHAVIOR_VOCAB)
        self.lane_dim = int(cfg.env.lanes_count)

        self.actor = Actor(state_dim, action_dim, cfg.sac.hidden_dim, behavior_dim=self.behavior_dim, lane_dim=self.lane_dim).to(self.device)
        ...
        self.replay = ReplayBuffer(
            ...
            cfg.sac.buffer_size,
            self.device,
            behavior_prior_dim=self.behavior_dim,
            lane_prior_dim=self.lane_dim,
            safety_priority_mix=float(getattr(cfg.sac, "safety_priority_mix", 0.0)),
            safety_priority_power=float(getattr(cfg.sac, "safety_priority_power", 1.0)),
            safety_priority_min_cost=float(getattr(cfg.sac, "safety_priority_min_cost", 0.0)),
        )
    ...
    def _teacher_guidance_loss(self, batch: dict, new_action: torch.Tensor, behavior_logits: torch.Tensor, lane_logits: torch.Tensor) -> tuple[torch.Tensor, dict]:
        teacher_weight = batch.get("teacher_weight")
        teacher_action = batch.get("teacher_action")
        behavior_prior = batch.get("teacher_behavior_prior")
        lane_prior = batch.get("teacher_lane_prior")
        teacher_is_corrective = batch.get("teacher_is_corrective")
        if teacher_weight is None or teacher_action is None:
            zero = torch.zeros((), device=self.device)
            return zero, {
                "teacher_guidance_loss": 0.0,
                "teacher_weight_mean": 0.0,
                "teacher_corrective_weight_mean": 0.0,
                "teacher_guided_weight_mean": 0.0,
                "teacher_action_mse": 0.0,
                "teacher_behavior_kl": 0.0,
                "teacher_lane_kl": 0.0,
            }
        teacher_weight = teacher_weight.clamp(min=0.0)
        corrective_mask = torch.zeros_like(teacher_weight)
        if teacher_is_corrective is not None:
            corrective_mask = (teacher_is_corrective > 0.5).float()
        guided_mask = 1.0 - corrective_mask

        guided_weight = teacher_weight * guided_mask
        if bool(getattr(self.cfg.sac, "teacher_qfilter_enabled", False)) and torch.any(guided_mask > 0.0):
            with torch.no_grad():
                q_teacher = torch.min(self.q1(batch["state"], teacher_action), self.q2(batch["state"], teacher_action))
                q_policy = torch.min(self.q1(batch["state"], new_action), self.q2(batch["state"], new_action))
                accept = (q_teacher >= (q_policy - float(getattr(self.cfg.sac, "teacher_qfilter_margin", 0.0)))).float()
            guided_weight = guided_weight * accept
        corrective_weight = teacher_weight * corrective_mask

        action_mse = ((new_action - teacher_action) ** 2).mean(dim=-1, keepdim=True)
        guided_action_loss = (guided_weight * action_mse).mean() * float(self.cfg.sac.guided_teacher_bc_coef)
        corrective_action_loss = (corrective_weight * action_mse).mean() * float(self.cfg.sac.corrective_teacher_bc_coef)
        action_loss = (
            guided_action_loss + corrective_action_loss
        ) * float(self.cfg.sac.teacher_guidance_coef) * float(self.cfg.sac.teacher_action_coef)

        behavior_prior = behavior_prior / behavior_prior.sum(dim=-1, keepdim=True).clamp(min=1e-6)
        lane_prior = lane_prior / lane_prior.sum(dim=-1, keepdim=True).clamp(min=1e-6)
        behavior_kl = F.kl_div(F.log_softmax(behavior_logits, dim=-1), behavior_prior, reduction="none").sum(dim=-1, keepdim=True)
        lane_kl = F.kl_div(F.log_softmax(lane_logits, dim=-1), lane_prior, reduction="none").sum(dim=-1, keepdim=True)
        guided_behavior_loss = (guided_weight * behavior_kl).mean() * float(self.cfg.sac.guided_teacher_bc_coef)
        corrective_behavior_loss = (corrective_weight * behavior_kl).mean() * float(self.cfg.sac.corrective_teacher_bc_coef)
        behavior_loss = (guided_behavior_loss + corrective_behavior_loss) * float(self.cfg.sac.teacher_guidance_coef) * float(self.cfg.sac.behavior_kl_coef)
        guided_lane_loss = (guided_weight * lane_kl).mean() * float(self.cfg.sac.guided_teacher_bc_coef)
        corrective_lane_loss = (corrective_weight * lane_kl).mean() * float(self.cfg.sac.corrective_teacher_bc_coef)
        lane_loss = (guided_lane_loss + corrective_lane_loss) * float(self.cfg.sac.teacher_guidance_coef) * float(self.cfg.sac.lane_kl_coef)
        total_loss = action_loss + behavior_loss + lane_loss
        return total_loss, {
            "teacher_guidance_loss": float(total_loss.detach().item()),
            "teacher_weight_mean": float(teacher_weight.mean().detach().item()),
            "teacher_corrective_weight_mean": float(corrective_weight.mean().detach().item()),
            "teacher_guided_weight_mean": float(guided_weight.mean().detach().item()),
            "teacher_action_mse": float(action_mse.mean().detach().item()),
            "teacher_behavior_kl": float(behavior_kl.mean().detach().item()),
            "teacher_lane_kl": float(lane_kl.mean().detach().item()),
        }
    ...
    def select_action(self, state, evaluate=False):
        ...
        with torch.no_grad():
            if evaluate:
                _, _, action, _, _ = self.actor.sample(state_t)
            else:
                action, _, _, _, _ = self.actor.sample(state_t)
    ...
    def update(self):
        ...
        q1_loss.backward()
        _clip_grad_norm_(self.q1.parameters(), self.cfg.sac.critic_grad_clip_norm)
        self.q1_opt.step()

        self.q2_opt.zero_grad()
        q2_loss.backward()
        _clip_grad_norm_(self.q2.parameters(), self.cfg.sac.critic_grad_clip_norm)
        self.q2_opt.step()

        new_action, log_prob, _, behavior_logits, lane_logits = self.actor.sample(state)
        q1_new = self.q1(state, new_action)
        q2_new = self.q2(state, new_action)
        q_new = torch.min(q1_new, q2_new)

        teacher_loss, teacher_info = self._teacher_guidance_loss(batch, new_action, behavior_logits, lane_logits)
        actor_loss = (self.alpha.detach() * log_prob - q_new).mean() + teacher_loss

        self.actor_opt.zero_grad()
        actor_loss.backward()
        _clip_grad_norm_(self.actor.parameters(), self.cfg.sac.actor_grad_clip_norm)
        self.actor_opt.step()
...
class ConstrainedSACAgent(SACAgent):
    """
    Constrained SAC with:
    - explicit cost critics for collision/headway/overspeed/comfort/lane risk
    - projected PID-style dual updates driven by empirical costs rather than critic-only estimates
    - strong intervention cloning for corrected actions, plus softer decayed teacher guidance
    - dense lane-boundary costs before off-road happens
    """
    ...
    def __init__(self, state_dim, action_dim, cfg):
        ...
        self._lambda_collision = torch.tensor(init_lambda, dtype=torch.float32, device=self.device)
        self._lambda_headway = torch.tensor(init_lambda, dtype=torch.float32, device=self.device)
        self._lambda_overspeed = torch.tensor(init_lambda, dtype=torch.float32, device=self.device)
        self._lambda_comfort = torch.tensor(init_lambda, dtype=torch.float32, device=self.device)
        self._lambda_lane = torch.tensor(init_lambda, dtype=torch.float32, device=self.device)
        ...
        self.qc_lane = Critic(state_dim, action_dim, cfg.sac.hidden_dim).to(self.device)
        ...
        self.qc_lane_target = Critic(state_dim, action_dim, cfg.sac.hidden_dim).to(self.device)
        ...
        self.qc_lane_target.load_state_dict(self.qc_lane.state_dict())
        ...
        self.qc_lane_opt = torch.optim.Adam(self.qc_lane.parameters(), lr=cfg.sac.critic_lr)
        ...
        self.ema_lane_cost = None
        self.dual_pid_state = {
            "collision": {"integral": 0.0, "prev_error": 0.0},
            "headway": {"integral": 0.0, "prev_error": 0.0},
            "overspeed": {"integral": 0.0, "prev_error": 0.0},
            "comfort": {"integral": 0.0, "prev_error": 0.0},
            "lane": {"integral": 0.0, "prev_error": 0.0},
        }

    @property
    def lambda_collision(self):
        return self._lambda_collision

    @property
    def lambda_headway(self):
        return self._lambda_headway

    @property
    def lambda_overspeed(self):
        return self._lambda_overspeed

    @property
    def lambda_comfort(self):
        return self._lambda_comfort

    @property
    def lambda_lane(self):
        return self._lambda_lane

    def _discounted_cost_upper_bound(self) -> float:
        return float(1.0 / max(1.0 - float(self.gamma), 1e-6))

    def _average_cost_q(self, qc_value: torch.Tensor) -> torch.Tensor:
        bounded_q = torch.clamp(qc_value, min=0.0, max=self._discounted_cost_upper_bound())
        return (1.0 - float(self.gamma)) * bounded_q

    def _constraint_violation(self, qc_value: torch.Tensor, limit: float) -> torch.Tensor:
        limit = max(float(limit), 1e-6)
        avg_cost = self._average_cost_q(qc_value)
        violation = (avg_cost - limit) / limit
        return torch.clamp(violation, min=0.0, max=float(self.cfg.sac.dual_violation_clip))

    def _dual_signal(
        self,
        name: str,
        ema_cost: float,
        batch_cost_tensor: torch.Tensor,
        policy_cost_tensor: torch.Tensor,
        limit: float,
        ...
        limit = max(float(limit), 1e-6)
        batch_cost = float(batch_cost_tensor.item())
        policy_cost = float(policy_cost_tensor.item())
        ema_violation = (float(ema_cost) - limit) / limit
        batch_violation = (batch_cost - limit) / limit
        policy_violation = (policy_cost - limit) / limit
        batch_mix = float(np.clip(self.cfg.sac.dual_batch_mix, 0.0, 1.0))
        policy_mix = float(np.clip(self.cfg.sac.dual_policy_mix, 0.0, 1.0))
        empirical_error = (1.0 - batch_mix) * ema_violation + batch_mix * batch_violation
        error = (1.0 - policy_mix) * empirical_error + policy_mix * policy_violation
        clip = float(self.cfg.sac.dual_violation_clip)
        error = float(np.clip(error, -clip, clip))
        state = self.dual_pid_state[name]
        decay = float(np.clip(getattr(self.cfg.sac, "dual_integral_decay", 1.0), 0.0, 1.0))
        current_lambda = float(lambda_tensor.item())
        upper_blocked = current_lambda >= float(max_value) - 1e-9 and error > 0.0
        lower_blocked = current_lambda <= float(min_value) + 1e-9 and error < 0.0
        allow_integral = not (upper_blocked or lower_blocked)
        integral_clip = max(0.0, float(getattr(self.cfg.sac, "dual_integral_clip", 0.0)))
        integral = float(state.get("integral", 0.0)) * decay
        if allow_integral:
            integral += error
        integral = float(np.clip(integral, -integral_clip, integral_clip))
        derivative = error - float(state.get("prev_error", 0.0))
        state["integral"] = integral
        state["prev_error"] = error
        release_active = 0.0
        release_count = int(state.get("release_count", 0))
        signal = (
            float(getattr(self.cfg.sac, "dual_pid_kp", 0.0)) * error
            + float(getattr(self.cfg.sac, "dual_pid_ki", 0.0)) * integral
            + float(getattr(self.cfg.sac, "dual_pid_kd", 0.0)) * derivative
        )

        if name == "overspeed":
            release_margin = max(0.0, float(getattr(self.cfg.sac, "dual_overspeed_release_margin", 0.0)))
            patience = max(1, int(getattr(self.cfg.sac, "dual_overspeed_release_patience", 1)))
            if error <= -release_margin and current_lambda > float(min_value) + 1e-9:
                release_count += 1
            else:
                release_count = 0
            if release_count >= patience:
                release_active = 1.0
                release_decay = float(np.clip(
                    getattr(self.cfg.sac, "dual_overspeed_release_integral_decay", 1.0),
                    0.0,
                    1.0,
                ))
                integral = float(np.clip(integral * release_decay, -integral_clip, integral_clip))
                state["integral"] = integral
                signal = (
                    float(getattr(self.cfg.sac, "dual_pid_kp", 0.0)) * error
                    + float(getattr(self.cfg.sac, "dual_pid_ki", 0.0)) * integral
                ...
                lambda_span = max(float(max_value) - float(min_value), 1e-6)
                lambda_pressure = float(np.clip((current_lambda - float(min_value)) / lambda_span, 0.0, 1.0))
                release_strength = (
                    float(getattr(self.cfg.sac, "dual_overspeed_release_scale", 1.0))
                    * max(0.0, -error - release_margin)
                    + lambda_pressure
                )
                signal = min(signal, -release_strength)
            state["release_count"] = release_count

        raw_signal = float(signal)
        signal = float(np.clip(raw_signal, -clip, clip))
        details = {
            "ema_violation": float(ema_violation),
            "batch_violation": float(batch_violation),
            "policy_violation": float(policy_violation),
            "empirical_error": float(empirical_error),
            "policy_mix": float(policy_mix),
            "batch_mix": float(batch_mix),
            "integral": float(state.get("integral", integral)),
            "derivative": float(derivative),
            "raw_signal": float(raw_signal),
            "integral_blocked": float(not allow_integral),
            "release_active": float(release_active),
            "release_count": float(release_count),
        }
        return signal, error, details

    def _update_dual_lambda(
        self,
        name: str,
        lambda_tensor: torch.Tensor,
        signal: float,
        ...
        current = float(lambda_tensor.item())
        lr_scale = float(self.cfg.sac.lambda_lr) * float(scale)
        raw_updated = current + lr_scale * float(signal)
        updated = float(np.clip(raw_updated, min_value, max_value))
        saturated = abs(updated - raw_updated) > 1e-12
        anti_windup = 0.0
        if saturated and name in self.dual_pid_state:
            ki = float(getattr(self.cfg.sac, "dual_pid_ki", 0.0))
            aw_gain = max(0.0, float(getattr(self.cfg.sac, "dual_anti_windup_gain", 0.0)))
            if ki > 0.0 and aw_gain > 0.0:
                denom = max(abs(lr_scale * ki), 1e-12)
                anti_windup = aw_gain * (updated - raw_updated) / denom
                state = self.dual_pid_state[name]
                integral_clip = max(0.0, float(getattr(self.cfg.sac, "dual_integral_clip", 0.0)))
                state["integral"] = float(np.clip(
                    float(state.get("integral", 0.0)) + anti_windup,
                    -integral_clip,
                    integral_clip,
                ))
        if name in self.dual_pid_state:
            self.dual_pid_state[name]["last_saturated"] = float(saturated)
            self.dual_pid_state[name]["last_anti_windup"] = float(anti_windup)
        lambda_tensor.fill_(updated)
        return updated, {
            "saturated": float(saturated),
            "anti_windup": float(anti_windup),
            "raw_lambda": float(raw_updated),
        }
    ...
    def _update_single_cost_critic(self, critic, critic_target, optimizer, state, action, done, next_state, next_action, immediate_cost):
        with torch.no_grad():
            ...
            target_cost = torch.clamp(target_cost, min=0.0, max=self._discounted_cost_upper_bound())
        ...
        optimizer.zero_grad()
        loss.backward()
        _clip_grad_norm_(critic.parameters(), self.cfg.sac.critic_grad_clip_norm)
        optimizer.step()
    ...
    def update(self):
        ...
        qc_lane_loss, _, qc_lane_target_mean = self._update_single_cost_critic(
            self.qc_lane, self.qc_lane_target, self.qc_lane_opt,
            state, action, done, next_state, next_action, cost_lane
        )
        ...
        qc_lane_new = self.qc_lane(state, new_action)

        qc_collision_term = self._constraint_violation(qc_collision_new, self.cfg.sac.cost_limit_collision)
        qc_headway_term = self._constraint_violation(qc_headway_new, self.cfg.sac.cost_limit_headway)
        qc_overspeed_term = self._constraint_violation(qc_overspeed_new, self.cfg.sac.cost_limit_overspeed)
        qc_comfort_term = self._constraint_violation(qc_comfort_new, self.cfg.sac.cost_limit_comfort)
        qc_lane_term = self._constraint_violation(qc_lane_new, self.cfg.sac.cost_limit_lane)
        actor_penalty = (
            self.lambda_collision.detach() * qc_collision_term
            + self.lambda_headway.detach() * qc_headway_term
            + float(self.cfg.sac.actor_overspeed_penalty_coef) * self.lambda_overspeed.detach() * qc_overspeed_term
            + float(self.cfg.sac.actor_comfort_penalty_coef) * self.lambda_comfort.detach() * qc_comfort_term
            + float(self.cfg.sac.actor_lane_penalty_coef) * self.lambda_lane.detach() * qc_lane_term
        )
        ...
        batch_lane_mean = cost_lane.mean().detach()
        policy_collision_cost = self._average_cost_q(qc_collision_new).mean().detach()
        policy_headway_cost = self._average_cost_q(qc_headway_new).mean().detach()
        policy_overspeed_cost = self._average_cost_q(qc_overspeed_new).mean().detach()
        policy_comfort_cost = self._average_cost_q(qc_comfort_new).mean().detach()
        policy_lane_cost = self._average_cost_q(qc_lane_new).mean().detach()
        ...
        self.ema_lane_cost = _ema_update(self.ema_lane_cost, batch_lane_mean)

        collision_signal, collision_error, collision_dual = self._dual_signal(
            "collision",
            float(self.ema_collision_cost),
            batch_collision_mean,
            policy_collision_cost,
            self.cfg.sac.cost_limit_collision,
            self._lambda_collision,
            min_value=float(self.cfg.sac.min_lambda_collision),
            max_value=float(self.cfg.sac.lambda_max),
        )
        headway_signal, headway_error, headway_dual = self._dual_signal(
            "headway",
            float(self.ema_headway_cost),
            batch_headway_mean,
            policy_headway_cost,
            self.cfg.sac.cost_limit_headway,
            self._lambda_headway,
            min_value=float(self.cfg.sac.min_lambda_headway),
            max_value=float(self.cfg.sac.lambda_max),
        )
        overspeed_signal, overspeed_error, overspeed_dual = self._dual_signal(
            "overspeed",
            float(self.ema_overspeed_cost),
            batch_overspeed_mean,
            policy_overspeed_cost,
            self.cfg.sac.cost_limit_overspeed,
            self._lambda_overspeed,
            min_value=float(getattr(self.cfg.sac, "min_lambda_overspeed", 0.0)),
            max_value=float(self.cfg.sac.lambda_overspeed_max),
        )
        comfort_signal, comfort_error, comfort_dual = self._dual_signal(
            "comfort",
            float(self.ema_comfort_cost),
            batch_comfort_mean,
            policy_comfort_cost,
            self.cfg.sac.cost_limit_comfort,
            self._lambda_comfort,
            min_value=0.0,
            max_value=float(self.cfg.sac.lambda_max),
        )
        lane_signal, lane_error, lane_dual = self._dual_signal(
            "lane",
            float(self.ema_lane_cost),
            batch_lane_mean,
            policy_lane_cost,
            self.cfg.sac.cost_limit_lane,
            self._lambda_lane,
            min_value=float(self.cfg.sac.min_lambda_lane),
            max_value=float(self.cfg.sac.lambda_lane_max),
        )

        _, collision_update = self._update_dual_lambda(
            "collision",
            self._lambda_collision,
            collision_signal,
            min_value=float(self.cfg.sac.min_lambda_collision),
        ...
        _, headway_update = self._update_dual_lambda(
            "headway",
            self._lambda_headway,
            headway_signal,
            min_value=float(self.cfg.sac.min_lambda_headway),
        ...
        _, overspeed_update = self._update_dual_lambda(
            "overspeed",
            self._lambda_overspeed,
            overspeed_signal,
            min_value=float(getattr(self.cfg.sac, "min_lambda_overspeed", 0.0)),
            max_value=float(self.cfg.sac.lambda_overspeed_max),
            scale=float(self.cfg.sac.lambda_overspeed_update_scale),
        )
        _, comfort_update = self._update_dual_lambda(
            "comfort",
            self._lambda_comfort,
            comfort_signal,
            min_value=0.0,
        ...
        _, lane_update = self._update_dual_lambda(
            "lane",
            self._lambda_lane,
            lane_signal,
            min_value=float(self.cfg.sac.min_lambda_lane),
            max_value=float(self.cfg.sac.lambda_lane_max),
            scale=float(self.cfg.sac.lambda_lane_update_scale),
        )

        lambda_loss = float(
            collision_signal + headway_signal + overspeed_signal + comfort_signal + lane_signal
        )
        ...
        self._soft_update(self.qc_lane, self.qc_lane_target)
        ...
        out = {
            ...
            "lambda_lane": float(self.lambda_lane.item()),
            ...
            "mean_cost_lane": float(cost_lane.mean().item()),
            ...
            "qc_lane_loss": float(qc_lane_loss.item()),
            "qc_collision": float(self._average_cost_q(qc_collision_new).mean().item()),
            "qc_headway": float(self._average_cost_q(qc_headway_new).mean().item()),
            "qc_overspeed": float(self._average_cost_q(qc_overspeed_new).mean().item()),
            "qc_comfort": float(self._average_cost_q(qc_comfort_new).mean().item()),
            "qc_lane": float(self._average_cost_q(qc_lane_new).mean().item()),
            "qc_collision_target": float(self._average_cost_q(qc_collision_target_mean).item()),
            "qc_headway_target": float(self._average_cost_q(qc_headway_target_mean).item()),
            "qc_overspeed_target": float(self._average_cost_q(qc_overspeed_target_mean).item()),
            "qc_comfort_target": float(self._average_cost_q(qc_comfort_target_mean).item()),
            "qc_lane_target": float(self._average_cost_q(qc_lane_target_mean).item()),
            ...
            "batch_cost_lane": float(batch_lane_mean.item()),
            "policy_cost_collision": float(policy_collision_cost.item()),
            "policy_cost_headway": float(policy_headway_cost.item()),
            "policy_cost_overspeed": float(policy_overspeed_cost.item()),
            "policy_cost_comfort": float(policy_comfort_cost.item()),
            "policy_cost_lane": float(policy_lane_cost.item()),
            ...
            "ema_lane_cost": float(self.ema_lane_cost if self.ema_lane_cost is not None else float("nan")),
            "dual_signal_collision": float(collision_signal),
            "dual_signal_headway": float(headway_signal),
            "dual_signal_overspeed": float(overspeed_signal),
            "dual_signal_comfort": float(comfort_signal),
            "dual_signal_lane": float(lane_signal),
            "dual_error_collision": float(collision_error),
            "dual_error_headway": float(headway_error),
            "dual_error_overspeed": float(overspeed_error),
            "dual_error_comfort": float(comfort_error),
            "dual_error_lane": float(lane_error),
            "dual_empirical_error_collision": float(collision_dual.get("empirical_error", float("nan"))),
            "dual_empirical_error_headway": float(headway_dual.get("empirical_error", float("nan"))),
            "dual_empirical_error_overspeed": float(overspeed_dual.get("empirical_error", float("nan"))),
            "dual_empirical_error_comfort": float(comfort_dual.get("empirical_error", float("nan"))),
            "dual_empirical_error_lane": float(lane_dual.get("empirical_error", float("nan"))),
            "dual_policy_error_collision": float(collision_dual.get("policy_violation", float("nan"))),
            "dual_policy_error_headway": float(headway_dual.get("policy_violation", float("nan"))),
            "dual_policy_error_overspeed": float(overspeed_dual.get("policy_violation", float("nan"))),
            "dual_policy_error_comfort": float(comfort_dual.get("policy_violation", float("nan"))),
            "dual_policy_error_lane": float(lane_dual.get("policy_violation", float("nan"))),
            "dual_integral_collision": float(self.dual_pid_state["collision"].get("integral", float("nan"))),
            "dual_integral_headway": float(self.dual_pid_state["headway"].get("integral", float("nan"))),
            "dual_integral_overspeed": float(self.dual_pid_state["overspeed"].get("integral", float("nan"))),
            "dual_integral_comfort": float(self.dual_pid_state["comfort"].get("integral", float("nan"))),
            "dual_integral_lane": float(self.dual_pid_state["lane"].get("integral", float("nan"))),
            "dual_integral_blocked_overspeed": float(overspeed_dual.get("integral_blocked", 0.0)),
            "dual_release_overspeed": float(overspeed_dual.get("release_active", 0.0)),
            "dual_release_count_overspeed": float(overspeed_dual.get("release_count", 0.0)),
            "dual_saturated_collision": float(collision_update.get("saturated", 0.0)),
            "dual_saturated_headway": float(headway_update.get("saturated", 0.0)),
            "dual_saturated_overspeed": float(overspeed_update.get("saturated", 0.0)),
            "dual_saturated_comfort": float(comfort_update.get("saturated", 0.0)),
            "dual_saturated_lane": float(lane_update.get("saturated", 0.0)),
            "dual_anti_windup_collision": float(collision_update.get("anti_windup", 0.0)),
            "dual_anti_windup_headway": float(headway_update.get("anti_windup", 0.0)),
            "dual_anti_windup_overspeed": float(overspeed_update.get("anti_windup", 0.0)),
            "dual_anti_windup_comfort": float(comfort_update.get("anti_windup", 0.0)),
            "dual_anti_windup_lane": float(lane_update.get("anti_windup", 0.0)),
...
ACTION_CHANGE_EPS = 1e-6


def _as_action_array(action) -> np.ndarray:
    return np.asarray(action, dtype=np.float32).reshape(-1)


def _mean_abs_action_delta(before, after) -> float:
    try:
        a = _as_action_array(before)
        b = _as_action_array(after)
        if a.shape != b.shape:
            return 0.0
...
def _action_changed(before, after, eps: float = ACTION_CHANGE_EPS) -> bool:
    return bool(_mean_abs_action_delta(before, after) > float(eps))
...
TRACE_SCENE_KEYS = [
    ...
    "lane_id",
    "lane_center_y",
    "signed_lane_offset_norm",
    "abs_lane_offset_norm",
    "heading",
    "heading_error",
    "lateral_speed",
    "front_distance",
    ...
    "right_lane_id",
    "left_front_distance",
    "left_front_rel_speed",
    "left_front_closing_speed",
    "left_front_ttc",
    "left_rear_distance",
    "left_rear_rel_speed",
    ...
    "right_front_rel_speed",
    "right_front_closing_speed",
    "right_front_ttc",
    "right_rear_distance",
    "right_rear_rel_speed",
    "right_rear_closing_speed",
    "right_rear_ttc",
    "road_boundary_margin",
    "road_boundary_proximity",
    "lane_boundary_proximity",
    "collision",
]


def _scene_trace_fields(scene: dict) -> dict:
    return {k: _jsonable_plain(scene.get(k)) for k in TRACE_SCENE_KEYS if k in scene}


def _to_trace_action(action) -> List[float]:
    try:
        return [float(x) for x in _as_action_array(action).tolist()]
    except Exception:
        return []


def build_train_step_trace_record(
    ep: int,
    step_idx: int,
    total_steps: int,
    scene: dict,
    next_scene: dict,
    raw_action,
    safe_filtered_action,
    risk_corrected_action,
    shielded_action,
    verify_info: Optional[dict],
    shield_info: Optional[dict],
    risk_correction_info: Optional[dict],
    reward: float,
    reward_terms: dict,
    cost_dict: dict,
    done: bool,
    ...
    return {
        ...
        "total_steps_before_step": int(total_steps),
        "replanned": bool(replanned),
        "done": bool(done),
        "scene": _scene_trace_fields(scene),
        "next_scene": _scene_trace_fields(next_scene),
        "actions": {
            "raw_action": _to_trace_action(raw_action),
            "safe_filtered_action": _to_trace_action(safe_filtered_action),
            "risk_corrected_action": _to_trace_action(risk_corrected_action),
            "shielded_action": _to_trace_action(shielded_action),
        },
        "action_deltas": {
            "safe_filter": _mean_abs_action_delta(raw_action, safe_filtered_action),
            "risk_correction": _mean_abs_action_delta(safe_filtered_action, risk_corrected_action),
            "shield": _mean_abs_action_delta(risk_corrected_action, shielded_action),
            "total": _mean_abs_action_delta(raw_action, shielded_action),
        },
        "verify_info": _jsonable_plain(verify_info or {}),
        "shield_info": _jsonable_plain(shield_info or {}),
        "risk_correction_info": _jsonable_plain(risk_correction_info or {}),
        "reward": float(reward),
        "reward_terms": _jsonable_plain(reward_terms),
        "cost_dict": _jsonable_plain(cost_dict),
    }


def resolve_failure_trace_dir(cfg) -> str:
    explicit = str(getattr(cfg.diagnostics, "failure_trace_dir", "") or "").strip()
    if explicit:
        return explicit
    train_json = str(getattr(cfg.diagnostics, "train_json_path", "") or "").strip()
    if train_json:
        root, _ = os.path.splitext(train_json)
        return root + "_failure_traces"
    return ""


def episode_needs_failure_trace(ep_summary: dict) -> bool:
    return bool(
        int(ep_summary.get("collision", 0)) != 0
        or float(ep_summary.get("offroad_rate", 0.0)) > 0.0
        or int(ep_summary.get("success", 0)) == 0
    )


def export_failure_episode_trace(trace_dir: str, ep: int, ep_summary: dict, step_trace: List[dict]) -> str:
    if not trace_dir or not episode_needs_failure_trace(ep_summary) or not step_trace:
        return ""
    os.makedirs(trace_dir, exist_ok=True)
    filename = (
        f"episode_{int(ep):04d}"
        f"_success{int(ep_summary.get('success', 0))}"
        f"_collision{int(ep_summary.get('collision', 0))}"
        f"_offroad{float(ep_summary.get('offroad_rate', 0.0)):.3f}.jsonl"
    )
    ...
    header = {
        ...
        "episode": int(ep),
        "summary": _jsonable_plain(ep_summary),
    }
    with open(path, "w", encoding="utf-8") as f:
        f.write(json.dumps(header, ensure_ascii=False) + "\n")
        for record in step_trace:
            f.write(json.dumps(_jsonable_plain(record), ensure_ascii=False) + "\n")
    print(f"[TRACE] saved failure episode trace to {path}")
...
def _startup_debug_log_enabled(cfg) -> bool:
    return bool(getattr(cfg.eval, "startup_debug", False))


def _startup_debug_steps(cfg) -> int:
    return max(0, int(getattr(cfg.eval, "startup_debug_steps", 0)))
...
def _startup_debug_print(prefix: str, record: dict) -> None:
    print(
        f"[{prefix}-STARTUP][ep={record['episode_idx']}][t={record['step_idx']}] "
        f"lane={record['lane_id']} "
        f"ego_y={record['ego_y']:.2f} "
        f"lane_center_y={record['lane_center_y']:.2f} "
        f"lane_offset={record['lane_offset']:.3f} "
        f"heading={record['heading']:.3f} "
        f"behavior={record['behavior']} "
        f"target_lane={record['target_lane']} "
        f"target_speed={record['target_speed']:.2f} "
        f"planner_state={record['planner_state']} "
        f"events={record['trigger_events']} "
        f"true_raw={record.get('true_raw_action', record.get('raw_action', []))} "
        f"wrapped_raw={record.get('wrapped_raw_action', record.get('recovery_action', []))} "
        f"raw_before={record['raw_action_before_startup_alignment']} "
        f"aligned={record['startup_alignment_action']} "
        f"raw_after={record['raw_action_after_startup_alignment']} "
        f"align_active={int(record['startup_alignment_active'])} "
        f"align_mode={record['startup_alignment_mode']} "
        f"align_blend={record['startup_alignment_blend']:.2f} "
        f"align_reason={record['startup_alignment_reason']} "
        f"recovery={record['recovery_action']} "
        f"shielded={record['shielded_action']}"
    )


def _append_startup_debug_record(cfg, record: dict, prefix: str) -> None:
    if not _startup_debug_log_enabled(cfg):
        return
    _startup_debug_print(prefix, record)
    log_path = str(getattr(cfg.eval, "startup_debug_log_path", "") or "").strip()
    if not log_path:
        return
...
class EpisodeMetrics:
    def __init__(self, cfg=None):
        ...
        self.smooth_steer = []
        self.lane_offset = []
        self.road_boundary_margin = []
        self.heading_error_abs = []
        self.lateral_speed_abs = []
        self.road_boundary_proximity = []
        self.lane_boundary_proximity = []
        self.lane_changes = 0
        self.replans = 0
        self.collided = False
        ...
        self.safe_exploration_filter_count = 0
        self.risk_correction_count = 0
        self.shield_modified_count = 0
        self.train_action_modified_count = 0
        self.delta_safe_filter = []
        self.delta_risk_correction = []
        self.delta_shield = []
        self.delta_total = []

        self.cost_collision = []
        self.cost_collision_front = []
        self.cost_collision_rear = []
        self.cost_collision_lateral = []
        self.cost_headway = []
        self.cost_overspeed = []
        self.cost_comfort = []
        self.cost_lane = []
    ...
    def update(self, scene: dict, next_scene: dict, action, reward: float, replanned: bool,
               ...
               verify_info=None, shield_info=None, cost_dict=None, action_trace_info=None):
        action = np.asarray(action, dtype=np.float32)
        last_action = np.asarray(scene.get("last_action", np.zeros_like(action)), dtype=np.float32)
        ...
        self.front_closing_speeds.append(float(next_scene.get("front_closing_speed", max(0.0, -float(next_scene.get("front_rel_speed", 0.0))))))
        ...
        self.lane_offset.append(float(abs(next_scene.get("signed_lane_offset_norm", next_scene.get("abs_lane_offset_norm", 0.0)))))
        self.road_boundary_margin.append(float(next_scene.get("road_boundary_margin", 0.0)))
        self.heading_error_abs.append(float(abs(next_scene.get("heading_error", next_scene.get("heading", 0.0)))))
        self.lateral_speed_abs.append(float(abs(next_scene.get("lateral_speed", 0.0))))
        self.road_boundary_proximity.append(float(next_scene.get("road_boundary_proximity", 0.0)))
        self.lane_boundary_proximity.append(float(next_scene.get("lane_boundary_proximity", 0.0)))
        ...
        if next_scene["collision"]:
            self.collided = True
        offroad_now = _rl_is_offroad_scene(self.cfg, next_scene) if self.cfg is not None else bool(float(next_scene.get("road_boundary_margin", 1e6)) < 0.05)
        if offroad_now:
        ...
        if shield_info is not None:
            ...
            self.shield_modified_count += int(shield_info.get("shield_modified", False))

        if action_trace_info is not None:
            raw_action = action_trace_info.get("raw_action", action)
            safe_filtered_action = action_trace_info.get("safe_filtered_action", raw_action)
            risk_corrected_action = action_trace_info.get("risk_corrected_action", safe_filtered_action)
            shielded_action = action_trace_info.get("shielded_action", action)

            safe_delta = _mean_abs_action_delta(raw_action, safe_filtered_action)
            risk_delta = _mean_abs_action_delta(safe_filtered_action, risk_corrected_action)
            shield_delta = _mean_abs_action_delta(risk_corrected_action, shielded_action)
            total_delta = _mean_abs_action_delta(raw_action, shielded_action)

            self.delta_safe_filter.append(float(safe_delta))
            self.delta_risk_correction.append(float(risk_delta))
            self.delta_shield.append(float(shield_delta))
            self.delta_total.append(float(total_delta))

            self.safe_exploration_filter_count += int(
                bool(action_trace_info.get("safe_exploration_filter_used", False)) and safe_delta > ACTION_CHANGE_EPS
            )
            self.risk_correction_count += int(
                bool(action_trace_info.get("risk_correction_active", False)) or risk_delta > ACTION_CHANGE_EPS
            )
            self.train_action_modified_count += int(total_delta > ACTION_CHANGE_EPS)
        ...
        if cost_dict is not None:
            ...
            self.cost_boundary.append(float(cost_dict.get("boundary_cost", cost_dict.get("lane_cost", 0.0))))
    ...
    def summary(self) -> dict:
        ...
        return {
            ...
            "success": int(not self.collided and not self.failed_offroad),
            "mean_env_reward": safe_mean(self.env_rewards),
            ...
            "mean_front_closing_speed": safe_mean(self.front_closing_speeds),
            ...
            "mean_abs_lane_offset": safe_mean(self.lane_offset),
            "max_abs_lane_offset": float(np.max(self.lane_offset)) if len(self.lane_offset) > 0 else 0.0,
            "min_road_boundary_margin": safe_min(self.road_boundary_margin),
            "mean_abs_heading_error": safe_mean(self.heading_error_abs),
            "max_abs_heading_error": float(np.max(self.heading_error_abs)) if len(self.heading_error_abs) > 0 else 0.0,
            "mean_abs_lateral_speed": safe_mean(self.lateral_speed_abs),
            "mean_road_boundary_proximity": safe_mean(self.road_boundary_proximity),
            "mean_lane_boundary_proximity": safe_mean(self.lane_boundary_proximity),
            "lane_changes": int(self.lane_changes),
            "replans": int(self.replans),
            "interventions": int(self.interventions),
            "safe_exploration_filter_rate": float(self.safe_exploration_filter_count / max(self.steps, 1)),
            "risk_correction_rate": float(self.risk_correction_count / max(self.steps, 1)),
            "shield_rate": float(self.shield_modified_count / max(self.steps, 1)),
            "train_action_modified_rate": float(self.train_action_modified_count / max(self.steps, 1)),
            "mean_abs_delta_safe_filter": safe_mean(self.delta_safe_filter),
            "mean_abs_delta_risk_correction": safe_mean(self.delta_risk_correction),
            "mean_abs_delta_shield": safe_mean(self.delta_shield),
            "mean_abs_delta_total": safe_mean(self.delta_total),
            ...
            "collision_cost_front": safe_mean(self.cost_collision_front),
            "collision_cost_rear": safe_mean(self.cost_collision_rear),
            "collision_cost_lateral": safe_mean(self.cost_collision_lateral),
            ...
            "mean_cost_lane": safe_mean(self.cost_lane),
            "boundary_cost": safe_mean(self.cost_boundary),
...
def summarize_eval_results(results: list[dict]) -> dict:
    ...
    keys = [
        ...
        "max_abs_lane_offset",
        "min_road_boundary_margin",
        "offroad_rate",
        "lane_changes",
        "replans",
        "interventions",
        "safe_exploration_filter_rate",
        "risk_correction_rate",
        "shield_rate",
        "train_action_modified_rate",
        "mean_abs_delta_safe_filter",
        "mean_abs_delta_risk_correction",
        "mean_abs_delta_shield",
...
def _evaluate_policy_once(cfg, agent, mode: str, use_shield_eval: bool, prefix: str):
    ...
    eval_results = []
    videos = []
    report_tag = "shielded" if bool(use_shield_eval) else "raw"

    for ep in range(cfg.eval.episodes):
        eval_seed = cfg.train.seed + cfg.eval.seed_offset + ep
        eval_env, video_meta = _make_eval_env(cfg, mode=mode, report_tag=report_tag, seed=eval_seed, episode_idx=ep)
        try:
            state, _ = eval_env.reset(seed=eval_seed)
            planner = build_planner(cfg, memory_readonly=True)

            metrics = EpisodeMetrics(cfg)
            latest_plan_info: Dict[str, Any] = {}
            latest_waypoint0: List[float] = []
            ...
            if planner is not None:
                ...
                latest_plan_info = copy.deepcopy(plan.get("planner_info", {}))
                latest_waypoint0 = _to_float_list(plan["waypoints"][0]) if len(plan.get("waypoints", [])) > 0 else []
            ...
            for t in range(cfg.train.max_steps_per_episode):
                ...
                true_raw_action = agent.select_action(state, evaluate=True).astype(np.float32)
                aligned_action, alignment_info = apply_startup_alignment_override(cfg, scene, true_raw_action, constraints, latest_plan_info, t)
                recovery_action, recovery_info = apply_local_recovery_override(cfg, scene, aligned_action, constraints)
                wrapped_raw_action = recovery_action.astype(np.float32)

                verify_info = verifier.check(scene, wrapped_raw_action, constraints) if verifier is not None else {}
                shield_info = {
                    "shield_modified": False,
                    "shield_reasons": [],
                    "startup_alignment_active": bool(alignment_info.get("startup_alignment_active", False)),
                    "startup_alignment_reason": str(alignment_info.get("startup_alignment_reason", "inactive")),
                    "recovery_active": bool(recovery_info.get("recovery_active", False)),
                    "recovery_reason": str(recovery_info.get("recovery_reason", "none")),
                }
                action = wrapped_raw_action

                if shield is not None:
                    action, shield_info_core = shield.apply(scene, wrapped_raw_action, constraints, verify_info)
                    shield_info.update(shield_info_core)

                next_state, env_reward, terminated, truncated, _ = eval_env.step(action)
                next_scene = eval_env.get_scene_dict()
                done = terminated or truncated or _rl_terminal_failure(cfg, next_scene)
                ...
                if _startup_debug_log_enabled(cfg) and t < _startup_debug_steps(cfg):
                    lane_offset = float(scene["ego_y"] - scene["lane_center_y"])
                    next_lane_offset = float(next_scene["ego_y"] - next_scene["lane_center_y"])
                    startup_record = {
                        "timestamp_utc": _utc_now_iso(),
                        "mode": str(mode),
                        "prefix": str(prefix),
                        ...
                        "replanned": bool(replanned),
                        "lane_id": int(scene.get("lane_id", 0)),
                        "ego_y": float(scene.get("ego_y", 0.0)),
                        "lane_center_y": float(scene.get("lane_center_y", 0.0)),
                        "lane_offset": float(lane_offset),
                        "heading": float(scene.get("heading", 0.0)),
                        "front_distance": float(scene.get("front_distance", 1e6)),
                        "ttc": float(scene.get("ttc", 99.0)),
                        "target_lane": int(latest_plan_info.get("target_lane", scene.get("lane_id", 0))),
                        "target_speed": float(latest_plan_info.get("target_speed", scene.get("ego_speed", 0.0))),
                        "behavior": str(latest_plan_info.get("behavior", "none")),
                        "planner_state": str(latest_plan_info.get("planner_state", "INIT")),
                        "trigger_events": list(latest_plan_info.get("events", [])),
                        "true_raw_action": _to_float_list(true_raw_action),
                        "wrapped_raw_action": _to_float_list(wrapped_raw_action),
                        "raw_action": _to_float_list(true_raw_action),
                        "raw_action_before_startup_alignment": _to_float_list(alignment_info.get("raw_action_before_startup_alignment", true_raw_action)),
                        "startup_alignment_action": _to_float_list(aligned_action),
                        "raw_action_after_startup_alignment": _to_float_list(alignment_info.get("raw_action_after_startup_alignment", aligned_action)),
                        "recovery_action": _to_float_list(recovery_action),
                        "shielded_action": _to_float_list(action),
                        "startup_alignment_active": bool(alignment_info.get("startup_alignment_active", False)),
                        "startup_alignment_reason": str(alignment_info.get("startup_alignment_reason", "inactive")),
                        "startup_alignment_mode": str(alignment_info.get("startup_alignment_mode", "inactive")),
                        "startup_alignment_blend": float(alignment_info.get("startup_alignment_blend", 0.0)),
                        "recovery_active": bool(recovery_info.get("recovery_active", False)),
                        "recovery_reason": str(recovery_info.get("recovery_reason", "none")),
                        "shield_modified": bool(shield_info.get("shield_modified", False)),
                        "shield_reasons": list(shield_info.get("shield_reasons", [])),
                        "waypoint0": list(latest_waypoint0),
                        "constraint_min_headway": float(constraints.get("min_headway", 0.0)),
                        "constraint_speed_max": float(constraints.get("speed_max", 0.0)),
                        "next_lane_id": int(next_scene.get("lane_id", 0)),
                        "next_ego_y": float(next_scene.get("ego_y", 0.0)),
                        "next_lane_center_y": float(next_scene.get("lane_center_y", 0.0)),
                        "next_lane_offset": float(next_lane_offset),
                        "next_heading": float(next_scene.get("heading", 0.0)),
                        "next_front_distance": float(next_scene.get("front_distance", 1e6)),
                        "next_ttc": float(next_scene.get("ttc", 99.0)),
                    }
                    _append_startup_debug_record(cfg, startup_record, prefix=prefix)
                ...
                if cfg.eval.print_step:
                    print(
                        ...
                        f"raw_action=[{true_raw_action[0]:.2f},{true_raw_action[1]:.2f}] "
            ...
            eval_env.close()
            if video_meta.get("enabled", False):
                video_record = _finalize_eval_video_metadata(video_meta)
                if video_record is not None:
                    videos.append(video_record)
                print(f"[EVAL-VIDEO] saved to {video_meta.get('path_hint', '')}")
    ...
    return {"episodes": eval_results, "summary": summary, "use_shield": bool(use_shield_eval), "videos": videos}
...
def maybe_print_update(mode: str, update_info: dict):
    ...
    if mode_is_constrained(mode):
        msg += (
            ...
            f" lambda_lane={update_info.get('lambda_lane', 0.0):.4f}"
        )
        if 'qc_collision' in update_info:
            msg += (
                ...
                f" qc_lane={update_info.get('qc_lane', 0.0):.4f}"
            )
        if 'dual_signal_overspeed' in update_info:
            msg += (
                f" dual_over_sig={update_info.get('dual_signal_overspeed', 0.0):.3f}"
                f" dual_over_err={update_info.get('dual_error_overspeed', 0.0):.3f}"
                f" over_release={update_info.get('dual_release_overspeed', 0.0):.0f}"
            )
...
TRAIN_DIAGNOSTIC_FIELDS = [
    ...
    "ema_comfort_cost",
    "ema_lane_cost",
    "batch_cost_overspeed",
    "policy_cost_overspeed",
    "dual_empirical_error_overspeed",
    "dual_policy_error_overspeed",
    "dual_error_overspeed",
    "dual_signal_overspeed",
    "dual_integral_overspeed",
    "dual_integral_blocked_overspeed",
    "dual_release_overspeed",
    "dual_release_count_overspeed",
    "dual_saturated_overspeed",
    "dual_anti_windup_overspeed",
]
...
def build_train_diagnostic_row(ep: int, mode: str, ep_summary: dict, agent, last_update_info: dict) -> dict:
    row = {
        ...
        "safe_exploration_filter_rate": float(ep_summary.get("safe_exploration_filter_rate", 0.0)),
        "risk_correction_rate": float(ep_summary.get("risk_correction_rate", 0.0)),
        "shield_rate": float(ep_summary.get("shield_rate", 0.0)),
        "train_action_modified_rate": float(ep_summary.get("train_action_modified_rate", 0.0)),
        "mean_abs_delta_safe_filter": float(ep_summary.get("mean_abs_delta_safe_filter", 0.0)),
        "mean_abs_delta_risk_correction": float(ep_summary.get("mean_abs_delta_risk_correction", 0.0)),
        "mean_abs_delta_shield": float(ep_summary.get("mean_abs_delta_shield", 0.0)),
        "mean_abs_delta_total": float(ep_summary.get("mean_abs_delta_total", 0.0)),
        ...
        "ema_overspeed_cost": _diag_nan(),
        "ema_comfort_cost": _diag_nan(),
        "ema_lane_cost": _diag_nan(),
        "batch_cost_overspeed": _diag_nan(),
        "policy_cost_overspeed": _diag_nan(),
        "dual_empirical_error_overspeed": _diag_nan(),
        "dual_policy_error_overspeed": _diag_nan(),
        "dual_error_overspeed": _diag_nan(),
        "dual_signal_overspeed": _diag_nan(),
        "dual_integral_overspeed": _diag_nan(),
        "dual_integral_blocked_overspeed": _diag_nan(),
        "dual_release_overspeed": _diag_nan(),
        "dual_release_count_overspeed": _diag_nan(),
        "dual_saturated_overspeed": _diag_nan(),
        "dual_anti_windup_overspeed": _diag_nan(),
    }
    ...
    if mode_is_constrained(mode):
        ...
        row["lambda_lane"] = float(last_update_info.get("lambda_lane", float(getattr(agent, "lambda_lane", torch.tensor(float("nan"))).item())))
        ...
        row["qc_lane"] = float(last_update_info.get("qc_lane", _diag_nan()))
        ...
        row["ema_overspeed_cost"] = float(last_update_info.get("ema_overspeed_cost", _diag_nan()))
        row["ema_comfort_cost"] = float(last_update_info.get("ema_comfort_cost", _diag_nan()))
        row["ema_lane_cost"] = float(last_update_info.get("ema_lane_cost", _diag_nan()))
        row["batch_cost_overspeed"] = float(last_update_info.get("batch_cost_overspeed", _diag_nan()))
        row["policy_cost_overspeed"] = float(last_update_info.get("policy_cost_overspeed", _diag_nan()))
        row["dual_empirical_error_overspeed"] = float(last_update_info.get("dual_empirical_error_overspeed", _diag_nan()))
        row["dual_policy_error_overspeed"] = float(last_update_info.get("dual_policy_error_overspeed", _diag_nan()))
        row["dual_error_overspeed"] = float(last_update_info.get("dual_error_overspeed", _diag_nan()))
        row["dual_signal_overspeed"] = float(last_update_info.get("dual_signal_overspeed", _diag_nan()))
        row["dual_integral_overspeed"] = float(last_update_info.get("dual_integral_overspeed", _diag_nan()))
        row["dual_integral_blocked_overspeed"] = float(last_update_info.get("dual_integral_blocked_overspeed", _diag_nan()))
        row["dual_release_overspeed"] = float(last_update_info.get("dual_release_overspeed", _diag_nan()))
        row["dual_release_count_overspeed"] = float(last_update_info.get("dual_release_count_overspeed", _diag_nan()))
        row["dual_saturated_overspeed"] = float(last_update_info.get("dual_saturated_overspeed", _diag_nan()))
        row["dual_anti_windup_overspeed"] = float(last_update_info.get("dual_anti_windup_overspeed", _diag_nan()))
    return row


ENV_METRIC_EXPORT_FIELDS = [
    "episode",
    "steps",
    "episode_env_return",
    "mean_env_reward",
    "success_without_collision_or_offroad",
    "collision_flag",
    "offroad_rate",
    "mean_speed",
    "mean_front_distance",
    "min_front_distance_episode",
    "mean_front_closing_speed",
    "mean_ttc",
    "min_ttc_episode",
    "lane_changes",
    "mean_lane_offset_norm",
    "max_lane_offset_norm_episode",
    "min_road_boundary_margin",
    "mean_abs_heading_error_rad",
    "max_abs_heading_error_rad",
    "mean_abs_lateral_speed",
    "mean_road_boundary_proximity",
    "mean_lane_boundary_proximity",
    "unsafe_headway_rate",
    "overspeed_rate",
    "low_ttc_rate",
]
...
ALGO_METRIC_EXPORT_FIELDS = [
    "episode",
    "steps",
    "shaped_episode_return",
    "mean_abs_action_acc_norm",
    "mean_abs_action_steer_norm",
    "mean_smooth_action_acc_norm",
    "mean_smooth_action_steer_norm",
    "planner_replans",
...
def build_environment_metric_row(episode_idx: int, ep_summary: dict) -> dict:
    return {
        "episode": int(episode_idx),
        "steps": int(ep_summary.get("steps", 0)),
        "episode_env_return": float(ep_summary.get("episode_env_return", 0.0)),
        "mean_env_reward": float(ep_summary.get("mean_env_reward", 0.0)),
        "success_without_collision_or_offroad": float(ep_summary.get("success", 0.0)),
        "collision_flag": float(ep_summary.get("collision", 0.0)),
        "offroad_rate": float(ep_summary.get("offroad_rate", 0.0)),
        "mean_speed": float(ep_summary.get("mean_speed", 0.0)),
        "mean_front_distance": float(ep_summary.get("mean_front_distance", 0.0)),
        "min_front_distance_episode": float(ep_summary.get("min_front_distance", 0.0)),
        "mean_front_closing_speed": float(ep_summary.get("mean_front_closing_speed", 0.0)),
        "mean_ttc": float(ep_summary.get("mean_ttc", 0.0)),
        "min_ttc_episode": float(ep_summary.get("min_ttc", 0.0)),
        "lane_changes": float(ep_summary.get("lane_changes", 0.0)),
        "mean_lane_offset_norm": float(ep_summary.get("mean_abs_lane_offset", 0.0)),
        "max_lane_offset_norm_episode": float(ep_summary.get("max_abs_lane_offset", 0.0)),
        "min_road_boundary_margin": float(ep_summary.get("min_road_boundary_margin", 0.0)),
        "mean_abs_heading_error_rad": float(ep_summary.get("mean_abs_heading_error", 0.0)),
        "max_abs_heading_error_rad": float(ep_summary.get("max_abs_heading_error", 0.0)),
        "mean_abs_lateral_speed": float(ep_summary.get("mean_abs_lateral_speed", 0.0)),
        "mean_road_boundary_proximity": float(ep_summary.get("mean_road_boundary_proximity", 0.0)),
        "mean_lane_boundary_proximity": float(ep_summary.get("mean_lane_boundary_proximity", 0.0)),
        "unsafe_headway_rate": float(ep_summary.get("unsafe_headway_rate", 0.0)),
        "overspeed_rate": float(ep_summary.get("overspeed_rate", 0.0)),
        "low_ttc_rate": float(ep_summary.get("low_ttc_rate", 0.0)),
    }
...
def build_algorithm_metric_row(episode_idx: int, ep_summary: dict, train_diag_row: Optional[dict] = None) -> dict:
    train_diag_row = dict(train_diag_row or {})
    return {
        "episode": int(episode_idx),
        "steps": int(ep_summary.get("steps", 0)),
        "shaped_episode_return": float(ep_summary.get("episode_return", 0.0)),
        "mean_abs_action_acc_norm": float(ep_summary.get("mean_abs_acc", 0.0)),
        "mean_abs_action_steer_norm": float(ep_summary.get("mean_abs_steer", 0.0)),
        "mean_smooth_action_acc_norm": float(ep_summary.get("mean_smooth_acc", 0.0)),
        "mean_smooth_action_steer_norm": float(ep_summary.get("mean_smooth_steer", 0.0)),
        "planner_replans": float(ep_summary.get("replans", 0.0)),
        "shield_interventions": float(ep_summary.get("interventions", 0.0)),
        "safe_exploration_filter_rate": float(ep_summary.get("safe_exploration_filter_rate", 0.0)),
        "risk_correction_rate": float(ep_summary.get("risk_correction_rate", 0.0)),
        "shield_rate": float(ep_summary.get("shield_rate", 0.0)),
        "train_action_modified_rate": float(ep_summary.get("train_action_modified_rate", 0.0)),
        ...
        "boundary_cost": float(ep_summary.get("boundary_cost", 0.0)),
        "mean_cost_total": float(ep_summary.get("mean_cost_total", 0.0)),
        "lambda_collision": float(train_diag_row.get("lambda_collision", _diag_nan())),
        "lambda_headway": float(train_diag_row.get("lambda_headway", _diag_nan())),
        "lambda_overspeed": float(train_diag_row.get("lambda_overspeed", _diag_nan())),
        "lambda_comfort": float(train_diag_row.get("lambda_comfort", _diag_nan())),
        "lambda_lane": float(train_diag_row.get("lambda_lane", _diag_nan())),
        "qc_collision": float(train_diag_row.get("qc_collision", _diag_nan())),
        "qc_headway": float(train_diag_row.get("qc_headway", _diag_nan())),
        "qc_overspeed": float(train_diag_row.get("qc_overspeed", _diag_nan())),
        "qc_comfort": float(train_diag_row.get("qc_comfort", _diag_nan())),
        "qc_lane": float(train_diag_row.get("qc_lane", _diag_nan())),
        "ema_collision_cost": float(train_diag_row.get("ema_collision_cost", _diag_nan())),
        "ema_headway_cost": float(train_diag_row.get("ema_headway_cost", _diag_nan())),
        "ema_overspeed_cost": float(train_diag_row.get("ema_overspeed_cost", _diag_nan())),
        "ema_comfort_cost": float(train_diag_row.get("ema_comfort_cost", _diag_nan())),
        "ema_lane_cost": float(train_diag_row.get("ema_lane_cost", _diag_nan())),
        "batch_cost_overspeed": float(train_diag_row.get("batch_cost_overspeed", _diag_nan())),
        "policy_cost_overspeed": float(train_diag_row.get("policy_cost_overspeed", _diag_nan())),
        "dual_empirical_error_overspeed": float(train_diag_row.get("dual_empirical_error_overspeed", _diag_nan())),
        "dual_policy_error_overspeed": float(train_diag_row.get("dual_policy_error_overspeed", _diag_nan())),
        "dual_error_overspeed": float(train_diag_row.get("dual_error_overspeed", _diag_nan())),
        "dual_signal_overspeed": float(train_diag_row.get("dual_signal_overspeed", _diag_nan())),
        "dual_integral_overspeed": float(train_diag_row.get("dual_integral_overspeed", _diag_nan())),
        "dual_integral_blocked_overspeed": float(train_diag_row.get("dual_integral_blocked_overspeed", _diag_nan())),
        "dual_release_overspeed": float(train_diag_row.get("dual_release_overspeed", _diag_nan())),
        "dual_release_count_overspeed": float(train_diag_row.get("dual_release_count_overspeed", _diag_nan())),
        "dual_saturated_overspeed": float(train_diag_row.get("dual_saturated_overspeed", _diag_nan())),
        "dual_anti_windup_overspeed": float(train_diag_row.get("dual_anti_windup_overspeed", _diag_nan())),
    }


def split_episode_metric_rows(episode_rows: List[dict], train_diag_rows: Optional[List[dict]] = None) -> tuple[List[dict], List[dict]]:
    env_rows: List[dict] = []
    algo_rows: List[dict] = []
    diag_rows = list(train_diag_rows or [])
    for idx, ep_summary in enumerate(episode_rows):
        diag_row = diag_rows[idx] if idx < len(diag_rows) else None
        env_rows.append(build_environment_metric_row(idx, ep_summary))
        algo_rows.append(build_algorithm_metric_row(idx, ep_summary, train_diag_row=diag_row))
    return env_rows, algo_rows


def summarize_metric_rows(rows: List[dict], skip_keys: Optional[List[str]] = None) -> dict:
    if len(rows) == 0:
        return {}
    skip = set(skip_keys or ["episode"])
    ...
    for key in rows[0].keys():
        ...
        if len(values) == 0:
            continue
        summary[key] = float(np.nanmean(np.asarray(values, dtype=np.float64)))
    return summary


def build_metric_export_notes() -> dict:
    return {
        "environment_metrics": {
            "episode_env_return": "Direct sum of env.step reward before any script-side reward shaping.",
            "mean_speed": "Environment vehicle speed in the simulator's native speed unit.",
            "mean_front_distance": "Front gap derived from scene geometry along the road direction.",
            "mean_ttc": "Time-to-collision derived from front gap and closing speed.",
            "mean_lane_offset_norm": "Absolute lane offset normalized by runtime lane width when available.",
            "mean_abs_heading_error_rad": "Absolute heading error relative to runtime lane heading when available.",
        },
        "algorithm_metrics": {
            "shaped_episode_return": "Script-defined reward sum used by the agent; not the raw environment return.",
            "mean_abs_action_acc_norm": "Absolute longitudinal action in normalized action space.",
            "mean_abs_action_steer_norm": "Absolute steering action in normalized action space.",
            "lambda_*": "Constraint dual variables from constrained SAC.",
            "qc_*": "Constraint critic estimates from constrained SAC.",
            "ema_*": "EMA-smoothed batch costs used by the dual update.",
            "dual_*_overspeed": "Overspeed dual diagnostics: mixed empirical/policy error, PID signal, anti-windup, and release state.",
        },
    }


def _derive_companion_export_path(primary_path: str, suffix: str, ext_override: Optional[str] = None) -> str:
    if not primary_path:
        return ""
...
def export_metric_rows_csv(rows: List[dict], save_path: str, fieldnames: List[str], label: str) -> str:
    if not save_path:
        return ""
    os.makedirs(os.path.dirname(save_path) or ".", exist_ok=True)
    ...
    print(f"[DIAG] saved {label} csv to {save_path}")
    return os.path.abspath(save_path)


def export_metric_rows_json(rows: List[dict], save_path: str, label: str) -> str:
    if not save_path:
        return ""
    os.makedirs(os.path.dirname(save_path) or ".", exist_ok=True)
    with open(save_path, "w", encoding="utf-8") as f:
        json.dump(rows, f, ensure_ascii=False, indent=2)
    print(f"[DIAG] saved {label} json to {save_path}")
    return os.path.abspath(save_path)
...
def resolve_run_result_json_path(cfg) -> str:
    explicit = str(getattr(cfg.diagnostics, "run_result_json_path", "") or "").strip()
    if explicit:
        return explicit
    for candidate in [
        str(getattr(cfg.diagnostics, "train_json_path", "") or "").strip(),
        str(getattr(cfg.diagnostics, "train_log_path", "") or "").strip(),
        str(getattr(cfg.diagnostics, "train_plot_path", "") or "").strip(),
    ]:
        if candidate:
            root, _ = os.path.splitext(candidate)
            return root + "_run_result.json"
    return ""


def export_run_result_json(payload: dict, save_path: str) -> str:
    if not save_path:
        return ""
    os.makedirs(os.path.dirname(save_path) or ".", exist_ok=True)
    ...
    with open(save_path, "w", encoding="utf-8") as f:
        json.dump(payload, f, ensure_ascii=False, indent=2)
    print(f"[DIAG] saved run result json to {save_path}")
    return os.path.abspath(save_path)
...
def _build_run_timestamp_prefix() -> str:
    return datetime.now().strftime('%Y%m%d_%H%M%S')


def _get_run_timestamp_prefix(cfg) -> str:
    prefix = str(getattr(cfg, "_run_timestamp_prefix", "") or "").strip()
    if not prefix:
        prefix = _build_run_timestamp_prefix()
        setattr(cfg, "_run_timestamp_prefix", prefix)
    return prefix


def _apply_prefix_to_path(path_value: str, prefix: str) -> str:
    ...
    return os.path.join(parent, prefixed_name) if parent else prefixed_name


def _apply_prefix_to_name(name_value: str, prefix: str, allow_empty_prefix_value: bool = False) -> str:
    name_text = str(name_value or "").strip()
    if not name_text:
        return prefix if allow_empty_prefix_value else ""
    safe_name = _sanitize_video_component(name_text)
    if not safe_name:
        return prefix if allow_empty_prefix_value else ""
    if safe_name.startswith(prefix + "_"):
        return safe_name
    return f"{prefix}_{safe_name}"


def apply_timestamp_prefix_to_output_paths(cfg) -> str:
    prefix = _get_run_timestamp_prefix(cfg)

    cfg.diagnostics.train_log_path = _apply_prefix_to_path(cfg.diagnostics.train_log_path, prefix)
    cfg.diagnostics.train_json_path = _apply_prefix_to_path(cfg.diagnostics.train_json_path, prefix)
    cfg.diagnostics.train_plot_path = _apply_prefix_to_path(cfg.diagnostics.train_plot_path, prefix)
    cfg.diagnostics.run_result_json_path = _apply_prefix_to_path(cfg.diagnostics.run_result_json_path, prefix)
    cfg.diagnostics.failure_trace_dir = _apply_prefix_to_path(cfg.diagnostics.failure_trace_dir, prefix)
    cfg.diagnostics.tensorboard_log_dir = _apply_prefix_to_path(cfg.diagnostics.tensorboard_log_dir, prefix)

    cfg.eval.video_dir = _apply_prefix_to_path(cfg.eval.video_dir, prefix)
    cfg.eval.video_prefix = _apply_prefix_to_name(cfg.eval.video_prefix, prefix, allow_empty_prefix_value=True)
    cfg.eval.startup_debug_log_path = _apply_prefix_to_path(cfg.eval.startup_debug_log_path, prefix)

    cfg.llm.call_log_path = _apply_prefix_to_path(cfg.llm.call_log_path, prefix)

    cfg.compare.save_json_path = _apply_prefix_to_path(cfg.compare.save_json_path, prefix)
    cfg.compare.save_csv_path = _apply_prefix_to_path(cfg.compare.save_csv_path, prefix)
    cfg.compare.save_latex_path = _apply_prefix_to_path(cfg.compare.save_latex_path, prefix)

    if str(cfg.workflow.stage).lower() == "freeze":
        cfg.workflow.freeze_save_path = _apply_prefix_to_path(cfg.workflow.freeze_save_path, prefix)

    print(f"[OUTPUT-PREFIX] using timestamp prefix: {prefix}")
    return prefix
...
def write_train_diagnostics_tensorboard(writer, row: dict, mode: str) -> None:
    ...
    writer.add_scalar('train_episode/offroad_rate', float(row.get('offroad_rate', 0.0)), ep)
    writer.add_scalar('train_episode/unsafe_headway_rate', float(row.get('unsafe_headway_rate', 0.0)), ep)
    writer.add_scalar('train_episode/overspeed_rate', float(row.get('overspeed_rate', 0.0)), ep)
    writer.add_scalar('train_episode/low_ttc_rate', float(row.get('low_ttc_rate', 0.0)), ep)
    writer.add_scalar('action_instrumentation/safe_exploration_filter_rate', float(row.get('safe_exploration_filter_rate', 0.0)), ep)
    writer.add_scalar('action_instrumentation/risk_correction_rate', float(row.get('risk_correction_rate', 0.0)), ep)
    writer.add_scalar('action_instrumentation/shield_rate', float(row.get('shield_rate', 0.0)), ep)
    writer.add_scalar('action_instrumentation/train_action_modified_rate', float(row.get('train_action_modified_rate', 0.0)), ep)
    writer.add_scalar('action_instrumentation/mean_abs_delta_safe_filter', float(row.get('mean_abs_delta_safe_filter', 0.0)), ep)
    writer.add_scalar('action_instrumentation/mean_abs_delta_risk_correction', float(row.get('mean_abs_delta_risk_correction', 0.0)), ep)
    writer.add_scalar('action_instrumentation/mean_abs_delta_shield', float(row.get('mean_abs_delta_shield', 0.0)), ep)
    writer.add_scalar('action_instrumentation/mean_abs_delta_total', float(row.get('mean_abs_delta_total', 0.0)), ep)
    writer.add_scalar('train_episode/mean_cost_collision', float(row.get('mean_cost_collision', 0.0)), ep)
    writer.add_scalar('train_episode/collision_cost_front', float(row.get('collision_cost_front', 0.0)), ep)
    writer.add_scalar('train_episode/collision_cost_rear', float(row.get('collision_cost_rear', 0.0)), ep)
    writer.add_scalar('train_episode/collision_cost_lateral', float(row.get('collision_cost_lateral', 0.0)), ep)
    writer.add_scalar('train_episode/mean_cost_headway', float(row.get('mean_cost_headway', 0.0)), ep)
    writer.add_scalar('train_episode/mean_cost_overspeed', float(row.get('mean_cost_overspeed', 0.0)), ep)
    writer.add_scalar('train_episode/mean_cost_comfort', float(row.get('mean_cost_comfort', 0.0)), ep)
    writer.add_scalar('train_episode/mean_cost_total', float(row.get('mean_cost_total', 0.0)), ep)
    writer.add_scalar('train_episode/mean_cost_lane', float(row.get('mean_cost_lane', 0.0)), ep)
    writer.add_scalar('train_episode/boundary_cost', float(row.get('boundary_cost', 0.0)), ep)
    if mode_is_constrained(mode):
        for key in ['lambda_collision', 'lambda_headway', 'lambda_overspeed', 'lambda_comfort', 'lambda_lane']:
            value = float(row.get(key, float('nan')))
            if np.isfinite(value):
                writer.add_scalar(f'constraint_lambda/{key}', value, ep)
        for key in ['qc_collision', 'qc_headway', 'qc_overspeed', 'qc_comfort', 'qc_lane']:
            value = float(row.get(key, float('nan')))
            if np.isfinite(value):
                writer.add_scalar(f'cost_critic/{key}', value, ep)
        for key in ['ema_collision_cost', 'ema_headway_cost', 'ema_overspeed_cost', 'ema_comfort_cost', 'ema_lane_cost']:
            value = float(row.get(key, float('nan')))
            if np.isfinite(value):
        ...
        for key in [
            ...
            value = float(row.get(key, float('nan')))
            if np.isfinite(value):
                writer.add_scalar(f'constraint_dual/{key}', value, ep)
...
def plot_train_diagnostics(rows: List[dict], save_path: str, mode: str = "", include_plot_grid: bool = True) -> str:
    ...
    if mode_is_constrained(mode):
        ...
        axes = np.asarray(axes).reshape(4, 2)
        ...
        ax.plot(episodes, _safe_float_list(rows, "offroad_rate"), marker="o", label="offroad_rate")
        ax.set_title("Success / Collision / Offroad")
        ax.set_xlabel("Episode")
        ax.legend()
        if include_plot_grid:
            ax.grid(True, alpha=0.3)
        ...
        ax = axes[1, 0]
        ax.plot(episodes, _safe_float_list(rows, "unsafe_headway_rate"), marker="o", label="unsafe_headway_rate")
        ax.plot(episodes, _safe_float_list(rows, "overspeed_rate"), marker="o", label="overspeed_rate")
        ax.plot(episodes, _safe_float_list(rows, "low_ttc_rate"), marker="o", label="low_ttc_rate")
        ax.set_title("Safety Rates")
        ax.set_xlabel("Episode")
        ax.legend()
        if include_plot_grid:
            ax.grid(True, alpha=0.3)
        ...
        ax = axes[1, 1]
        ax.plot(episodes, _safe_float_list(rows, "mean_cost_total"), marker="o", label="mean_cost_total")
        ax.plot(episodes, _safe_float_list(rows, "collision_cost_front"), marker="o", label="collision_cost_front")
        ax.plot(episodes, _safe_float_list(rows, "collision_cost_rear"), marker="o", label="collision_cost_rear")
        ax.plot(episodes, _safe_float_list(rows, "collision_cost_lateral"), marker="o", label="collision_cost_lateral")
        ax.plot(episodes, _safe_float_list(rows, "mean_cost_overspeed"), marker="o", label="mean_cost_overspeed")
        ax.plot(episodes, _safe_float_list(rows, "mean_cost_comfort"), marker="o", label="mean_cost_comfort")
        ax.plot(episodes, _safe_float_list(rows, "mean_cost_lane"), marker="o", label="mean_cost_lane")
        ...
        ax.plot(episodes, _safe_float_list(rows, "lambda_lane"), marker="o", label="lambda_lane")
        ...
        ax.plot(episodes, _safe_float_list(rows, "qc_lane"), marker="o", label="qc_lane")
        ...
        ax = axes[3, 0]
        ax.plot(episodes, _safe_float_list(rows, "ema_collision_cost"), marker="o", label="ema_collision_cost")
        ax.plot(episodes, _safe_float_list(rows, "ema_headway_cost"), marker="o", label="ema_headway_cost")
        ax.plot(episodes, _safe_float_list(rows, "ema_overspeed_cost"), marker="o", label="ema_overspeed_cost")
        ax.plot(episodes, _safe_float_list(rows, "ema_comfort_cost"), marker="o", label="ema_comfort_cost")
        ax.plot(episodes, _safe_float_list(rows, "ema_lane_cost"), marker="o", label="ema_lane_cost")
        ax.set_title("Constraint EMA Costs")
        ax.set_xlabel("Episode")
        ax.legend()
        if include_plot_grid:
            ax.grid(True, alpha=0.3)
        ...
        ax = axes[3, 1]
        ax.plot(episodes, _safe_float_list(rows, "mean_cost_collision"), marker="o", label="mean_cost_collision")
        ax.plot(episodes, _safe_float_list(rows, "mean_cost_headway"), marker="o", label="mean_cost_headway")
        ax.plot(episodes, _safe_float_list(rows, "mean_cost_overspeed"), marker="o", label="mean_cost_overspeed")
        ax.plot(episodes, _safe_float_list(rows, "mean_cost_comfort"), marker="o", label="mean_cost_comfort")
        ax.plot(episodes, _safe_float_list(rows, "mean_cost_lane"), marker="o", label="mean_cost_lane")
        ax.plot(episodes, _safe_float_list(rows, "boundary_cost"), marker="o", label="boundary_cost")
        ax.set_title("Per-Episode Cost Breakdown")
        ax.set_xlabel("Episode")
        ax.legend()
        if include_plot_grid:
            ax.grid(True, alpha=0.3)
...
def teacher_schedule_factor(cfg, episode_idx: int) -> float:
    ...
    if progress <= ratio:
        factor = 1.0 - 0.45 * (progress / max(ratio, 1e-6))
    else:
        tail = (progress - ratio) / max(1.0 - ratio, 1e-6)
        factor = 0.55 * (1.0 - tail) + min_weight * tail
    return float(np.clip(factor, min_weight, 1.0))
...
def build_teacher_buffer_dict(cfg, teacher_guidance: Optional[dict], episode_idx: int) -> Optional[dict]:
    if teacher_guidance is None:
        return None
    corrective_teacher = bool(teacher_guidance.get("corrective_teacher", False))
    if corrective_teacher:
        scheduled_weight = float(teacher_guidance.get("teacher_weight", 0.0))
        if teacher_guidance.get("teacher_action") is not None:
            scheduled_weight = max(scheduled_weight, float(getattr(cfg.sac, "corrective_teacher_weight_floor", 0.0)))
        scheduled_weight = float(np.clip(
            scheduled_weight,
            float(getattr(cfg.sac, "corrective_teacher_weight_floor", 0.0)),
            float(cfg.sac.corrective_teacher_weight_cap),
        ))
    else:
        schedule = teacher_schedule_factor(cfg, episode_idx)
        scheduled_weight = float(teacher_guidance.get("teacher_weight", 0.0)) * schedule
        if teacher_guidance.get("teacher_action") is not None:
            scheduled_weight = max(scheduled_weight, float(getattr(cfg.sac, "teacher_base_weight_floor", 0.0)) * schedule)
    return {
        "teacher_action": np.asarray(teacher_guidance.get("teacher_action", np.zeros((2,), dtype=np.float32)), dtype=np.float32),
        "teacher_weight": float(scheduled_weight),
        ...
        "teacher_is_corrective": float(1.0 if corrective_teacher else 0.0),
    }


def _boundary_correction_priors(cfg, scene: dict, verify_info: Optional[dict]) -> tuple[np.ndarray, np.ndarray]:
    behavior_prior = np.full((len(BEHAVIOR_VOCAB),), 0.03, dtype=np.float32)
    behavior_prior[BEHAVIOR_TO_INDEX["keep_lane"]] = 0.56
    behavior_prior[BEHAVIOR_TO_INDEX["follow"]] = 0.30
    if bool((verify_info or {}).get("unsafe_headway", False)) or bool((verify_info or {}).get("low_ttc", False)):
        behavior_prior[BEHAVIOR_TO_INDEX["brake"]] += 0.15
    behavior_prior = behavior_prior / max(float(behavior_prior.sum()), 1e-6)

    lane_prior = np.full((cfg.env.lanes_count,), 0.04, dtype=np.float32)
    lane_idx = int(np.clip(int(scene.get("lane_id", 0)), 0, max(cfg.env.lanes_count - 1, 0)))
    lane_prior[lane_idx] = 0.88
    lane_prior = lane_prior / max(float(lane_prior.sum()), 1e-6)
    return behavior_prior.astype(np.float32), lane_prior.astype(np.float32)



def build_augmented_teacher_guidance(
    cfg,
    base_teacher_guidance: Optional[dict],
    scene: dict,
    constraints: dict,
    raw_action,
    aligned_action,
    recovery_action,
    final_action,
    alignment_info: Optional[dict],
    recovery_info: Optional[dict],
    shield_info: Optional[dict],
    verify_info: Optional[dict],
    risk_correction_info: Optional[dict] = None,
) -> Optional[dict]:
    guidance = None if base_teacher_guidance is None else copy.deepcopy(base_teacher_guidance)
    abs_offset = float(abs(scene.get("signed_lane_offset_norm", scene.get("abs_lane_offset_norm", 0.0))))
    heading_abs = abs(float(scene.get("heading_error", scene.get("heading", 0.0))))
    road_proximity = float(scene.get("road_boundary_proximity", 0.0))
    lane_proximity = float(scene.get("lane_boundary_proximity", 0.0))
    correction_pressure = _clip01(max(road_proximity, lane_proximity, max(0.0, abs_offset - float(cfg.safety.lane_center_soft_ratio)), heading_abs / 1.0))

    corrective_action = None
    corrective_source = "none"
    weight_floor = 0.0
    shield_reasons = set((shield_info or {}).get("shield_reasons", []))

    if bool((shield_info or {}).get("shield_modified", False)) and correction_pressure >= 0.18 and ("boundary_steer_guard" in shield_reasons or "steer_suppression" in shield_reasons):
        corrective_action = np.asarray(final_action, dtype=np.float32)
        corrective_source = "shield_lane_guard"
        weight_floor = max(weight_floor, 0.18 + 0.18 * correction_pressure)
    elif bool((shield_info or {}).get("shield_modified", False)) and ("emergency_brake" in shield_reasons) and bool((verify_info or {}).get("unsafe_headway", False) or (verify_info or {}).get("low_ttc", False)):
        corrective_action = np.asarray(final_action, dtype=np.float32)
        corrective_source = "shield_emergency_brake"
        weight_floor = max(weight_floor, 0.16 + 0.12 * float((verify_info or {}).get("projected_boundary_risk", 0.0)))

    if bool((risk_correction_info or {}).get("correction_active", False)):
        risk_action = np.asarray((risk_correction_info or {}).get("corrected_action", final_action), dtype=np.float32)
        risk_floor = 0.14 + 0.24 * float((risk_correction_info or {}).get("severity", 0.0))
        if corrective_action is None:
            corrective_action = risk_action
            corrective_source = str((risk_correction_info or {}).get("correction_source", "risk_teacher_correction"))
        else:
            corrective_action = np.asarray(0.50 * corrective_action + 0.50 * risk_action, dtype=np.float32)
        weight_floor = max(weight_floor, risk_floor)

    if corrective_action is None:
        return guidance

    behavior_prior, lane_prior = _boundary_correction_priors(cfg, scene, verify_info)
    if guidance is None:
        guidance = {
            "teacher_action": corrective_action.astype(np.float32),
            "teacher_weight": 0.0,
            "teacher_target_speed": float(min(scene.get("ego_speed", 0.0), constraints.get("speed_max", scene.get("ego_speed", 0.0)))),
            "teacher_target_lane": float(scene.get("lane_id", 0)),
            "behavior_prior": behavior_prior,
            "lane_prior": lane_prior,
            "confidence": 1.0,
            "risk_vector": {"max_risk": float(max(road_proximity, lane_proximity, correction_pressure))},
            "novelty": 0.0,
            "recovery_source": corrective_source,
        }
    else:
        guidance["teacher_action"] = corrective_action.astype(np.float32)
        guidance["teacher_target_lane"] = float(scene.get("lane_id", 0))
        guidance["teacher_target_speed"] = float(min(scene.get("ego_speed", 0.0), constraints.get("speed_max", scene.get("ego_speed", 0.0))))
        guidance["behavior_prior"] = behavior_prior
        guidance["lane_prior"] = lane_prior
        guidance["recovery_source"] = corrective_source

    base_weight = float(guidance.get("teacher_weight", 0.0))
    guidance["teacher_weight"] = float(np.clip(
        max(base_weight, weight_floor, float(getattr(cfg.sac, "corrective_teacher_weight_floor", 0.0))),
        0.0,
        float(cfg.sac.corrective_teacher_weight_cap),
    ))
    guidance["corrective_teacher"] = True
    return guidance


def run(cfg):
    EVAL_VIDEO_RUNTIME_STATE["saved_count"] = 0
    EVAL_VIDEO_RUNTIME_STATE["warned_dual_mode"] = False
    ...
    exploration_shield = SafetyShield(cfg) if (cfg.safety.verifier_enabled and cfg.sac.safe_exploration_filter) else None
    ...
    failure_trace_dir = resolve_failure_trace_dir(cfg)
    failure_trace_paths = []
    if failure_trace_dir:
        print(f"[TRACE] failure episode traces enabled: {failure_trace_dir}")
    ...
    try:
        for ep in range(cfg.train.episodes):
            ...
            metrics = EpisodeMetrics(cfg)
            last_update_info = {}
            episode_step_trace: List[dict] = []

            latest_plan_info_train: Dict[str, Any] = {}
            if planner is not None:
                init_scene = env.get_scene_dict()
                plan = planner.plan(init_scene, step_idx=None)
                env.apply_plan(plan["waypoints"], plan["constraints"], plan.get("teacher_guidance"))
                latest_plan_info_train = copy.deepcopy(plan.get("planner_info", {}))
            ...
            for t in range(cfg.train.max_steps_per_episode):
                ...
                if total_steps < cfg.sac.start_steps:
                    raw_action = _sample_teacher_guided_warmup_action(cfg, env, constraints, total_steps)
                else:
                    raw_action = agent.select_action(state, evaluate=False).astype(np.float32)
                raw_action = raw_action.astype(np.float32)
                safe_filtered_action = raw_action.copy()
                safe_exploration_filter_used = _should_use_safe_exploration_filter(cfg, total_steps)
                if safe_exploration_filter_used:
                    safe_filtered_action = _apply_safe_exploration_filter(cfg, scene, safe_filtered_action, constraints, verifier, exploration_shield)
                true_raw_action = safe_filtered_action.astype(np.float32)
                aligned_action, alignment_info = apply_startup_alignment_override(cfg, scene, true_raw_action, constraints, latest_plan_info_train if planner is not None else {}, t)
                recovery_action, recovery_info = apply_local_recovery_override(cfg, scene, aligned_action, constraints)
                wrapped_raw_action, risk_correction_info, verify_info = _apply_risk_aware_teacher_correction(
                    cfg,
                    env,
                    scene,
                    recovery_action,
                ...
                wrapped_raw_action = wrapped_raw_action.astype(np.float32)
                shield_info = {
                    "shield_modified": False,
                    "shield_reasons": [],
                    "startup_alignment_active": bool(alignment_info.get("startup_alignment_active", False)),
                ...
                if shield is not None:
                    action, shield_info_core = shield.apply(scene, wrapped_raw_action, constraints, verify_info)
                    shield_info.update(shield_info_core)
                shielded_action = np.asarray(action, dtype=np.float32)
                ...
                action_trace_info = {
                    ...
                    "safe_filtered_action": safe_filtered_action,
                    "risk_corrected_action": wrapped_raw_action,
                    "shielded_action": shielded_action,
                    "safe_exploration_filter_used": bool(safe_exploration_filter_used),
                    "risk_correction_active": bool((risk_correction_info or {}).get("correction_active", False)),
                }
                if failure_trace_dir:
                    episode_step_trace.append(build_train_step_trace_record(
                        ep=ep,
                        step_idx=t,
                        ...
                        verify_info=verify_info,
                        shield_info=shield_info,
                        risk_correction_info=risk_correction_info,
                        reward=reward,
                ...
                teacher_guidance_for_buffer = build_augmented_teacher_guidance(
                    cfg,
                    env.current_teacher_guidance,
                    scene,
                    constraints,
                    true_raw_action,
                    aligned_action,
                ...
                teacher_dict = build_teacher_buffer_dict(cfg, teacher_guidance_for_buffer, episode_idx=ep)
                ...
                metrics.update(
                    ...
                    shield_info=shield_info,
                    cost_dict=cost_dict,
                    action_trace_info=action_trace_info,
                )
                if planner is not None and hasattr(planner, "observe_execution"):
                    planner.observe_execution(next_scene, verify_info, shield_info, t)
            ...
            failure_trace_path = export_failure_episode_trace(failure_trace_dir, ep, ep_summary, episode_step_trace)
            if failure_trace_path:
                ep_summary["failure_trace_path"] = failure_trace_path
                failure_trace_paths.append(failure_trace_path)
            ...
            print(
                ...
                f"offroad_rate={ep_summary['offroad_rate']:.3f} "
            ...
            print(
                ...
                f"offroad_rate={train_diag_row['offroad_rate']:.3f} "
                f"unsafe_headway_rate={train_diag_row['unsafe_headway_rate']:.3f} "
                f"overspeed_rate={train_diag_row['overspeed_rate']:.3f} "
                f"low_ttc_rate={train_diag_row['low_ttc_rate']:.3f} "
                f"safe_filter_rate={train_diag_row['safe_exploration_filter_rate']:.3f} "
                f"risk_corr_rate={train_diag_row['risk_correction_rate']:.3f} "
                f"train_action_modified_rate={train_diag_row['train_action_modified_rate']:.3f} "
                f"mean_cost_over={train_diag_row['mean_cost_overspeed']:.3f} "
                f"mean_cost_comf={train_diag_row['mean_cost_comfort']:.3f} "
                f"mean_cost_lane={train_diag_row['mean_cost_lane']:.3f} "
                ...
                + (
                    ...
                    f" lambda_lane={train_diag_row['lambda_lane']:.4f}"
                    ...
                    f" qc_lane={train_diag_row['qc_lane']:.4f}"
                    f" dual_over_sig={train_diag_row['dual_signal_overspeed']:.3f}"
                    f" dual_over_err={train_diag_row['dual_error_overspeed']:.3f}"
                    f" over_release={train_diag_row['dual_release_overspeed']:.0f}"
    ...
    result = {
        ...
        "failure_trace_dir": os.path.abspath(failure_trace_dir) if failure_trace_dir else "",
        "failure_trace_paths": list(failure_trace_paths),
        "run_result_json_path": "",
    }
    if cfg.eval.enabled:
        result["eval"] = evaluate_policy(cfg, agent, mode=cfg.train.mode)
    result.setdefault("metadata", {})["eval_video_fps"] = summarize_eval_video_fps(result.get("eval"), cfg)
    ...
    run_result_target = resolve_run_result_json_path(cfg)
    result["run_result_json_path"] = run_result_target
    run_result_json_path = export_run_result_json(result, run_result_target)
    result["run_result_json_path"] = run_result_json_path or run_result_target
    return result
...
def resolve_compare_seeds(cfg) -> List[int]:
    stage = str(cfg.workflow.stage).lower()
    requested = max(1, int(cfg.compare.num_seeds))
    ...
    if stage in {"dev", "freeze"}:
        default_dev_seeds = _normalize_seed_list(cfg.freeze_protocol.dev_seeds, [cfg.train.seed])
        if requested <= 1:
            return list(default_dev_seeds)
        if len(default_dev_seeds) >= requested:
            return list(default_dev_seeds[:requested])
        return [int(cfg.train.seed + i * cfg.compare.seed_stride) for i in range(requested)]
...
def merge_compare_reports(raw_rows: List[dict], shielded_rows: List[dict]) -> List[dict]:
    ...
    for raw in raw_rows:
        ...
        merged.append({
            ...
            "raw_cost_total": raw.get("cost_total", "0.000±0.000"),
            ...
            "shield_intervention_rate": shield.get("shield_rate", "0.000±0.000"),
...
def export_compare_csv(merged_rows: List[dict], save_path: str) -> None:
    ...
    fieldnames = [
        "mode", "num_runs", "seeds",
        "raw_return", "raw_success", "raw_collision", "raw_cost_total", "raw_mean_speed", "raw_unsafe_headway",
        "shield_success", "shield_collision", "shield_interventions", "shield_intervention_rate", "shield_cost_total", "time_sec",
    ]
    os.makedirs(os.path.dirname(save_path) or ".", exist_ok=True)
    with open(save_path, "w", encoding="utf-8", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=fieldnames)
        ...
        for row in merged_rows:
            writer.writerow({
                ...
                "raw_cost_total": row["raw_cost_total"],
                ...
                "shield_intervention_rate": row["shield_intervention_rate"],
...
def export_compare_latex(merged_rows: List[dict], save_path: str) -> str:
    ...
    headers = [
        ...
        ("shield_intervention_rate", "ShieldRate"),
...
def run_compare(cfg, modes: Optional[List[str]] = None) -> dict:
    ...
    if len(seeds) < 5:
        print(f"[COMPARE][WARN] stage4 protocol recommends at least 5 training seeds; current_seeds={len(seeds)}")
    if int(cfg.eval.episodes) < 20:
        print(f"[COMPARE][WARN] stage4 protocol recommends at least 20 eval episodes per seed; current_eval_episodes={int(cfg.eval.episodes)}")
    if str(cfg.eval.primary_report).lower() != "raw":
        print(f"[COMPARE][WARN] stage4 protocol uses raw as primary report; current_primary_report={cfg.eval.primary_report}")
            ...
            for seed_idx, seed in enumerate(seeds):
                ...
                try:
                    ...
                    print(
                        ...
                        f"raw_cost={raw_row['cost_total']:.3f} shield_rate={shield_row['shield_rate']:.3f} "
    ...
    if cfg.compare.save_json_path:
        ...
        with open(cfg.compare.save_json_path, "w", encoding="utf-8") as f:
            json.dump(_redact_sensitive_payload(payload), f, ensure_ascii=False, indent=2)
...
def _build_test_scene(cfg, **overrides) -> dict:
    lane_id = int(overrides.get("lane_id", 1))
    left_lane_id = overrides.get("left_lane_id", lane_id - 1 if lane_id > 0 else None)
    right_lane_id = overrides.get("right_lane_id", lane_id + 1 if lane_id < cfg.env.lanes_count - 1 else None)
    scene = {
        "ego_x": 0.0,
        "ego_y": float(lane_id) * float(cfg.env.lane_width),
        "ego_speed": 24.0,
        "heading": 0.0,
        "lane_id": lane_id,
        "lane_center_y": float(lane_id) * float(cfg.env.lane_width),
        "left_lane_id": left_lane_id,
        "right_lane_id": right_lane_id,
        "front_distance": 120.0,
        "front_rel_speed": 0.0,
        "rear_distance": 40.0,
        "rear_rel_speed": 0.0,
        "left_front_distance": 80.0,
        "left_front_rel_speed": 0.0,
        "left_rear_distance": 30.0,
        "left_rear_rel_speed": 0.0,
        "right_front_distance": 80.0,
        "right_front_rel_speed": 0.0,
        "right_rear_distance": 30.0,
        "right_rear_rel_speed": 0.0,
        "collision": False,
        "last_action": np.zeros((2,), dtype=np.float32),
    }
    scene.update(overrides)
    closing_speed = float(max(0.0, -float(scene.get("front_rel_speed", 0.0))))
    scene["front_closing_speed"] = float(scene.get("front_closing_speed", closing_speed))
    scene["front_ttc"] = float(scene.get("front_ttc", scene["front_distance"] / max(scene["front_closing_speed"], 1e-3) if scene["front_distance"] < 1e5 and scene["front_closing_speed"] > 0 else 99.0))
    scene["ttc"] = float(scene.get("ttc", scene["front_ttc"]))
    rear_closing_speed = float(max(0.0, -float(scene.get("rear_rel_speed", 0.0))))
    scene["rear_closing_speed"] = float(scene.get("rear_closing_speed", rear_closing_speed))
    scene["rear_ttc"] = float(scene.get("rear_ttc", scene["rear_distance"] / max(scene["rear_closing_speed"], 1e-3) if scene["rear_distance"] < 1e5 and scene["rear_closing_speed"] > 0 else 99.0))
    for side in ("left", "right"):
        ...
        front_closing = float(scene.get(f"{side}_front_closing_speed", max(0.0, -front_rel)))
        rear_closing = float(scene.get(f"{side}_rear_closing_speed", max(0.0, -rear_rel)))
        front_ttc = float(scene.get(f"{side}_front_ttc", front_d / max(front_closing, 1e-3) if front_d < 1e5 and front_closing > 0 else 99.0))
        rear_ttc = float(scene.get(f"{side}_rear_ttc", rear_d / max(rear_closing, 1e-3) if rear_d < 1e5 and rear_closing > 0 else 99.0))
        clear_geom = bool(scene.get(f"{side}_clear_geom", lane_exists and front_d > 18.0 and rear_d > 12.0))
        clear_dyn = bool(scene.get(f"{side}_clear_dyn", lane_exists and clear_geom and (front_closing <= 3.0 or front_ttc >= 4.5) and (rear_closing <= 2.5 or rear_ttc >= 4.0)))
        scene[f"{side}_front_closing_speed"] = front_closing
        scene[f"{side}_front_ttc"] = front_ttc
        scene[f"{side}_rear_closing_speed"] = rear_closing
        scene[f"{side}_rear_ttc"] = rear_ttc
        scene[f"{side}_clear_geom"] = clear_geom
        scene[f"{side}_clear_dyn"] = clear_dyn
        scene[f"{side}_clear"] = bool(scene.get(f"{side}_clear", clear_dyn))
    return scene
...
def run_offline_tests() -> dict:
    ...
    def _record(name: str, ok: bool, details: str = ""):
        status = "PASS" if ok else "FAIL"
        print(f"[OFFLINE-TEST][{status}] {name} {details}".rstrip())
        results.append({"name": name, "ok": bool(ok), "details": str(details)})
        if not ok:
            raise AssertionError(f"offline test failed: {name} {details}")

    safe_scene = _build_test_scene(cfg, front_distance=160.0, ttc=99.0, front_rel_speed=0.0)
    safe_validity = build_plan_validity(cfg, safe_scene, {"behavior": "keep_lane", "target_lane": safe_scene["lane_id"], "risk_mode": 0}, {"target_lane": safe_scene["lane_id"], "target_speed": 25.0, "behavior": "keep_lane"})
    safe_commit = {"created_step": 0, "reference_scene": copy.deepcopy(safe_scene), "reference_lane_feasibility": safe_validity["reference_lane_feasibility"], "plan_validity": safe_validity, "command": {"behavior": "keep_lane"}}
    safe_state = plan_event_state(cfg, safe_commit, safe_scene, safe_scene, False, False, step_idx=5)
    _record("safe_cruise", safe_validity["hold_min_steps"] >= 20 and safe_state["state"] == "COMMIT", f"hold_min={safe_validity['hold_min_steps']} state={safe_state['state']}")

    brake_scene = _build_test_scene(cfg, front_distance=8.0, front_rel_speed=-10.0)
    brake_scene["ttc"] = 0.8
    brake_state = plan_event_state(cfg, safe_commit, brake_scene, safe_scene, False, False, step_idx=max(25, safe_validity["hold_min_steps"] + 1))
    _record("front_brake", brake_state["state"] in {"REPLAN", "HARD_REPLAN"} and ("ttc_cross" in brake_state["events"] or "front_distance_cross" in brake_state["events"]), f"state={brake_state['state']} events={brake_state['events']}")

    lane_ref_scene = _build_test_scene(cfg, left_front_distance=45.0, left_rear_distance=24.0, left_rear_rel_speed=0.0, left_clear_dyn=True, left_clear=True)
    lane_validity = build_plan_validity(cfg, lane_ref_scene, {"behavior": "change_left", "target_lane": lane_ref_scene["lane_id"] - 1, "risk_mode": 1}, {"target_lane": lane_ref_scene["lane_id"] - 1, "target_speed": 22.0, "behavior": "change_left"})
    lane_commit = {"created_step": 0, "reference_scene": copy.deepcopy(lane_ref_scene), "reference_lane_feasibility": lane_validity["reference_lane_feasibility"], "plan_validity": lane_validity, "command": {"behavior": "change_left"}}
    lane_bad_scene = _build_test_scene(cfg, left_front_distance=45.0, left_rear_distance=16.0, left_rear_rel_speed=-6.0)
    lane_bad_scene["left_clear_dyn"] = False
    lane_bad_scene["left_clear"] = False
    lane_state = plan_event_state(cfg, lane_commit, lane_bad_scene, lane_ref_scene, False, False, step_idx=6)
    _record("dependent_lane_rear_gap", lane_state["state"] == "HARD_REPLAN" and "dependent_lane_feasibility_flip" in lane_state["events"], f"events={lane_state['events']}")

    far_prev = _build_test_scene(cfg, front_distance=220.0, front_rel_speed=0.0, ttc=99.0)
    far_curr = _build_test_scene(cfg, front_distance=180.0, front_rel_speed=-6.0)
    far_curr["ttc"] = 30.0
    far_validity = build_plan_validity(cfg, far_prev, {"behavior": "keep_lane", "target_lane": far_prev["lane_id"], "risk_mode": 0}, {"target_lane": far_prev["lane_id"], "target_speed": 25.0, "behavior": "keep_lane"})
    far_commit = {"created_step": 0, "reference_scene": copy.deepcopy(far_prev), "reference_lane_feasibility": far_validity["reference_lane_feasibility"], "plan_validity": far_validity, "command": {"behavior": "keep_lane"}}
    far_state = plan_event_state(cfg, far_commit, far_curr, far_prev, False, False, step_idx=8)
    _record("far_closing_speed", far_state["state"] == "MONITOR" and "closing_speed_rise_far" in far_state["events"] and far_state["valid"], f"events={far_state['events']}")

    shield_state = plan_event_state(cfg, safe_commit, safe_scene, safe_scene, True, False, step_idx=8)
    _record("shield_intervention", shield_state["state"] == "HARD_REPLAN" and "shield_intervened" in shield_state["events"], f"events={shield_state['events']}")

    planner = LLMPlanner(cfg, MockLLMBackend(cfg), memory_readonly=True)
    planner.plan_commit = safe_commit
    planner.previous_scene = copy.deepcopy(safe_scene)
    planner.last_observed_shield_intervened = True
    planner.force_replan_next = True
    _ = planner.should_replan(safe_scene, step_idx=9)
    dbg = planner.last_replan_debug
    _record("should_replan_debug", isinstance(dbg.get("events", []), list) and "plan_age" in dbg and dbg.get("shield_intervened") is True and dbg.get("force_replan_next") is True, f"debug={dbg}")

    lane_risk_scene = _build_test_scene(cfg, front_distance=140.0, front_rel_speed=0.0, left_front_distance=30.0, left_front_rel_speed=-5.0, left_rear_distance=14.0, left_rear_rel_speed=-5.0)
    lane_risk_scene["left_clear_dyn"] = False
    lane_risk_scene["left_clear"] = False
    rv = compute_scene_risk_vector(cfg, lane_risk_scene, novelty=0.1)
    _record(
        "lane_change_risk_separated_from_current_plan_risk",
        rv["max_lane_change_risk"] >= 0.70 and rv["global_scene_risk"] >= rv["max_lane_change_risk"] and rv["max_risk"] <= rv["global_scene_risk"],
        f"risk={rv}",
    )

    constraints = {
        "min_headway": float(cfg.planner.normal_headway),
        "speed_max": float(cfg.planner.speed_max),
        "acc_max": float(cfg.planner.acc_max),
        "steer_max": float(cfg.planner.steer_max),
        "comfort_acc_delta": float(cfg.planner.comfort_acc_delta),
    }
    lateral_scene = _build_test_scene(
        cfg,
        front_distance=140.0,
        front_rel_speed=0.0,
        left_front_distance=70.0,
        left_front_rel_speed=0.0,
        left_rear_distance=10.0,
        left_rear_rel_speed=-8.0,
    )
    lateral_next_scene = copy.deepcopy(lateral_scene)
    lateral_next_scene["lane_id"] = int(max(0, int(lateral_scene["lane_id"]) - 1))
    lateral_action = np.array([0.20, -0.70], dtype=np.float32)
    lateral_risk = compute_scene_risk_vector(cfg, lateral_scene, novelty=0.0)
    lateral_cost = compute_costs(cfg, lateral_scene, lateral_next_scene, lateral_action, constraints)
    lateral_verify = SafetyVerifier(cfg).check(lateral_scene, lateral_action, constraints)
    _record(
        "stage3_lateral_risk_primitives",
        lateral_risk["front_collision_risk"] < 0.05
        and lateral_risk["left_rear_ttc"] < 2.0
        and lateral_risk["left_merge_conflict_risk"] >= 0.70
        and lateral_risk["max_lane_change_risk"] >= 0.70,
        f"risk={lateral_risk}",
    )
    _record(
        "stage3_lateral_collision_cost",
        lateral_cost["collision_cost_lateral"] >= 0.70
        and lateral_cost["collision_cost_front"] < 0.05
        and lateral_cost["collision_cost"] >= lateral_cost["collision_cost_lateral"],
        f"cost={lateral_cost}",
    )
    _record(
        "stage3_verifier_lateral_merge_risk",
        lateral_verify["lateral_merge_risk_high"] and lateral_verify["needs_intervention"],
        f"verify={lateral_verify}",
    )

    rear_scene = _build_test_scene(
        cfg,
        front_distance=140.0,
        front_rel_speed=0.0,
        rear_distance=8.0,
    ...
    rear_next_scene = copy.deepcopy(rear_scene)
    rear_action = np.array([-0.80, 0.0], dtype=np.float32)
    rear_cost = compute_costs(cfg, rear_scene, rear_next_scene, rear_action, constraints)
    rear_verify = SafetyVerifier(cfg).check(rear_scene, rear_action, constraints)
    _record(
        "stage3_rear_collision_cost",
        rear_cost["collision_cost_rear"] > 0.55 and rear_cost["collision_cost_front"] < 0.05,
        f"cost={rear_cost}",
    )
    _record(
        "stage3_verifier_rear_risk_not_front_emergency",
        rear_verify["rear_collision_risk_high"]
        and not rear_verify["front_collision_risk_high"]
        and not rear_verify["emergency"],
        f"verify={rear_verify}",
    )

    payload = {"passed": len(results), "total": len(results), "results": results}
    print(f"[OFFLINE-TEST] {payload['passed']}/{payload['total']} passed")
    return payload
...
def parse_args():
    ...
    parser.add_argument("--failure-trace-dir", type=str, default="")
    ...
    parser.add_argument("--eval-video-dir", type=str, default="results/eval_videos")
    parser.add_argument("--eval-video-fps", type=int, default=15)
    parser.add_argument("--eval-max-videos", type=int, default=8)
    parser.add_argument("--eval-video-prefix", type=str, default="")
    parser.add_argument("--startup-debug", action="store_true")
    parser.add_argument("--startup-debug-steps", type=int, default=20)
    parser.add_argument("--startup-debug-log-path", type=str, default="")
    parser.add_argument("--disable-startup-alignment", action="store_true")
    parser.add_argument("--startup-alignment-steps", type=int, default=20)
    ...
    parser.add_argument("--min-lambda-overspeed", type=float, default=0.030)
    parser.add_argument("--lambda-overspeed-max", type=float, default=12.0)
    parser.add_argument("--lambda-overspeed-update-scale", type=float, default=1.0)
    parser.add_argument("--dual-batch-mix", type=float, default=0.25)
    parser.add_argument("--dual-policy-mix", type=float, default=0.55)
    parser.add_argument("--dual-anti-windup-gain", type=float, default=0.25)
    parser.add_argument("--dual-overspeed-release-margin", type=float, default=0.05)
    parser.add_argument("--dual-overspeed-release-patience", type=int, default=2)
    parser.add_argument("--dual-overspeed-release-scale", type=float, default=1.50)
    parser.add_argument("--dual-overspeed-release-integral-decay", type=float, default=0.50)
...
if __name__ == "__main__":
    ...
    if args.plot_diagnostics_from_csv.strip():
        ...
        sys.exit(0)
    if bool(args.run_offline_tests):
        run_offline_tests()
        sys.exit(0)
    ...
    cfg.diagnostics.failure_trace_dir = str(args.failure_trace_dir).strip()
    ...
    cfg.eval.video_dir = str(args.eval_video_dir).strip() or "results/eval_videos"
    cfg.eval.video_fps = max(1, int(args.eval_video_fps))
    cfg.eval.max_videos = max(0, int(args.eval_max_videos))
    cfg.eval.video_prefix = str(args.eval_video_prefix).strip()
    cfg.eval.startup_debug = bool(args.startup_debug)
    cfg.eval.startup_debug_steps = max(0, int(args.startup_debug_steps))
    cfg.eval.startup_debug_log_path = str(args.startup_debug_log_path).strip()
    cfg.planner.startup_alignment_enabled = not bool(args.disable_startup_alignment)
    cfg.planner.startup_alignment_steps = max(0, int(args.startup_alignment_steps))
    ...
    cfg.sac.min_lambda_overspeed = float(args.min_lambda_overspeed)
    cfg.sac.lambda_overspeed_max = float(args.lambda_overspeed_max)
    cfg.sac.lambda_overspeed_update_scale = float(args.lambda_overspeed_update_scale)
    cfg.sac.dual_batch_mix = float(args.dual_batch_mix)
    cfg.sac.dual_policy_mix = float(args.dual_policy_mix)
    cfg.sac.dual_anti_windup_gain = float(args.dual_anti_windup_gain)
    cfg.sac.dual_overspeed_release_margin = float(args.dual_overspeed_release_margin)
    cfg.sac.dual_overspeed_release_patience = max(1, int(args.dual_overspeed_release_patience))
    cfg.sac.dual_overspeed_release_scale = float(args.dual_overspeed_release_scale)
    cfg.sac.dual_overspeed_release_integral_decay = float(args.dual_overspeed_release_integral_decay)
    ...
    apply_timestamp_prefix_to_output_paths(cfg)

    initialize_workflow(cfg, run_mode=args.mode)
    if cfg.workflow.stage == "freeze":
        sys.exit(0)

    if bool(cfg.eval.startup_debug) and str(cfg.eval.startup_debug_log_path).strip():
        os.makedirs(os.path.dirname(cfg.eval.startup_debug_log_path) or ".", exist_ok=True)
        with open(cfg.eval.startup_debug_log_path, "w", encoding="utf-8") as _f:
            _f.write("")
