...
class PlannerConfig:
    ...
    replan_ttc_threshold: float = 3.0
    replan_front_distance: float = 16.0

    target_speed_free: float = 25.5
    target_speed_cautious: float = 19.5
    target_speed_brake: float = 13.5

    conservative_headway: float = 22.0
    normal_headway: float = 15.0

    speed_max: float = 27.5
    acc_max: float = 2.5
    steer_max: float = 0.24
    comfort_acc_delta: float = 1.2
...
class CompareConfig:
    ...
    # policy-only comparison is fairer; shield can still be enabled explicitly later
...
def evaluate_policy(cfg, agent, mode: str):
    print("\n================ EVALUATION START ================\n")
    print(f"[EVAL-CONFIG] mode={mode} shield_enabled_eval={int(cfg.safety.shield_enabled_eval)}")
...
def resolve_compare_seeds(cfg) -> List[int]:
    if len(cfg.compare.explicit_seeds) > 0:
        return [int(s) for s in cfg.compare.explicit_seeds]
    return [int(cfg.train.seed + i * cfg.compare.seed_stride) for i in range(max(1, cfg.compare.num_seeds))]


def build_compare_run_row(exp_result: dict) -> dict:
    row = {
        "mode": exp_result["mode"],
        "seed": int(exp_result.get("seed", -1)),
    }
    for metric_key, alias in COMPARE_METRIC_SPECS:
        row[alias] = float(_metric_from_result(exp_result, metric_key))
    return row


def aggregate_compare_rows(mode: str, run_rows: List[dict]) -> dict:
    agg = {
        "mode": mode,
...
def format_compare_table(aggregated_rows: List[dict]) -> str:
    headers = [
        "mode",
        "runs",
        "return",
...
def export_compare_csv(aggregated_rows: List[dict], save_path: str) -> None:
    if not save_path:
        return

    fieldnames = ["mode", "num_runs", "seeds"]
    for _, alias in COMPARE_METRIC_SPECS:
        fieldnames.extend([f"{alias}_mean", f"{alias}_std"])
    ...
    with open(save_path, "w", encoding="utf-8", newline="") as f:
        ...
        for row in aggregated_rows:
            ...
            for _, alias in COMPARE_METRIC_SPECS:
                csv_row[f"{alias}_mean"] = float(row[f"{alias}_mean"])
                csv_row[f"{alias}_std"] = float(row[f"{alias}_std"])
...
def export_compare_latex(aggregated_rows: List[dict], save_path: str) -> str:
    if not save_path:
        return ""
    ...
    headers = [
        ...
        ("min_front_d", "MinFrontD"),
        ("interventions", "Interventions"),
        ("unsafe_headway", "UnsafeHeadway"),
...
def run_compare(cfg, modes: Optional[List[str]] = None) -> dict:
    ...
    for idx, mode in enumerate(modes):
        ...
        for seed_idx, seed in enumerate(seeds):
            ...
            exp_cfg.safety.shield_enabled_eval = bool(exp_cfg.compare.use_shield_in_eval)
            if exp_cfg.compare.disable_step_print_during_compare:
                exp_cfg.train.print_every_step = False

            print(f"[COMPARE][RUN] mode={mode} seed={seed} ({seed_idx + 1}/{len(seeds)})")
            start = time.time()
            result = run(exp_cfg)
            ...
            row = build_compare_run_row(result)
            run_rows.append(row)
            mode_run_rows.append(row)

            print(
                f"[COMPARE][DONE-RUN] mode={mode} seed={seed} "
                f"return={row['return']:.3f} "
                f"success={row['success']:.3f} "
                f"collision={row['collision']:.3f} "
                f"time_sec={row['time_sec']:.2f}"
            )

        agg_row = aggregate_compare_rows(mode, mode_run_rows)
        aggregated_rows.append(agg_row)

        print(
            f"[COMPARE][DONE-MODE] mode={mode} runs={agg_row['num_runs']} "
            f"return={agg_row['return']} success={agg_row['success']} "
            f"collision={agg_row['collision']} time_sec={agg_row['time_sec']}"
        )
        print()

    table = format_compare_table(aggregated_rows)
    print("\n================ COMPARE TABLE ================\n")
    print(table)
    ...
    export_compare_csv(aggregated_rows, cfg.compare.save_csv_path)
    payload["latex_table"] = export_compare_latex(aggregated_rows, cfg.compare.save_latex_path)
    return payload
...
# 单跑：
# python single_file_llm_guided_rl_v4_optimized.py --mode baseline_sac --episodes 5
...
#
# 比较：
# python single_file_llm_guided_rl_v4_optimized.py --mode compare --episodes 5 --compare-modes baseline_sac,rule_hier,llm_hier,constrained_rule_hier,constrained_llm_hier
...
# 指定 seeds：
# python single_file_llm_guided_rl_v4_optimized.py --mode compare --episodes 5 --compare-seeds 42,142,242 --compare-save-json compare_results.json --compare-save-csv compare_summary.csv --compare-save-latex compare_table.tex




# 3 个 seed，按 base seed + stride 生成：42, 142, 242
