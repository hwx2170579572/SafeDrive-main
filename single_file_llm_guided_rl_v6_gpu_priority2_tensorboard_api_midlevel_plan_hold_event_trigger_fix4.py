...
def compute_scene_risk_vector(cfg, scene: dict, novelty: float = 0.0) -> dict:
    ...
    def _lane_change_risk(side: str):
        if side == "left":
    ...
    risks = {
        ...
        "left_change": None if left_change is None else float(left_change),
        "right_change": None if right_change is None else float(right_change),
...
def build_plan_validity(cfg, scene: dict, command: dict, decision: dict) -> dict:
    ...
    near_risk = bool(risk_mode >= 1 or front_distance < 50.0 or ttc < 10.0)
    ...
    if lane_change_required:
        ...
        hold_default = int(cfg.planner.hold_min_steps_risky)
        hold_lb, hold_ub = 4, min(int(cfg.planner.hold_max_steps), 6)
    else:
        hold_default = max(10, int(cfg.planner.hold_min_steps_default))
        hold_lb, hold_ub = hold_default, min(int(cfg.planner.hold_max_steps), 20)
...
def plan_event_state(cfg, plan_commit: Optional[dict], current_scene: dict, previous_scene: Optional[dict], shield_intervened: bool, force_replan: bool, step_idx: int) -> dict:
    ...
    ref_feas = plan_commit.get("reference_lane_feasibility", lane_feasibility_signature(ref_scene))
    dependent_side = str(validity.get("dependent_lane_side", "none"))
    if current_feas != ref_feas:
        ...
        if bool(validity.get("expire_if_lane_feasibility_changes", False)) and dependent_side in {"left", "right"} and current_dep_feas != ref_dep_feas:
    ...
    closing_speed_delta = abs(float(current_scene.get("front_rel_speed", 0.0)) - ref_front_rel_speed)
    closing_speed_relevant = bool(
        curr_fd < float(cfg.planner.event_closing_speed_front_distance)
        or curr_ttc < float(cfg.planner.event_closing_speed_ttc)
    )
    if closing_speed_relevant and closing_speed_delta >= float(cfg.planner.event_closing_speed_delta):
    ...
    combined_monitor = monitor_events + [e for e in events if e in monitor_events]
...
- For safe cruising keep_lane/follow scenes with front_distance >= 50m and ttc >= 10s, set hold_min_steps to 10-20.
- Only when following closely or under near-risk conditions (for example front_distance < 50m or ttc < 10s) may hold_min_steps be 4-6.
