...
def translate_midlevel_command(cfg, scene: dict, command: dict, planner_state: dict, step_idx: Optional[int]) -> dict:
    ...
    very_safe_recovery = bool(
        safe_cruise
        and int(target_lane) == lane_id
        and front_distance >= max(120.0, float(cfg.planner.safe_cruise_front_distance) + 40.0)
        and ttc >= max(15.0, float(cfg.planner.safe_cruise_ttc))
        and closing_pressure <= 0.0
        and current_overspeed_gap <= 0.0
        and not near_overspeed
    )
    if very_safe_recovery:
        cruise_floor = max(
            float(cfg.planner.safe_cruise_target_speed_floor),
            min(float(cfg.planner.target_speed_free), ego_speed + 2.0),
        )
        requested_speed = max(requested_speed, cruise_floor)
        if behavior == "follow":
            behavior = "keep_lane"
        if headway_level == "conservative":
            headway_level = "normal"
        if speed_mode != "brake" and requested_speed >= float(cfg.planner.target_speed_cautious) + 1.0:
            speed_mode = "free"
...
class RealLLMBackend(BaseLLMBackend):
    ...
    def _post_json(self, url: str, headers: dict, payload: dict, timeout_sec: Optional[float] = None) -> dict:
        ...
        timeout_value = float(timeout_sec if timeout_sec is not None else self.cfg.llm.timeout_sec)
        with urllib.request.urlopen(req, timeout=timeout_value) as resp:
            return json.loads(resp.read().decode("utf-8"))

    def _call_real_api(self, prompt: str, system_prompt: Optional[str] = None, max_tokens: Optional[int] = None, response_format_json: Optional[bool] = None, timeout_sec: Optional[float] = None) -> str:
        ...
        response = self._post_json(api_base, headers, payload, timeout_sec=timeout_sec)
    ...
    def _call_real_api_with_retry(self, prompt: str, system_prompt: Optional[str] = None, max_tokens: Optional[int] = None, response_format_json: Optional[bool] = None, call_tag: str = "generate") -> str:
        ...
        base_timeout = max(20.0, float(self.cfg.llm.timeout_sec))
        last_error: Optional[Exception] = None
        for attempt in range(1, retry_times + 1):
            attempt_timeout = base_timeout + float(min(attempt - 1, 2) * 8.0)
            try:
...
class LLMPlanner:
    ...
    def _can_accept_low_confidence_plan(self, parsed: dict, scene: dict) -> bool:
        command = dict(parsed.get("command", {}))
        confidence = float(command.get("confidence", 1.0))
        if bool(command.get("abstain", False)):
            return False
        lane_id = int(scene.get("lane_id", 0))
        target_lane = int(command.get("target_lane", lane_id))
        behavior = _normalize_behavior_token(command.get("behavior", "keep_lane"), default="keep_lane")
        front_distance = float(scene.get("front_distance", 1e6))
        ttc = float(scene.get("ttc", 99.0))
        ego_speed = float(scene.get("ego_speed", 0.0))
        closing_speed = float(max(0.0, -float(scene.get("front_rel_speed", 0.0))))
        overspeed_gap = max(0.0, ego_speed - float(self.cfg.planner.speed_max))
        return bool(
            confidence >= max(0.20, 0.60 * float(self.cfg.llm.min_command_confidence))
            and int(command.get("risk_mode", _scene_risk_mode(self.cfg, scene))) <= 0
            and target_lane == lane_id
            and behavior in {"keep_lane", "follow"}
            and front_distance >= float(self.cfg.planner.safe_cruise_front_distance)
            and ttc >= float(self.cfg.planner.safe_cruise_ttc)
            and closing_speed < float(self.cfg.planner.closing_speed_soft_threshold)
            and overspeed_gap <= 0.0
            and not bool(parsed.get("plan_validity", {}).get("expire_if_lane_feasibility_changes", False))
        )
    ...
    def _maybe_replace_low_confidence_plan(self, parsed: dict, scene: dict, step_idx: Optional[int]) -> dict:
        ...
        if (not abstain) and confidence >= float(self.cfg.llm.min_command_confidence):
            return parsed
        if self._can_accept_low_confidence_plan(parsed, scene):
            accepted = copy.deepcopy(parsed)
            accepted["command"] = dict(accepted.get("command", {}))
            accepted["command"]["confidence"] = max(confidence, float(self.cfg.llm.min_command_confidence))
            if "decision" in accepted:
                accepted["decision"] = dict(accepted.get("decision", {}))
                accepted["decision"]["confidence"] = float(accepted["command"]["confidence"])
            accepted["recovery_source"] = "low_confidence_safe_accept"
            append_llm_call_log(self.cfg, {
                "timestamp_utc": _utc_now_iso(),
                "stage": str(self.cfg.workflow.stage),
                "mode": str(self.cfg.train.mode),
                "backend": f"{self.backend.name}_low_confidence_accept",
                "backend_requested": str(getattr(self.cfg.llm, "backend", self.backend.name)),
                "backend_effective": str(self.backend.name),
                "status": "accepted",
                "confidence": float(confidence),
                "fallback_used": False,
            })
