...
class PlannerConfig:
    ...
    hold_min_steps_safe_cruise: int = 30
    hold_min_steps_very_safe: int = 40
    hold_max_steps: int = 48
...
def _plan_hold_bounds(cfg, regime: dict) -> tuple[int, int, int, str]:
    ...
    if regime["lane_change_required"]:
        ...
        hold_lb, local_max = 8, 14
        reason = "lane_change_commit"
    elif regime["near_risk"]:
        hold_default = max(8, int(cfg.planner.hold_min_steps_risky) + 1)
        hold_lb, local_max = 7, 12
        reason = "near_risk_hold"
    elif regime["very_safe_cruise"]:
        hold_default = max(int(cfg.planner.hold_min_steps_very_safe), int(cfg.planner.hold_min_steps_default) + 20)
        hold_lb, local_max = max(32, hold_default - 4), 48
        reason = "very_safe_cruise_hold"
    elif regime["safe_cruise"]:
        hold_default = max(int(cfg.planner.hold_min_steps_safe_cruise), int(cfg.planner.hold_min_steps_default) + 12)
        hold_lb, local_max = max(24, hold_default - 4), 42
        reason = "safe_cruise_hold"
    else:
        hold_default = max(12, int(cfg.planner.hold_min_steps_default))
        hold_lb, local_max = 10, 22
        reason = "default_hold"
...
def _plan_expire_thresholds(cfg, scene: dict, regime: dict) -> tuple[float, float]:
    ...
    if regime["very_safe_cruise"]:
        fd_default = float(np.clip(max(100.0, front_distance * 0.35), 100.0, 260.0)) if front_distance < 1e5 else 120.0
        ttc_default = float(np.clip(max(12.0, ttc * 0.40), 12.0, 25.0))
    elif regime["safe_cruise"]:
        fd_default = float(np.clip(max(80.0, front_distance * 0.35), 80.0, 200.0)) if front_distance < 1e5 else 90.0
        ttc_default = float(np.clip(max(10.0, ttc * 0.35), 10.0, 20.0))
...
def translate_midlevel_command(cfg, scene: dict, command: dict, planner_state: dict, step_idx: Optional[int]) -> dict:
    ...
    if safe_cruise and current_overspeed_gap <= 0.0 and not near_overspeed:
        requested_speed = max(
            requested_speed,
            min(float(cfg.planner.target_speed_free), max(float(cfg.planner.safe_cruise_target_speed_floor), ego_speed))
        )
        if behavior == "follow":
            behavior = "keep_lane"
...
- For safe cruising keep_lane/follow scenes with front_distance >= 80m and ttc >= 15s, set hold_min_steps to 20-40.
