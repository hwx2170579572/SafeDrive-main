...
def run_compare(cfg, modes: Optional[List[str]] = None) -> dict:
    ...
    for idx, mode in enumerate(modes):
        ...
        for seed_idx, seed in enumerate(seeds):
            ...
            raw_run_rows.append(raw_row)
            shield_run_rows.append(shield_row)
            mode_raw_rows.append(raw_row)
            mode_shield_rows.append(shield_row)

            print(
                f"[COMPARE][DONE-RUN] mode={mode} seed={seed} "
                f"raw_success={raw_row['success']:.3f} "
                f"shield_success={shield_row['success']:.3f} "
                f"raw_collision={raw_row['collision']:.3f} "
                f"shield_collision={shield_row['collision']:.3f} "
        ...
        aggregated_raw_rows.append(agg_raw)
        aggregated_shield_rows.append(agg_shield)

        print(
            f"[COMPARE][DONE-MODE] mode={mode} runs={agg_raw['num_runs']} "
    ...
    raw_table = format_compare_table(aggregated_raw_rows, title="RAW POLICY TABLE")
    shielded_table = format_compare_table(aggregated_shield_rows, title="SHIELDED POLICY TABLE")
    merged_rows = merge_compare_reports(aggregated_raw_rows, aggregated_shield_rows)
    ...
    payload = {
        ...
        "shield_run_rows": shield_run_rows,
        "aggregated_raw_rows": aggregated_raw_rows,
        "aggregated_shielded_rows": aggregated_shield_rows,
    ...
    export_compare_csv(merged_rows, cfg.compare.save_csv_path)
    payload["latex_table"] = export_compare_latex(merged_rows, cfg.compare.save_latex_path)
    return payload
