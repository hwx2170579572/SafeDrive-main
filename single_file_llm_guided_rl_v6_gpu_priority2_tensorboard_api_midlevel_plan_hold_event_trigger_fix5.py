...
class PlannerConfig:
    ...
    hold_min_steps_default: int = 6
    hold_min_steps_risky: int = 3
    hold_max_steps: int = 20
    event_front_distance_margin: float = 2.0
    event_ttc_margin: float = 0.25
    event_closing_speed_delta: float = 2.0
    event_closing_speed_near_front_distance: float = 120.0
    event_closing_speed_near_ttc: float = 12.0
    event_rear_gap_delta: float = 6.0
    safe_cruise_front_distance: float = 120.0
    safe_cruise_ttc: float = 15.0
    safe_cruise_hold_min_steps: int = 12
    safe_cruise_front_distance_ratio: float = 0.50
    safe_cruise_front_distance_cap: float = 80.0
    safe_cruise_ttc_floor: float = 8.0
    safe_cruise_ttc_ceiling: float = 10.0
...
def build_plan_validity(cfg, scene: dict, command: dict, decision: dict) -> dict:
    ...
    hold_min_default = int(cfg.planner.hold_min_steps_risky if risk_mode >= 1 else cfg.planner.hold_min_steps_default)
    hold_min_steps = int(np.clip(int(command.get("hold_min_steps", hold_min_default)), 1, max(1, int(cfg.planner.hold_max_steps))))

    front_distance = float(scene.get("front_distance", 1e6))
    ttc = float(scene.get("ttc", 99.0))

    fd_default = max(float(cfg.planner.replan_front_distance), min(front_distance - 4.0, front_distance * 0.80)) if front_distance < 1e5 else float(cfg.planner.replan_front_distance)
    ...
    if ttc >= 90.0:
        ttc_default = float(cfg.planner.replan_ttc_threshold + 1.0)
    else:
        ttc_default = max(float(cfg.planner.replan_ttc_threshold), ttc - 0.75)
    ...
    target_lane = int(np.clip(int(decision.get("target_lane", scene.get("lane_id", 0))), 0, max(cfg.env.lanes_count - 1, 0)))
    current_lane = int(scene.get("lane_id", 0))
    changing = bool(target_lane != current_lane)
    expire_if_lane_feasibility_changes = bool(command.get("expire_if_lane_feasibility_changes", changing))
    expire_if_rear_gap_changes = bool(command.get("expire_if_rear_gap_changes", changing))
...
def postprocess_plan_validity(cfg, scene: dict, behavior: str, decision: dict, plan_validity: dict) -> dict:
    out = dict(plan_validity)
    current_lane = int(scene.get("lane_id", 0))
    target_lane = int(np.clip(int(decision.get("target_lane", current_lane)), 0, max(cfg.env.lanes_count - 1, 0)))
    front_distance = float(scene.get("front_distance", 1e6))
    ttc = float(scene.get("ttc", 99.0))
    no_lane_change_intent = bool(target_lane == current_lane and behavior in {"keep_lane", "follow", "brake", "yield"})
    safe_cruise = bool(
        no_lane_change_intent
        and front_distance > float(cfg.planner.safe_cruise_front_distance)
        and ttc > float(cfg.planner.safe_cruise_ttc)
    )
    if safe_cruise:
        out["hold_min_steps"] = int(max(int(out.get("hold_min_steps", cfg.planner.hold_min_steps_default)), int(cfg.planner.safe_cruise_hold_min_steps)))
        out["hold_max_steps"] = int(max(int(out.get("hold_max_steps", cfg.planner.hold_max_steps)), int(out["hold_min_steps"])))
        out["expire_if_lane_feasibility_changes"] = False
        out["expire_if_rear_gap_changes"] = False
        out["expire_if_front_distance_below"] = float(max(
            float(cfg.planner.replan_front_distance),
            min(front_distance * float(cfg.planner.safe_cruise_front_distance_ratio), float(cfg.planner.safe_cruise_front_distance_cap)),
        ))
        out["expire_if_ttc_below"] = float(np.clip(
            float(cfg.planner.safe_cruise_ttc_ceiling),
            max(float(cfg.planner.replan_ttc_threshold), float(cfg.planner.safe_cruise_ttc_floor)),
            float(cfg.planner.safe_cruise_ttc_ceiling),
        ))
    return out
...
def lane_feasibility_for_target(scene: dict, reference_lane: int, target_lane: int) -> tuple:
    if int(target_lane) < int(reference_lane):
        return ("left", scene.get("left_lane_id") is not None, bool(scene.get("left_clear", False)))
    if int(target_lane) > int(reference_lane):
        return ("right", scene.get("right_lane_id") is not None, bool(scene.get("right_clear", False)))
    return ("stay",)
...
def plan_event_state(cfg, plan_commit: Optional[dict], current_scene: dict, previous_scene: Optional[dict], shield_intervened: bool, force_replan: bool, step_idx: int) -> dict:
    ...
    events = []
    hard_events = []
    reference_scene = plan_commit.get("reference_scene", current_scene)
    prev_scene = previous_scene if previous_scene is not None else reference_scene
    validity = dict(plan_commit.get("plan_validity", {}))
    committed_command = dict(plan_commit.get("command", {}))
    ...
    reference_lane = int(reference_scene.get("lane_id", current_scene.get("lane_id", 0)))
    target_lane = int(committed_command.get("target_lane", reference_lane))
    behavior = _normalize_behavior_token(committed_command.get("behavior", "keep_lane"), default="keep_lane")
    lane_change_intent = bool(target_lane != reference_lane or behavior in {"change_left", "change_right"})
    if bool(validity.get("expire_if_lane_feasibility_changes", False)) and lane_change_intent:
        current_target_feas = lane_feasibility_for_target(current_scene, reference_lane, target_lane)
        reference_target_feas = lane_feasibility_for_target(reference_scene, reference_lane, target_lane)
        if current_target_feas != reference_target_feas:
            events.append("lane_feasibility_flip")

    ref_front_rel_speed = float(validity.get("reference_front_rel_speed", current_scene.get("front_rel_speed", 0.0)))
    current_front_rel_speed = float(current_scene.get("front_rel_speed", 0.0))
    near_field = bool(
        curr_fd < float(cfg.planner.event_closing_speed_near_front_distance)
        or curr_ttc < float(cfg.planner.event_closing_speed_near_ttc)
    )
    closing_speed_worsened = bool(
        current_front_rel_speed < (ref_front_rel_speed - float(cfg.planner.event_closing_speed_delta))
        and current_front_rel_speed < -1.0
    )
    if near_field and closing_speed_worsened:
    ...
    if hard_events:
        return {"valid": False, "state": "EMERGENCY", "events": hard_events + [e for e in events if e not in hard_events], "age": age}
    if age < hold_min_steps and len(events) == 0:
        return {"valid": True, "state": "COMMIT", "events": [], "age": age}
    if len(events) == 0:
        return {"valid": True, "state": "COMMIT", "events": [], "age": age}
    if age < hold_min_steps:
        return {"valid": True, "state": "MONITOR", "events": events, "age": age}
    return {"valid": False, "state": "REPLAN", "events": events, "age": age}
...
def translate_midlevel_command(cfg, scene: dict, command: dict, planner_state: dict, step_idx: Optional[int]) -> dict:
    ...
    plan_validity = build_plan_validity(cfg, scene, {**command, "risk_mode": int(risk_mode)}, {"target_lane": int(target_lane), "target_speed": float(target_speed)})
    plan_validity = postprocess_plan_validity(cfg, scene, behavior, {"target_lane": int(target_lane), "target_speed": float(target_speed)}, plan_validity)
    return {
        "command": {
            "behavior": behavior,
            "target_lane": int(target_lane),
...
6. For stable keep_lane/follow cruising with large front distance and large TTC, prefer longer holds instead of frequent replans.
...
- In stable cruising keep_lane/follow scenes, prefer hold_min_steps >= 12.
- Only set expire_if_lane_feasibility_changes=true when the committed plan actually depends on changing lanes.
