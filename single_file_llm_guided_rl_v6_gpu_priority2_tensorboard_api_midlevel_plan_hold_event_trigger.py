...
def plan_event_state(cfg, plan_commit: Optional[dict], current_scene: dict, previous_scene: Optional[dict], shield_intervened: bool, force_replan: bool, step_idx: int) -> dict:
    ...
    events = []
    hard_events = []
    prev_scene = previous_scene if previous_scene is not None else plan_commit.get("reference_scene")
    ...
    current_feas = lane_feasibility_signature(current_scene)
    ref_feas = plan_commit.get("reference_lane_feasibility", lane_feasibility_signature(plan_commit.get("reference_scene", current_scene)))
    if bool(validity.get("expire_if_lane_feasibility_changes", False)) and current_feas != ref_feas:
    ...
    if abs(float(current_scene.get("front_rel_speed", 0.0)) - ref_front_rel_speed) >= float(cfg.planner.event_closing_speed_delta):
...
- Choose a short local validity interval that is safe and conservative.
