...
class PlannerConfig:
    ...
    query_min_interval_steps: int = 6
    llm_periodic_query_mul: int = 2
    query_scene_front_delta_threshold: float = 8.0
    query_scene_speed_delta_threshold: float = 1.5
    query_scene_ttc_delta_threshold: float = 1.0
...
class LLMConfig:
    ...
    memory_fast_path_similarity_threshold: float = 0.88
    memory_fast_path_max_risk: float = 0.40
...
def canonical_protocol_dict(cfg: Config) -> dict:
    return {
        ...
        "llm": {
            ...
            "memory_fast_path_similarity_threshold": float(cfg.llm.memory_fast_path_similarity_threshold),
            "memory_fast_path_max_risk": float(cfg.llm.memory_fast_path_max_risk),
...
def apply_frozen_protocol_to_cfg(cfg: Config, protocol: dict) -> Config:
    ...
    for k in [
        ...
        "memory_fast_path_similarity_threshold", "memory_fast_path_max_risk",
...
def compute_scene_risk_vector(cfg, scene: dict, novelty: float = 0.0) -> dict:
    ...
    def _lane_change_risk(side: str) -> float:
        if side == "left":
            if scene.get("left_lane_id") is None:
                return 0.0
            ...
            if scene.get("right_lane_id") is None:
                return 0.0
    ...
    lane_stability = _clip01(abs(float(scene.get("ego_y", 0.0)) - float(scene.get("lane_center_y", 0.0))) / max(cfg.env.lane_width, 1e-6))
    ...
    valid_lane_change_risks = []
    if scene.get("left_lane_id") is not None:
        valid_lane_change_risks.append(risks["left_change"])
    if scene.get("right_lane_id") is not None:
        valid_lane_change_risks.append(risks["right_change"])
    driving_risk_terms = [
        risks["front_collision"],
        risks["headway"],
        risks["overspeed"],
        risks["lane_stability"],
        risks["scene_complexity"],
    ] + valid_lane_change_risks
    risks["max_risk"] = float(max(driving_risk_terms)) if driving_risk_terms else 0.0
    return risks


def _bounded_scene_distance(value: float, cap: float) -> float:
    return float(np.clip(float(value), 0.0, float(cap)))


def compute_scene_delta(cfg, prev_scene: Optional[dict], scene: dict) -> dict:
    if prev_scene is None:
        return {
            "front_distance_delta": float("inf"),
            "ego_speed_delta": float("inf"),
            "ttc_delta": float("inf"),
            "lane_changed": True,
            "clearance_changed": True,
            "large": True,
        }
    front_distance_delta = abs(
        _bounded_scene_distance(scene.get("front_distance", 1e6), 200.0)
        - _bounded_scene_distance(prev_scene.get("front_distance", 1e6), 200.0)
    )
    ego_speed_delta = abs(float(scene.get("ego_speed", 0.0)) - float(prev_scene.get("ego_speed", 0.0)))
    ttc_delta = abs(
        _bounded_scene_distance(scene.get("ttc", 99.0), 20.0)
        - _bounded_scene_distance(prev_scene.get("ttc", 99.0), 20.0)
    )
    lane_changed = int(scene.get("lane_id", 0)) != int(prev_scene.get("lane_id", 0))
    clearance_changed = (
        bool(scene.get("left_clear", False)) != bool(prev_scene.get("left_clear", False))
        or bool(scene.get("right_clear", False)) != bool(prev_scene.get("right_clear", False))
    )
    large = bool(
        lane_changed
        or clearance_changed
        or front_distance_delta >= float(cfg.planner.query_scene_front_delta_threshold)
        or ego_speed_delta >= float(cfg.planner.query_scene_speed_delta_threshold)
        or ttc_delta >= float(cfg.planner.query_scene_ttc_delta_threshold)
    )
    return {
        "front_distance_delta": float(front_distance_delta),
        "ego_speed_delta": float(ego_speed_delta),
        "ttc_delta": float(ttc_delta),
        "lane_changed": bool(lane_changed),
        "clearance_changed": bool(clearance_changed),
        "large": large,
    }
...
def _teacher_weight_from_guidance(cfg, command: dict, risk_vector: dict, novelty: float, recovery_source: str) -> float:
    ...
    base = confidence * (0.25 + float(cfg.llm.teacher_weight_risk_scale) * risk_max + float(cfg.llm.teacher_weight_novelty_scale) * float(novelty))
    if abstain:
        base *= 0.85
...
class TeacherQueryGate:
    ...
    def evaluate(self, scene: dict, step_idx: int, last_plan_step: Optional[int], last_command_confidence: float, last_scene_signature: str, current_signature: str, last_query_scene: Optional[dict]) -> dict:
        ...
        scene_delta = compute_scene_delta(self.cfg, last_query_scene, scene)
        reasons = []
        if step_idx % max(1, int(self.cfg.planner.planner_interval) * int(self.cfg.planner.llm_periodic_query_mul)) == 0:
            reasons.append("periodic")
        if risk_vector["max_risk"] >= float(self.cfg.llm.risk_query_threshold):
            reasons.append("risk")
        if novelty >= (1.0 - float(self.cfg.llm.novelty_similarity_threshold)):
            reasons.append("novelty")
        if float(scene.get("front_distance", 1e6)) < float(self.cfg.planner.replan_front_distance):
            reasons.append("front_distance")
        if float(scene.get("ttc", 99.0)) < float(self.cfg.planner.replan_ttc_threshold):
            reasons.append("ttc")
        if last_plan_step is not None and (step_idx - last_plan_step) >= max(1, int(self.cfg.planner.planner_interval) * 3):
            reasons.append("plan_stale")
        if last_command_confidence < float(self.cfg.llm.min_command_confidence):
            reasons.append("low_confidence")
        if last_scene_signature and current_signature != last_scene_signature and scene_delta["large"] and risk_vector["max_risk"] >= 0.35:
            reasons.append("scene_shift")

        immediate_hard_priority = bool(
            float(scene.get("front_distance", 1e6)) < float(self.cfg.planner.replan_front_distance)
            or float(scene.get("ttc", 99.0)) < float(self.cfg.planner.replan_ttc_threshold)
            or float(risk_vector.get("front_collision", 0.0)) >= max(float(self.cfg.llm.risk_query_threshold) + 0.15, 0.70)
            or last_command_confidence < float(self.cfg.llm.min_command_confidence)
        )
        cooldown_active = last_plan_step is not None and (step_idx - last_plan_step) < int(self.cfg.planner.query_min_interval_steps)
        if cooldown_active and not immediate_hard_priority:
            reasons = []

        if last_query_scene is not None and (not scene_delta["large"]) and not immediate_hard_priority:
            reasons = []

        best_similarity = float(memory_context.get("best_similarity", 0.0))
        use_memory_fast_path = bool(
            reasons
            and len(memory_context.get("success_cases", [])) > 0
            and best_similarity >= float(self.cfg.llm.memory_fast_path_similarity_threshold)
            and float(risk_vector.get("max_risk", 0.0)) <= float(self.cfg.llm.memory_fast_path_max_risk)
            and not immediate_hard_priority
        )
        return {
            "should_query": bool(reasons),
            "use_memory_fast_path": use_memory_fast_path,
            "reasons": reasons,
            "risk_vector": risk_vector,
...
def translate_midlevel_command(cfg, scene: dict, command: dict, planner_state: dict, step_idx: Optional[int]) -> dict:
    ...
    if speed_mode == "free":
        ...
        speed_cap = float(min(cfg.planner.speed_max, cfg.planner.target_speed_cautious + 3.0))
    else:
        speed_cap = float(min(cfg.planner.speed_max, cfg.planner.target_speed_brake + 3.0))
    ...
    speed_max = float(cfg.planner.speed_max if speed_mode == "free" else min(cfg.planner.speed_max, target_speed + 2.0))
    constraints = {
...
class PromptBuilder:
    ...
    def template_signature(self) -> str:
        ...
        return PROMPT_TEMPLATE_VERSION + "\n" + str(self.cfg.llm.system_prompt) + "\n" + self.build(dummy_scene, memory_context={"success_cases": [], "failure_cases": [], "novelty": 0.0, "best_similarity": 0.0, "num_items": 0}, query_context={"risk_vector": compute_scene_risk_vector(self.cfg, dummy_scene, novelty=0.0), "reasons": ["initial"]})
    ...
    def build(self, scene: dict, memory_context: Optional[dict] = None, query_context: Optional[dict] = None) -> str:
        ...
        query_context = query_context or {"risk_vector": compute_scene_risk_vector(self.cfg, scene, novelty=float(memory_context.get("novelty", 0.0))), "reasons": []}
...
Goal:
Output one mid-level driving command only. The downstream controller will deterministically translate your command into local waypoints, continuous teacher actions, and safety constraints for reinforcement learning.
...
5. If uncertain, abstain and choose follow/brake behavior.
...
- query_reasons: {json.dumps(list(query_context.get('reasons', [])), ensure_ascii=False)}
...
class LLMPlanner:
    ...
    def __init__(self, cfg, backend: BaseLLMBackend, memory_readonly: bool = False):
        ...
        self.last_query_scene: Optional[dict] = None
    ...
    def _scene_descriptor(self, scene: dict) -> dict:
        front_distance = float(scene.get("front_distance", 0.0))
        ttc_raw = float(scene.get("ttc", 99.0))
        ego_speed = float(scene.get("ego_speed", 0.0))
        left_front = float(scene.get("left_front_distance", 1e6))
        right_front = float(scene.get("right_front_distance", 1e6))
        left_rear = float(scene.get("left_rear_distance", 1e6))
        right_rear = float(scene.get("right_rear_distance", 1e6))
        risk_mode = _scene_risk_mode(self.cfg, scene)
        return {
            "lane_id": int(scene.get("lane_id", 0)),
            "front_distance_bucket": int(min(20, max(0, front_distance // 10))),
            "ttc_bucket": 20 if ttc_raw >= 40.0 else int(max(0, ttc_raw // 2)),
            "ego_speed_bucket": int(min(20, max(0, ego_speed // 2))),
            "left_front_bucket": int(min(20, max(0, left_front // 10))),
            "right_front_bucket": int(min(20, max(0, right_front // 10))),
            "left_rear_bucket": int(min(20, max(0, left_rear // 10))),
            "right_rear_bucket": int(min(20, max(0, right_rear // 10))),
            "left_clear": int(bool(scene.get("left_clear", False))),
            "right_clear": int(bool(scene.get("right_clear", False))),
            "risk_mode": int(risk_mode),
        }
    ...
    def _scene_signature(self, scene: dict) -> str:
        desc = self._scene_descriptor(scene)
        return "|".join([f"{k}={desc[k]}" for k in sorted(desc.keys())])

    def _cache_put(self, scene: dict, parsed: dict, step_idx: Optional[int]) -> None:
        signature = self._scene_signature(scene)
        descriptor = self._scene_descriptor(scene)
        self.semantic_cache[signature] = {"signature": signature, "descriptor": descriptor, "step_idx": None if step_idx is None else int(step_idx), "command": copy.deepcopy(parsed.get("command", {}))}
        while len(self.semantic_cache) > max(1, int(self.cfg.llm.semantic_cache_max_size)):
            first_key = next(iter(self.semantic_cache))
            self.semantic_cache.pop(first_key, None)
        self.last_plan_step = None if step_idx is None else int(step_idx)
        self.last_command_confidence = float(parsed.get("command", {}).get("confidence", 1.0))
        self.last_scene_signature = signature

    def _descriptor_distance(self, a: dict, b: dict) -> Tuple[int, bool]:
        hard_match = int(a.get("lane_id", 0)) == int(b.get("lane_id", 0)) and int(a.get("left_clear", 0)) == int(b.get("left_clear", 0)) and int(a.get("right_clear", 0)) == int(b.get("right_clear", 0)) and int(a.get("risk_mode", 0)) == int(b.get("risk_mode", 0))
        dist = abs(int(a.get("front_distance_bucket", 0)) - int(b.get("front_distance_bucket", 0))) + abs(int(a.get("ttc_bucket", 0)) - int(b.get("ttc_bucket", 0))) + abs(int(a.get("ego_speed_bucket", 0)) - int(b.get("ego_speed_bucket", 0))) + abs(int(a.get("left_front_bucket", 0)) - int(b.get("left_front_bucket", 0))) + abs(int(a.get("right_front_bucket", 0)) - int(b.get("right_front_bucket", 0))) + abs(int(a.get("left_rear_bucket", 0)) - int(b.get("left_rear_bucket", 0))) + abs(int(a.get("right_rear_bucket", 0)) - int(b.get("right_rear_bucket", 0)))
        return dist, hard_match

    def _cache_reuse_age_ok(self, payload: dict, step_idx: Optional[int]) -> bool:
        cached_step = payload.get("step_idx")
        if step_idx is None or cached_step is None:
            return True
        return (int(step_idx) - int(cached_step)) <= int(self.cfg.llm.cache_reuse_max_age_steps)

    def _cache_get(self, scene: dict, step_idx: Optional[int]) -> Optional[dict]:
        signature = self._scene_signature(scene)
        current_desc = self._scene_descriptor(scene)
        payload = self.semantic_cache.get(signature)
        if payload is not None and self._cache_reuse_age_ok(payload, step_idx):
            state = _ensure_runtime_llm_state(self.cfg)
            state["num_cache_exact_hits"] = int(state.get("num_cache_exact_hits", 0)) + 1
            out = translate_midlevel_command(self.cfg, scene, copy.deepcopy(payload.get("command", {})), self.state, step_idx)
            out.update({"recovery_source": "semantic_cache_exact", "cache_hit": True, "cache_source": "exact", "cache_signature": signature})
            return out
        if int(current_desc.get("risk_mode", 0)) >= 1:
            return None
        best_payload, best_dist = None, 10**9
        for cached in self.semantic_cache.values():
            if not self._cache_reuse_age_ok(cached, step_idx):
                continue
            dist, hard_match = self._descriptor_distance(current_desc, dict(cached.get("descriptor", {})))
            if hard_match and dist < best_dist:
                best_dist, best_payload = dist, cached
        if best_payload is None or best_dist > 3:
            return None
        state = _ensure_runtime_llm_state(self.cfg)
        state["num_cache_near_hits"] = int(state.get("num_cache_near_hits", 0)) + 1
        out = translate_midlevel_command(self.cfg, scene, copy.deepcopy(best_payload.get("command", {})), self.state, step_idx)
        out.update({"recovery_source": "semantic_cache_near", "cache_hit": True, "cache_source": "near", "cache_similarity_distance": int(best_dist), "cache_signature": str(best_payload.get("signature", ""))})
        return out
    ...
    def _translate_legacy_response(self, scene: dict, obj: dict, step_idx: Optional[int]) -> dict:
        ...
        command = {"behavior": behavior, "target_lane": target_lane, "target_speed": target_speed, "speed_mode": speed_mode, "headway_level": "conservative" if float(constraints.get("min_headway", self.cfg.planner.normal_headway)) >= self.cfg.planner.conservative_headway else "normal", "risk_mode": int(constraints.get("risk_mode", 0)), "confidence": 0.70, "abstain": False, "reason": str(decision.get("reason", "legacy llm decision"))}
    ...
    def _schema_hint(self) -> str:
        return '{"command":{"behavior":"keep_lane|change_left|change_right|follow|brake|yield","target_lane":0,"target_speed":20.0,"speed_mode":"free|cautious|brake","headway_level":"normal|conservative","risk_mode":0,"confidence":0.8,"abstain":false,"reason":"..."}}'
    ...
    def _recover_after_parse_failure(self, raw_response: str, scene: dict, prompt_hash: str, step_idx: Optional[int], error: Exception) -> dict:
        ...
        cached = self._cache_get(scene, step_idx)
        if cached is not None:
            return cached
        if str(self.cfg.workflow.stage).lower() == "formal":
            raise RuntimeError(f"Formal run cannot recover planner parse failure without repair/cache: {type(error).__name__}: {error}")
        state = _ensure_runtime_llm_state(self.cfg)
    ...
    def _maybe_replace_low_confidence_plan(self, parsed: dict, scene: dict, step_idx: Optional[int]) -> dict:
        ...
        cached = self._cache_get(scene, step_idx)
        if cached is not None:
            cached["recovery_source"] = "low_confidence_cache"
            return cached
        if str(self.cfg.workflow.stage).lower() == "formal":
            raise RuntimeError("Formal run encountered low-confidence real-LLM command without valid cache reuse.")
        state = _ensure_runtime_llm_state(self.cfg)
        state["num_safe_rule_degradations"] = int(state.get("num_safe_rule_degradations", 0)) + 1
    ...
    def _build_query_context(self, scene: dict, step_idx: Optional[int]) -> dict:
        memory_context = self.memory.retrieve(scene, top_k=self.cfg.llm.memory_top_k)
        novelty = float(memory_context.get("novelty", 0.0))
        return {
            "memory_context": memory_context,
            "risk_vector": compute_scene_risk_vector(self.cfg, scene, novelty=novelty),
            "novelty": novelty,
            "reasons": ["initial_plan"] if step_idx is None else [],
            "use_memory_fast_path": False,
            "scene_delta": compute_scene_delta(self.cfg, self.last_query_scene, scene),
        }

    def should_replan(self, scene: dict, step_idx: int) -> bool:
        if step_idx <= 0:
            return False
        current_signature = self._scene_signature(scene)
        gate = self.query_gate.evaluate(
            scene=scene,
            step_idx=step_idx,
            last_plan_step=self.last_plan_step,
            last_command_confidence=self.last_command_confidence,
            last_scene_signature=self.last_scene_signature,
            current_signature=current_signature,
            last_query_scene=self.last_query_scene,
        )
        self.last_trigger_reason = "+".join(gate["reasons"]) if gate["reasons"] else "none"
        self.last_query_context = {
            ...
            "use_memory_fast_path": bool(gate.get("use_memory_fast_path", False)),
            "scene_delta": dict(gate.get("scene_delta", {})),
        }
    ...
    def _build_memory_fast_path_plan(self, scene: dict, query_context: dict, step_idx: Optional[int]) -> dict:
        memory_context = dict(query_context.get("memory_context", {}))
        success_cases = list(memory_context.get("success_cases", []))
        if len(success_cases) == 0:
            raise RuntimeError("memory fast path requested without successful memory cases")
        best_case = dict(success_cases[0])
        risk_mode = _scene_risk_mode(self.cfg, scene)
        behavior = _normalize_behavior_token(best_case.get("behavior", "follow"), default="follow")
        target_lane = int(np.clip(int(best_case.get("target_lane", scene.get("lane_id", 0))), 0, max(self.cfg.env.lanes_count - 1, 0)))
        target_speed = float(np.clip(float(best_case.get("target_speed", scene.get("ego_speed", self.cfg.planner.target_speed_cautious))), self.cfg.env.min_speed, self.cfg.planner.speed_max))
        speed_mode = "brake" if risk_mode >= 2 else ("cautious" if behavior in {"follow", "brake", "yield"} or target_speed <= self.cfg.planner.target_speed_cautious + 2.0 else "free")
        headway_level = "conservative" if (behavior in {"follow", "brake", "yield"} or risk_mode >= 1) else "normal"
        command = {
            "behavior": behavior,
            ...
            "headway_level": headway_level,
            "risk_mode": int(risk_mode),
            "confidence": float(np.clip(0.75 + 0.20 * float(memory_context.get("best_similarity", 0.0)), 0.0, 0.95)),
            "abstain": False,
            "reason": f"memory fast path from similar successful cases (sim={float(memory_context.get('best_similarity', 0.0)):.3f})",
        }
        parsed = translate_midlevel_command(self.cfg, scene, command, self.state, step_idx)
        parsed.update({
            "recovery_source": "memory_fast_path",
            "cache_hit": True,
            "cache_source": "memory_fast_path",
        })
        return parsed
    ...
    def plan(self, scene: dict, step_idx: Optional[int] = None) -> dict:
        ...
        use_memory_fast_path = bool(query_context.get("use_memory_fast_path", False))

        prompt = ""
        raw_response = ""
        prompt_hash = ""
        response_hash = ""
        if use_memory_fast_path:
            parsed = self._build_memory_fast_path_plan(scene, query_context, step_idx)
            ...
            append_llm_call_log(self.cfg, {
                ...
                "stage": str(self.cfg.workflow.stage),
                "mode": str(self.cfg.train.mode),
                "backend": "memory_fast_path",
                "backend_requested": str(getattr(self.cfg.llm, "backend", self.backend.name)),
                "backend_effective": "memory_fast_path",
                "status": "success",
                "seed": int(self.cfg.train.seed),
                "step_idx": None if step_idx is None else int(step_idx),
                "best_similarity": float(memory_context.get("best_similarity", 0.0)),
                "success_cases": len(memory_context.get("success_cases", [])),
                "reasons": list(query_context.get("reasons", [])),
            })
        else:
        ...
        if parsed.get("recovery_source") in {"direct", "json_repair", "memory_fast_path"}:
            self._cache_put(scene, parsed, step_idx)
        else:
            self.last_plan_step = None if step_idx is None else int(step_idx)
        ...
        if not self.memory_readonly and recovery_source in {"direct", "json_repair", "semantic_cache_exact", "semantic_cache_near", "low_confidence_cache", "memory_fast_path"}:
        ...
        self.last_query_scene = copy.deepcopy(scene)
        ...
        return {
            ...
            "planner_info": {
                ...
                "cache_hit": bool(parsed.get("cache_hit", False)),
                "cache_source": str(parsed.get("cache_source", "none")),
...
def teacher_schedule_factor(cfg, episode_idx: int) -> float:
    ...
    if progress <= ratio:
        factor = 1.0 - 0.5 * (progress / max(ratio, 1e-6))
    else:
        tail = (progress - ratio) / max(1.0 - ratio, 1e-6)
        factor = 0.5 * (1.0 - tail) + min_weight * tail
